...
/// §7.2): where an object lives on the forest — its routing slot, the
/// volume, tree 0's lessee word for the slot, and the stripe count of a
/// striped directory.
#[derive(Debug, Clone)]
pub struct LocateReport {
    ...
    /// Tree 0's word on the slot: the leasing appender's id (`None` =
    /// unleased — the manager maintains the tree) and the lease generation
    /// `g`. `None` on a volume whose tree 0 names no record for the slot.
    pub lessee: Option<crate::meta_backend::kv::backend::SlotLesseeWord>,
    /// The directory's stripe count `K` when it is striped (§5.6.5).
    pub stripes: Option<u16>,
}

/// `squeezefs volume locate <sqmeta-uri> <path>` — slot, volume, live
/// lessee and `g` (plus the stripe count of a striped directory).
///
...
async fn locate_report(
    ...
    let slot = probes.slot_of_ino(ino);
    let lessee = vol
        .slot_lessee(vol.forest_slot_of_routing(slot as u16))
        .await?;
    let stripes = probes.stripe_map(ino).await?.map(|m| m.k());
    Ok(LocateReport {
        path,
        ino,
        slot,
...
async fn remove_sym_marker(path: &str, vi: usize, flocks: &mut VerbFlocks) -> Result<()> {
    ...
    let body = async {
        be.removexattr_internal(1, crate::SYM_UPGRADE_MARKER_XATTR)
            .await?;
        // The retired `volume set-owners` verb's bracket (pre-1.3.0): the
        // forest gives it no meaning (§7.3) and the writable mount gate
        // refuses while it stands, so the conversion is where it goes.
        if let Ok(Some(_)) = be.getxattr(1, crate::OWNER_ASSIGN_MARKER_XATTR).await {
            be.removexattr_internal(1, crate::OWNER_ASSIGN_MARKER_XATTR)
                .await?;
        }
        be.checkpoint_now().await.map_err(|e| {
            SqueezefsError::InvalidOperation(format!(

...
engagement gates; armed rows gauge the direct+extract VEHICLE SPLIT;
per-row amp = device÷user bytes and wareq-sz per the standing law).
Rows per write leg: seqwr 1M / dur (fsync_on_close) / rand4k (HOLE
regime — the previous bracket's shape, kept for comparability) /
rand4kow (prewritten durably-published fileset — the overwrite regime
where W1/D14 engage). Reference DO-NOT-FLIP bracket:
0.998×/0.909×/0.936× (`.benchmarks/2026-08-06-fuse-zc-write-bracket.md`).

> RESULTS PENDING — filled by the bracket run below.

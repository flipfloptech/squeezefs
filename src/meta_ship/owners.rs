...
use crate::membership::member_id_matches;
...
impl OwnerMap {
    ...
    pub fn for_volumes(
        ...
        Ok(Self::build(routed, volume_owners))
    }

    /// The constructor's tail: derive the local slot set from the live
    /// slot map and freeze the width.
    fn build(routed: &RoutedMetaBackend, volume_owners: Vec<Option<Arc<PeerOwner>>>) -> Arc<Self> {
...
/// **The era-relearn follow-up**: this client learned a NEW era from
/// `peer_id`, which means that peer failed over. Under the symmetric plane
/// every volume of the set is local to its writer (the map is all-local),
/// so the follow-up is the log line — a foreign entry naming the peer would
/// be the retired per-volume-owner recipe's.
pub fn note_era_relearn(peer_id: &str) {
    if let Some(map) = owner_map() {
        if !map.volumes_owned_by(peer_id).is_empty() {
            log::warn!(
                "ownership map: '{peer_id}' relearned its era (failed over) while the installed \
                 map names it as a volume owner"
            );
        }
    }
}
...
///
/// The production issuer is the symmetric join ladder's rung 7
/// (`multi_writer::arm_authority_planes`), over an all-local map.
///
/// **Assumption the migration engine honours:** a slot migration while armed moves
...
pub fn rearm_ownership(routed: &RoutedMetaBackend) -> Result<()> {
    ...
    let foreign: Vec<(usize, PeerOwner)> = (0..map.volume_count())
        .filter_map(|v| map.owner_of_volume(v).map(|p| (v, (**p).clone())))
        .collect();
    let fresh = OwnerMap::for_volumes(routed, foreign)?;
    arm_ownership(fresh);
    Ok(())
}

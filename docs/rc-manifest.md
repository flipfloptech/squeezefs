...
| S9 | Multi-writer data plane | **LANDED, two field blockers** | `src/data_grant.rs` + `src/meta_ship/publish.rs` + `src/multi_writer.rs`: the custody state machine (grant/renew/revoke/expire), remote adoption ordered **adopt era → raise stripe floor → admit with the OWNER's token** so nothing on a client mints custody, S11's range algebra reused, S7's single `authorize_dma` door untouched, and S8's `arm_ownership` finally given its caller. Revocation is **pull-based by design**: a revoked co-writer's *belief* can outlive the revoke by one renewal cadence (bounded by its own `T_self`), its **writes cannot** (bounded by the device) — both directions pinned. **Blockers named, not hidden:** (1) the D0 Layer-B2 gate refuses a fresh foreign claim unconditionally, so a co-writer cannot open the metadata volumes at all; (2) no data-plane allocation partition, so two allocators collide on fresh offsets. Both are in flight |

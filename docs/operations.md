...
- **What IS per-uid.** Exactly two things, both on the interception control plane, both admission caps rather than accounting: `per_uid_session_cap` (concurrent IPC sessions per uid, default 64) and the `ipc_session_arenas` R5 component's shed = refuse-new-sessions behavior. Neither bounds the FUSE path.

...
* **NVMe-oF Utilities (top-level `squeezefs nvmeof`, dual-stack):**
  Share and dismantle NVMe-oF target subsystems, manage the target runtime, and connect clients (see [QUICKSTART §4](QUICKSTART.md#4-nvme-of-fabric-setup-remote-block-storage)). Target-stack selection is explicit — `--target-stack {spdk|nvmet}` (or `SQUEEZEFS_NVMEOF_TARGET_STACK`), default **spdk** — and failure is loud: there is **no silent cross-stack fallback**. ⚠️ **Interim (target-management program in flight):** as of milestone **N3** the SPDK **target lifecycle verbs** below are live (pinned v26.05 build with commit-sha verification, hugepage setup with recorded prior, pidfile start/stop, status, systemd-unit emission); SPDK **sharing** lands with milestone **N4** — until then `share`/`unshare`/`restore` on the spdk stack (including the default) fail loud pointing at the live target verbs, and `--target-stack nvmet` remains the sharing path (rebuilt configfs path: reserved port-id range 53000–53999 with ownership checks, `resv_enable`+`device_uuid` stamped before enable, write-ahead intent ledger, working `restore`).
  ```bash
...
| — SPDK-served namespace (`nvmeof share --target-stack spdk` — the target lifecycle verbs are live as of milestone N3; SPDK *sharing* lands with N4; measured on the scoping rig) | **Enforcement-grade, measured** (2026-07-17 rig: mount `flock+pr`, fencing EBADE class, preempt, ladder crash-remount ×10) — and with a `ptpl_file`-served namespace, reservations **persist through target restarts** (PTPL; a live holder rides out `spdk_tgt` kill + `load_config` with `writer_guard_fenced=0`). Note product SPDK sharing does not yet pin ns UUID / `ptpl_file` — that lands with N4 of the SPDK-target program (scoping §5) |

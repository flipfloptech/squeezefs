...
**Deferred to a named stage:** DUR-7 → S3.5 — **now CLOSED there** (2026-08-05: `src/meta_backend/crossvol_tx.rs`, `docs/design-cow-kv-metadata.md` §4.10a, `tests/crossvol_tx_tests.rs`; the handoff note's crash-window enumeration and `SQZXTX01` wire shipped, and the seam gate indeed needed no new harness) · RES-16's remaining half → S3 · POSIX-14's handle allocation → assessed and argued sound under FUSE-2 + 3k.
...
|---|---|---|---|
| Single writer, single mount | Full POSIX; whole-tx atomic metadata; torn-write immune. **Since the PR-14 flip a plain `format` stamps the symmetric forest (bit 17 + the nine multi-writer bits) and a plain `mount` is ONE armed writer** — the manager of its own set: `dlm_rpcs == 0` by construction (every slot its own), the must-stay-0 set 0; `--single-writer` is the flat opt-out for a volume that will never see a second host. The stamped-solo cost was measured within noise at the rung-10b flip; the PR-1 solo re-gate on the forest is MET (`.benchmarks/2026-09-13-sym-forest.md`) | (i) | **Shipped** — `.benchmarks/2026-08-15-mw-s4-regate.md`, `.benchmarks/2026-09-13-sym-forest.md`; the flipped default's local solo re-gate is PR 14's act 7 (the box bracket on the flip binary owed) |
| Concurrent PLAIN write mounts of one volume set | **Every RW mount of an armed set is a WRITER** (design-symmetric-metadata §7.3 — the join ladder: the D0 winner is the MANAGER, every later RW mount JOINS it over the wire as a full writer with its own ring, page, checkpoint task and listener, N unbounded by design); a set's `--single-writer` volumes keep the D0 refusal (flock + PR where available). No role knob, no admission ladder beyond the join's rungs | (i) — N = 2 and 3 in one process (`sym_n_daemon_tests`), N = 3 processes on kernel nvmet (the fidelity tier's `sym-join-ladder` / `sym_n_daemon_leg`), a manager + 3 joined writers + a reader on the fleet rig (`sym-crash` / `sym-storm`), TWO HOSTS on one nvme-tcp LUN (`sym-two-host`: a qemu/KVM guest as the second kernel) | **Shipped** — `.benchmarks/2026-09-16-sym-pr12-mount-posture.md`, `.benchmarks/2026-09-17-sym-pr12b-n-daemon.md`, `.benchmarks/2026-09-19-sym-acceptance.md` (+ §3.10 the two-host row) |
| One writer + N coherent readers | A `-o ro` mount is a member-reader + READ-TOKEN client (GPFS-strict, R-SYM-4 — the ONLY foreign-read method): user-visible metadata is EXACT at the next resolve, `reader_staleness_bound_ms` reads 0 (the S5 poll survives as the CONTROL plane's cadence); a `--single-writer` volume refuses a `-o ro` mount at the token arm naming `enable-symmetric` | (i) — 1 × 31 token readers on the fleet rig (`sym-readers`: 0 misses, 155 ≡ 155 recalls) | **Shipped** (PR 5 + PR 12's per-holder binding) — `.benchmarks/2026-09-15-sym-pr5-tokens.md`, `.benchmarks/2026-09-19-sym-acceptance.md` gate 5 |
| Multi-writer, disjoint file sets (D1's shape) | N joined writers: every writer mints in its own leased slots (KD-SYM-1/2), allocates from ranged BLOCK GRANTS its data volume's allocation-lease holder carves (PR 8), DMAs directly under slot-holder custody (PR 9), ships a foreign slot's mutation as a cross-owner intent (PR 6) or a record-level metanode ship (PR 13b); a dead writer's ring is recovered by the set itself (PR 10); kill-9 / failover / seven-victim storms with fsck clean and the acked-writes oracle LOST 0 | (i) for the correctness rows (real mounts, real PR target, two kernels on the two-host leg); (ii) for fleet fan-out throughput (one box); (iii) for every 15 k number (SIM-1 at 12,500 × 64 with the death ledger + token recall fan-out, tier (ii)) | **Shipped** — gate table `.benchmarks/2026-09-19-sym-acceptance.md` §2 (the box brackets §3.9); the retired authority + co-writer recipe's record stays as history (`.benchmarks/2026-08-18-mw-program-closing.md`) |
| Multi-writer, ONE shared file (the MPI-IO shape) | EX byte-range custody: block-aligned spans DMA directly, sub-block-shared blocks demote to the assembler, required never trimmed, budgets derived + refuse-loud | (ii) — pinned ior 4.0.0, 32 ranks, one box; **verdict MET** (A-B-B-A ≥ 0.8× both brackets: 1.411× / 2.273×; read-back exact; fsck + C8 clean) | **Shipped dark** (`SQUEEZEFS_RANGE_CUSTODY` default-off; the flip gates on the indirect-map width-N composition) — `.benchmarks/2026-08-18-s11-mpiio-row.md` + `.benchmarks/2026-08-18-s11-widthn-refs-fix.md` |
| Serial metadata latency on a joined writer | A writer's own-slot metadata is LOCAL (its own ring, its own leases); only a foreign slot's mutation pays a ship. The S8 R1 row ("remote clients are throughput-oriented") and the S10 6.73× `tar -x` verdict described the retired authority + co-writer topology, where a client held no metadata volume; on the symmetric plane the shared-directory rows are gate 3b's (3,400–3,581 creates/s into one directory on the box, one flip) | (i) | **Superseded** — `.benchmarks/2026-09-19-sym-acceptance.md` §2 rows 2 / 3 / 3b (the S8-a / S10 rows are history) |
| Cross-host arbitration on non-PR substrates | Detection-grade only: the join ladder's rung 4 REFUSES a second host without a reservation-capable data namespace unless `SQUEEZEFS_SYM_ALLOW_NON_PR=1` (the LOUD lab opt-in, announced at every mount); a file-backed volume is one kernel by construction and needs no opt-in | (i) | Design law (KD-SYM-13), enforced |

**Note for the public RC:** the current cluster's namespaces expose no `resv_enable` knob, so the reset-v5 venue itself runs PR-less — every multi-writer claim must therefore cite a PR-capable substrate. The MW proving fleet's substrate is nvmet-tcp on localhost with `resv_enable=1` (a real kernel PR target — WERO rejection, preemption and PTPL-class behavior are exercised for real); what it cannot supply is multi-host fabric physics, which §12 of the design classifies as a perf concern, not a correctness one.

---

## 3. Format / incompat ledger

**The nine multi-writer bits (7–15) are stamped by the DEFAULT `format` since
...
| 7 | `WriterClaim.term` | at format (10b flip) | DLM **S2** — durable writer term |
| 8 | `KV_PARTITIONED_APPEND` | at format (10b flip) | Multi-writer §6.2 items 2/3/4 |
| 9 | `KV_BLOCK_REFCOUNTS` | at format (10b flip) | Multi-writer §6.2 item 1. Renumbered from 8 after a same-wave collision — two definitions of one bit is silent aliasing, so every claim now carries a disjointness assertion. The stamped ledger ran drift-free live under the S4 re-gate |
| 10 | `KV_WRITER_SCOPED_STAGING` | at format (10b flip) | Multi-writer §6.2 items 8/10 (keys **and** the node-scoped generation stamp share one bit: a half-engaged state is unsound in both directions) |
| 12 | `KV_INO_LANES` | at format (10b flip) | Multi-writer §6.2 item 5 — per-writer ino lanes |
| 13 | `KV_BLOCK_KEY_INCARNATION` | at format (10b flip) | Multi-writer §6.2 item 6 — `offset ‖ incarnation` block keys (retires bare-offset key reuse on the stamped class). Both authored themselves at 10/11 and renumbered at integration: the **third** parallel claim, and the second the union pin caught on contact |
| 14 | `KV_CLAIM_SET` | at format (10b flip) | Multi-writer §6.2 **item 7** — the claim-set record (membership, not exclusion), DLM S6's capability gate. Renumbered from 12 at integration: the **fourth** parallel claim |
| 15 | `KV_LAYOUT_VERSIONS` | at format (10b flip) | Multi-writer §6.2 **item 9** — era-composed layout-delta base versions. The first parallel-safe claim: all four pins updated by its own author |
| 16 | `KV_BLOCK_MAP_TREE` | on the first PB-class block map | The block-map KV tree (`kvmap`) — the indirect map's successor for the PB-class file |
| 17 | `KV_SYMMETRIC_FOREST` | **at format (PR-14 flip), presence-REQUIRED on the multi-writer class** | The symmetric forest (design-symmetric-metadata §7): one slot tree per routing slot + tree 0, the appender region, the manager lease, slot leases, tokens, block grants, custody by the slot holder, dead-appender recovery, the record ship. Stamped by the DEFAULT `format` since PR 14 (`--single-writer` = the flat class, never a forest); a multi-writer-class volume WITHOUT it refuses the writer's door naming `squeezefs volume enable-symmetric` (the offline conversion) / `format --force` — readers, probes, fsck and the conversion keep reading it. Dark from PR 1 (2026-09-13) to the flip |
| 11 | `KV_MULTI_WRITER_DATA` | at format (10b flip) | DLM **S7**'s capability gate — the join ladder's rung 2 refuses a writer without it (rung 4 refuses a second host on a non-PR substrate without the loud lab opt-in; the retired `SQUEEZEFS_MULTI_WRITER=1` arm read the same bit). Renumbered from 10 at integration (the second parallel claim; this one was caught by the disjointness pin's union clause going red, not by reading a diff) |
...
| S3.5 | Cross-volume transaction machinery (ruling D4) | **LANDED** | `SQZXTX01` intent rides step 0's commit; roll-forward recovery at mount through one witnessed applier; **DUR-7 closed** (unlink/rmdir, link, cross-parent rename incl. whiteout, exchange) with the two-volume commit-boundary seam suite; no incompat bit, +1 entry per cross-volume op. The class it deliberately left owed — cross-volume `create` stays un-wrapped — **landed as fsck C9** (unreferenced inodes: one dentry pass + the census's inode walk, candidate filter = the writer era's ino floor so a live create cannot false-positive; repair destroys the record and reclaims its blocks quarantine-first). C9 is also the **only detector, and the only cleanup path, for pre-S3.5 field damage** (a crash mid cross-volume link/unlink leaked the inode AND its blocks permanently) — the reason it was a P1 blocker. The gap C9's own author then flagged — the same damage's `nlink`-vs-names shapes, of which one is DATA LOSS rather than a leak (a name whose inode's count no longer covers it becomes reclaimable once ordinary unlinks reach 0; S3.5's `24ef223c` made such a dentry removable and nothing FOUND one) — **is closed as fsck C10** (inode-plane reference consistency: both directions, `nlink == 0` with a live name, and dangling names, all riding C9's single dentry pass; zero-FP by the record witness + the open-intent exemption rather than the era floor; raising a count is safe, lowering runs only on a third agreeing pass, dangling names are removable only). S8's cross-volume verbs call `crossvol_tx::execute`; a REMOTE participant still waits on S8's wire |
...
| S6 | Membership off the journal (lease-based liveness) | **LANDED, default off — ARMED & PROVEN 2026-08-16 (tier: measured-real)** | **The rung-7 arm rows** (`.benchmarks/2026-08-16-mw-s6-arm.md`): S6-a at N=32 — 1,868 renewals against 185 owner-residue journal entries = **0.099 journal txs/beat** (the §6.5 455-beats/s serialization structurally gone), registration commits 0, readers VISIBLE in the census; S6-b — the self-fence clock law under netem 200 ms + SIGSTOP (`membership_self_fences` exactly 1 on the victim, `T_self 42955 < T_owner 45000` asserted from the published gauges); S6-b′ — the hung-KERNEL shape via qemu pause (an independent kernel AND clock domain): the frozen member **fenced + purged FIRST, then re-joined fresh under a new epoch** — the pre-fix binary resumed it as a live member on stale caches, which is finding #1 of the five the rows convicted (all red-first pinned; #4 is the KD-MW-14 fleet-share cgroup-arm divide). `src/membership{,_wire,_sim}.rs`. Liveness left the journal entirely: a durable registration written ONCE at arm (one `membership_owner` xattr per volume — the rendezvous DISC-1 needs) plus lease renewal over `cluster_wire`, lease table in RAM. The rejected alternative is recorded as a category error: *durability of liveness* is meaningless, because the only consumer of a durable liveness record is a process that crashed, and a crashed process's answer is "dead" — which the ABSENCE of a renewal already says. Read side went from `listxattr(1)` + one `getxattr` per client under a shared `I{1}` lock to one `getxattr` + RAM-served census pages (pinned: 500 joins add **zero** root-ino xattr keys and **zero** journal entries). Two-clock asymmetry implemented with `skew_max` from a **physical** bound (500 ppm oscillator drift, not a tuning constant) and unsafe knob combinations **refusing** rather than clamping. §6.2 item 7's claim set rides bit 14 (stamped by the default format since the rung-10b flip), and un-engaged it is a *projection* of `writer_claim` so no `claim_set` key can appear and single-writer bytes are untouched. `SQUEEZEFS_MEMBERSHIP_BIND=off` by default ⇒ shipped behavior byte-identical. **No D0 guarantee row changed**: the plane answers *who is here*, never *who may write* |
| S7 | Data-plane custody-epoch fence + dead-epoch quarantine + WERO | **LANDED — ARMED & PROVEN 2026-08-16, the device-rejection gate MET (tier: measured-real)** | One authorization point; quarantine release needs a drain proof; pressure answer is ENOSPC. **The deferred device-rejection leg ran for real** (`.benchmarks/2026-08-16-mw-s7-arm.md`, spec risk R2's exact wording): a SIGSTOP-past-TTL writer's resumed DMA was rejected **by the namespace** (reservation-conflict/EBADE on a real nvmet-tcp kernel PR target), the zombie fail-stopped (`writer guard FENCED` + custody poison + `data_dma_fence_refusals` counting), and the PR preempt was observed on the target. S7-b: kill-9 ×10 from zero, fsck C1–C10 + the engaged C8 oracle green after EVERY kill. Four product findings red-first (incl. the EBADE `classify_dma_outcome` fence latch — the device's rejection was previously any-other-I/O-error — and the durable claim-identity law that stopped phantom-writer accretion under crash loops) |
| S8 | Metadata function shipping | **LANDED — ARMED & PROVEN 2026-08-16, the R1 row PUBLISHED (tier: measured-real)** | **The rung-9 arm** (`.benchmarks/2026-08-16-mw-s8-arm.md`): the authority serves the S8 verb block beside custody + publish, and a co-writer daemon's trait verbs SHIP at the `RoutedMetaBackend` trait boundary (no call site can bypass it). **S8-a, the spec-verbatim R1 honesty row**: serial linux-src `tar -x` on a real co-writer at RTT {veth-floor, +250 µs, +1 ms} = 0.60× / **0.14×** / 0.046× of authority-local, 100 % rtt-attributed (owner execute ≤ 8–32 µs), coalesce 1.00 on the serial stream — the regression ruling D10 accepted, published as demanded, and the baseline S10 was measured against. **S8-b**: the K=5 30-min crucible — ~5.3 M shipped mutations with **zero un-routed local commits**, dedup exactly-once under injected session kills, the era split across an authority kill-9 with **zero silent old-era survivors** (S7's stricter client clock wins the race), fsck + C8 clean after a co-writer kill-9. Historical posture (superseded): the pre-arm text said `arm_ownership` had no caller on purpose — S9's arm is that caller. S4's contract #2 resolved by a **client token cache fed only by grants piggybacked on shipped replies** (a per-read round trip was excluded by §6.5 item 1's ≥ 99.5 % locally-served requirement against ~24 reads per write); a miss serves the owner era's base — same as a fresh local mount returns for an ungranted object, so stale still classifies stale — and trips a must-stay-0 counter, because a miss *means the intent-lock property was violated*. Cross-**owner** shapes refuse `EXDEV` naming S3.5; one-owner cross-volume ops are unchanged. Idempotency is request-id + owner-side dedup window (a duplicate awaits the winner's own outcome), **exactly-once within an era, at-least-once across a failover** — durable exactly-once needs S3.5's intent records and was not faked. Ownership granularity is the **volume**; an intra-volume split is unrepresentable until bit 8 is stamped and the node cache gains its third gate state |
| S9 | Multi-writer data plane | **LANDED — ARMED & PROVEN 2026-08-16 (tier: measured-simulated for fan-out throughput, measured-real for the fencing/correctness gates); the two historical blockers CLOSED** | `src/data_grant.rs` + `src/meta_ship/publish.rs` + `src/multi_writer.rs`: the custody state machine (grant/renew/revoke/expire), remote adoption ordered **adopt era → raise stripe floor → admit with the OWNER's token** so nothing on a client mints custody, S11's range algebra reused, S7's single `authorize_dma` door untouched, and S8's `arm_ownership` finally given its caller. Revocation is **pull-based by design**: a revoked co-writer's *belief* can outlive the revoke by one renewal cadence (bounded by its own `T_self`), its **writes cannot** (bounded by the device) — both directions pinned. The historical blockers: (1) the D0 Layer-B2 gate — closed by the co-writer posture (`open_co_writer` behind the five-rung admission); (2) the allocation partition — closed by the lane grant + the shipped raise/free/harvest ladder. **The rung-10 proof** (`.benchmarks/2026-08-16-mw-s9-arm.md` + the finding-#6 fix `.benchmarks/2026-08-16-mw-publish-era-gate.md`): S9-a — 3 concurrent writers, engagement exact (publish ships ≡ serves, displaced frees shipped, `local_commit_refusals` 0), write-amp columns present (1.34–1.41×, attributed), fsck + engaged-C8 clean; S9-b — authority kill-9 with the successor up in 1 s, 2/2 co-writers fenced with zero silent survivors, **zero acked-data loss** (sha256 corpus byte-identical through the successor AND every re-admitted co-writer); S9-c — the co-located (shared-PR-identity) fencing story proven client-side-only, as classified. Finding #6 (a swept-but-not-yet-self-fenced zombie's layout publishes still applied → durable divergent delta chain) closed by the **era gate over the whole mutating publish vocabulary + the layout-publish idempotence witness** (publish schema 5, design-mw-layout-versions §6a), s9-colocated-fence GREEN ×3 from zero. Automatic co-writer re-admission after failover = the documented remount deferral (operations.md carries the posture) |
| S10 | Subtree delegation + client-owned-slot placement | **LANDED; gate adjudicated — the honest product statement** | Rungs 11–14: the recall lane + thrash valve, LOOKUP-class delegations, per-directory EXCLUSIVE UPDATE intents (creates + deferred utimes fully local), and client-owned-slot placement (per-client mint targeting + the valve-bounded migration policy over the existing online `migrate-meta-slot` engine). **The §8.3 gate — serial `tar -x` ≤ 1.10× S0 at 250 µs — is NOT MET and is structurally unmeetable on the shipped topology**: measured **6.73×** (14.87 s vs 2.21 s authority-local; real linux `fs/` tree, A-B-B-A, engagement exact — `.benchmarks/2026-08-17-s10-slot-placement.md`; evidence tier measured-real). The reason is architecture, not tuning: exactly one node holds every metadata volume's D0 claim, a co-writer is metadata-read-only (`CoWriterMount` latch), so **no client-owned slot exists to place its work into** — the placement policy's candidate inversion is empty on every mountable fleet, and the ON-arm residual ~11.7 shipped verbs/entry is per-entry reads + per-file custody/publish ceremony, none of it localizable for a volume-less client. Spec R1's own fallback governs, verbatim: *"remote clients are throughput-oriented; latency-sensitive metadata work runs on the owner"* (docs/operations.md §Metadata function shipping carries it). The gate becomes meetable when a client can hold the D0 claim on ≥ 1 volume of a shared set (per-volume claim admission — the §6.10 R4 fleet-of-authorities recipe, future work); the placement machinery for that day is landed, dark, and pinned in-process against the fleet-of-authorities shape (`tests/mw_slot_placement_tests.rs`) |
| S11 | Byte-range custody + the W1 seventh clause | **LANDED; §9.5 rows GREEN — the MPI-IO verdict ISSUED (MET), both rung-18 standing-reds RETIRED** | The full ladder (rungs 15–19): the distributed range wire, the demotion barrier + authority assembler, the §9.5 closing rows on **pinned ior 4.0.0** (`.benchmarks/2026-08-18-s11-mpiio-row.md`), and the **width-N refs/lineage composition fix** (`.benchmarks/2026-08-18-s11-widthn-refs-fix.md`): displaced/inserted computed INSIDE the chained merge and the custody-scoped Put (never the caller's frame), node compaction preserves the version lineage a live link claims (the retained restamped head link — the fork-latch kill), the accounting owner is the GLOBAL ino, and a chained composition meeting an `indirect:` head REFUSES retried-class instead of staging fold-poison or verbatim-clobbering. **The MPI-IO acceptance row PASSED end-to-end** (A-B-B-A shared/disjoint ≥ 0.8× in both brackets — min 1.411×; engagement exact, cap_refusals 0; read-back-exact zero errors; cold fsck findings 0 / drift 0) at the leg's self-sized 5,120 MiB inline-map shape; the **block-cyclic fsck half is GREEN ×3 from zero**, and the **sub-block fourth dangling-take face is FIXED** (s11-subblock GREEN ×3 from zero, fsck clean). Named residual (rung 20): the **indirect-map width-N composition** — a shared file whose composed map exceeds the inline cap (≈ 6 GiB at 4 MiB blocks) spills indirect and concurrent publishes onto it refuse loud (fail-safe fsync EIO, never corruption) until the blob-aware owner-side merge lands. `SQUEEZEFS_RANGE_CUSTODY` stays default-OFF (the flip now gates only on the indirect-domain composition + the flipped-default ×3 re-runs); `patch_ineligible_range_shared` keeps the W1 ledger honest

**§6.8 item 3 — the freed-offset grace period — LANDED (armed with the
membership plane, off by default).** `src/free_grace.rs`. A terminally-freed
offset is not reallocatable until every live reader has acknowledged passing
it, enforced at the allocator's `finish_free`, with a laggard **fenced rather
than waited on**. It deliberately did **not** reuse S7's dead-epoch quarantine
— same seam, different lifecycle: S7 admits on a death event and releases on an
external drain proof (an O(live-entries) scan), item 3 admits on every terminal
free and releases on a monotone bound that advances on its own, which wants an
O(1) FIFO front peek on the write path. They compose in one fixed order,
**custody proof first, reader coherence second**, so an offset released by a
drain proof enters grace rather than the free list. Epoch identity is an
**owner-minted causal label** already on the S6 wire (zero wire change); the
revalidation epoch was rejected because it is per-volume and the only sound
scalar composition starves in an unbalanced set — which is a *measured* field
shape, not a hypothetical. Space pressure is **ENOSPC, never an unacknowledged
release**, and `df` counts held offsets as used because they genuinely are.

Reader status, finalized at the MW program close (2026-08-18): **one writer
plus N readers is shipped whole** — the metadata half (S5), the data-plane
grace period (item 3, armed with the membership plane), and reader
visibility in `squeezefs clients` (S6) all landed, and the measured
capability row the D11 deferral owed ran as the rung-7 S6-a fleet row
(N=32 on the tcp substrate, 600 s sustained —
`.benchmarks/2026-08-16-mw-s6-arm.md`).
...
| `mem_budget_tests::advisory_integration_phases` | "the paused victim must NOT reach the tier" — reported identical at base by two independent agents | **FIXED** (`fb821b72`) — not R5 and not the eviction-channel components (the Yellow drop was intact and counting): the tier copy was the fill site's own ghost-admitted fetch-time publish, which PERF-11 (`558e26ef`) deferred off the serve path — the fixture's remove raced it (phase E's pause-counter assert was the same class). The test now waits for the deferred publish/pause verdict before asserting; assertions unweakened, argued from §5.7's own rows (Yellow pauses dehydration; the tier-publish pause is the Red unreclaimable arm's) |
| `writeback_tests::test_write_block_from_staging_transform_leg_drops_guard_before_dma` | An LZ4 image declares 65,536 B against a 4,096 B block bound — a process-global block-size ordering effect inside that suite | **FIXED** (`ea29b100`) — test-isolation bug confirmed: siblings set `SQUEEZEFS_DEFAULT_BLOCK_SIZE=4096` and the un-installed `CryptoCompressState`'s DUR-8e cap fell back to the live env read. The fixture now derives the bound per fixture (`init_scratch_pool(CHUNK_SIZE)`, the mount's own path); the guard-drop law untouched. Latent same-class residue noted: `crypto_incompressible_tests`/`crypto_block_framing_tests` also decode through un-installed states and survive on env-value luck |
| *(not a failure — two behaviour changes to re-check)* | S3.5 moved cross-volume `unlink`/`rename` decisions: a cross-volume unlink whose child inode is missing now removes the name **and** accounts nothing with a warning (it used to remove the name then return `NotFound`), and a cross-volume rename's `ENOTEMPTY`/nlink-floor decisions are taken one commit earlier (same values, same errors) | The three suites that pin these — `rename_semantics_tests`, `posix_errno_tests`, `posix_p2_semantics_tests` — were run at merge and are green (6/13/7). Recorded because the full suite is still parked and the change is POSIX-visible |
...
- **DISC-1 landed** (ruling D2): peers auto-discovered from `client:{uuid}` records — the shared volume is the rendezvous, no multicast and no seed list. The interim form's cost is stated: `listxattr(1)` + one `getxattr` per record per volume, against an ino-1 hotspot §6.5 item 3 measures as saturating at ~4,550 clients; **S6 landed that move** (2026-08-05): `membership_wire::peers_from_census` projects peers off the owner's RAM census under the SAME selection law, so discovery adds zero load to the ino-1 hotspot on any mount that arms `SQUEEZEFS_MEMBERSHIP_BIND`. The record-based projection stays as the un-armed fallback (the shipped default), which is why both forms exist.

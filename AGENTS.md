...
* **Env/CLI knobs:** `--throttle <1-100>` on fsck/defrag/remove-data + `job throttle` live retune; `squeezefs job worker <sqmeta-uri>` (remote enrollment); `format --meta-slots <n>` (growth-planned routing width); `SQUEEZEFS_FSCK_SETTLE_MS` (suspect settle window).

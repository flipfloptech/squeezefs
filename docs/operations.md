...
- `SQUEEZEFS_FUSE_ZC_WRITE_FUSION` (**default off** — field-falsified 2026-08-08: at ~300 µs fabric RTT the fused lane's bounded future polling collapsed armed rand-4k writes to 0.45× of the fusion-off posture — 386k unarmed / 300k fusion-off / 175k fused on the field venue — inverting the local +12 % bracket; the default returns to on only when the RTT-tolerant re-derivation proves itself on the fabric-emulated venue): handler/worker fusion for small armed WRITEs — hold-candidate deliveries at or under the fusion ceiling run their handler future on the queue worker's own fused lane, deleting the bridge round trip's cross-thread wakes. `1` opts in (low-RTT/local-class fabrics). Engagement: `fuse3_zc_write_fusions`/`_bytes`; `fuse3_zc_write_fusion_demotions` ≈ 0 in steady state.

...
| 20 | `feat/kernel-bg-per-queue` — **kernel patch AUTHORED + compile-proven 2026-09-06 (7.2-track 0026; `docs/design-kernel-bg-per-queue.md`), boot + A/B OWED** | K-1 (sqz kernel) | R-4 §7 item 1: the COMMIT-lock split — per-queue FUSE background accounting so neither `fuse_uring_queue_bq_req` nor `fuse_uring_req_end` takes the connection-wide `bg_lock` (the ledger's 1.9 µs/op contention class, paid by the worker AND every submitter) | kernel A (0001–0025) vs B (+0026), same daemon, A A B B across reboots, thermal state recorded: `daemon_cpu_ns_by_class[fuse3-ur]`/op, `read_transport_phase_ns`, the perf ledger's `native_queued_spin_lock_slowpath` share, IOPS, clat p50/p99 — rig `.benchmarks/rigs/2026-09-06-kernel-bg-per-queue-ab.sh`; expected −1.5…−2 µs/op on the worker |

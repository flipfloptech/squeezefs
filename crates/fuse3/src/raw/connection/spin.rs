...
//! cadence). `SQUEEZEFS_FUSE_IO_URING_SPIN_US` is the cap, µs: absent or
//! `0` = never spin — **the shipped default, on measurement** (the field
//! A-B-B-A, `.benchmarks/2026-09-03-r4-reap-thread-economy.md` §6: with a
//! 200 µs cap the governor absorbed 0.50 parks/op — 69 % of the worker's
//! parks — and `msg_hop` / `device_cq` / `wake_hop` did not move by a
//! µs, at +5.5 % CPU/op and a wash-to-negative IOPS bracket: the absorbed
//! class was the ≤ 16 µs waits the kernel already returns from without
//! sleeping, and the long class that carries the wake latency sits past
//! any window a 75–80 %-busy box can afford); an explicit value = the
//! governor with that cap, every gate applying (the registered A/B lever).
//!
...
/// The knob (`SQUEEZEFS_FUSE_IO_URING_SPIN_US`): the cap in ns; `0` =
/// never spin (absent = 0, the shipped default). Read once per process;
/// the daemon's startup registry gate refused a malformed or out-of-range
/// value before any worker spawned, so an unparseable value here keeps
/// the default.
pub fn spin_cap_ns() -> u64 {
...
/// value in µs → cap in ns; absent/blank/unparseable → 0 (off).
pub fn spin_cap_ns_from(raw: Option<&str>) -> u64 {
...
impl WorkerSpin {
    pub(crate) fn new(cap_ns: u64) -> Self {
        Self {
            cap_ns,
            gap_ewma_ns: 0,
        }
...
mod tests {
    ...
    const RAIL: u64 = crate::spin_governor_core::SPIN_RAIL_US * 1_000;

    /// The knob's postures: absent and `0` = off (the shipped default), a
    /// value = the cap in ns; out-of-range/malformed keep the default (the
    /// startup gate is what refuses them).
    #[test]
    fn knob_resolves_absent_off_and_explicit() {
        assert_eq!(spin_cap_ns_from(None), 0);
        assert_eq!(spin_cap_ns_from(Some("")), 0);
        assert_eq!(spin_cap_ns_from(Some("0")), 0);
        assert_eq!(spin_cap_ns_from(Some("50")), 50_000);
        assert_eq!(spin_cap_ns_from(Some("bogus")), 0);
        assert_eq!(spin_cap_ns_from(Some("999999")), 0);
    }
    ...
    /// closes it whatever the EWMA says; cap 0 (the default) closes it
    /// always.
    #[test]
    fn worker_spin_engages_only_on_observed_gaps_with_ops_in_flight() {
        let mut w = WorkerSpin::new(RAIL);

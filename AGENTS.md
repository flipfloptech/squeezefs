...
### Metadata format: v2 fixed geometry + v3 CoW KV (dual-format)

Two on-disk metadata formats mount side by side; `route_ino` may stripe a mixed v2/v3 volume set. Shipped by PRs K0–K10 — normative spec `docs/design-cow-kv-metadata.md`, measured gates `.benchmarks/2026-07-09-kv-v3-gates.md`.

* **v2 (legacy, still supported):** the fixed-geometry store (`src/meta_backend/{storage,inode,dentry,xattr}.rs`) — direct-indexed inode table, static-hash dentry table, per-ino xattr blocks. Hard caps ~20,000 inodes / 131,072 dentries per volume, 3 × 8 KiB xattrs/ino. Keeps the D0/D1/D2 sector-commit crash contract. `format` no longer writes v2 (test-scoped `format_v2_for_tests` only); retirement is decided by fleet telemetry (`meta_format_version == "2"` at zero), not a date.
* **v3 (default for new volumes):** a bcachefs-style **copy-on-write typed-KV btree** (`src/meta_backend/kv/`). One node layer, three logical trees per volume (`TREE_INODES`/`TREE_DENTRIES`/`TREE_XATTRS`), memcmp-ordered keys. **256 KiB log-structured CoW nodes** (format knob `--meta-node-kib`, 64 KiB–1 MiB) with internal sorted-bset appends; a **lock-free reservation journal** (ring `clamp(volume/64, 8 MiB, 32 MiB)`, `--meta-journal-mb` override, **read at every mount**) carrying one checksummed entry per transaction; **checkpoints** flip per-tree roots in an A/B root ledger on the background flush cadence, **never on the commit path**; **monotonic ino allocation** (no reuse, no seeding scan); an **A/B extent-bitmap allocator** (1 bit / 256 KiB extent) with a journaled pending-free protocol. Superblock: magic `METALV01`, version 3, `features_incompat` bit 0 = `KV_V3` (`src/meta_backend/kv/superblock.rs`). Caps lift to ≥ 100 M inodes, 1 M+ entries/dir, unlimited xattrs (value ≤ `min(65536, node_size/4)` B). Reads stay RAM-authoritative and latch-free (scc node index + arc-swap snapshots, demand-paged, clock eviction). Every on-disk unit is checksummed and copy-on-write, so the crash contract is **strictly stronger than v2's D0/D1/D2**: whole-transaction atomicity (one tx = one checksummed journal entry) + torn-write immunity (torn writes detected-and-ignored) — **collapsing the D1/D2 distinction** for KV-resident metadata (§4.10).
* **Migration:** offline `squeezefs migrate <sqmeta-uri> [--grow <bytes>] [--dry-run]` converts v2 → v3 in place (`src/meta_backend/kv/migrate.rs`) — verify-then-flip at a single checksummed superblock sector, idempotent (re-run after interruption), reclaims the dead journal region + xattr reservation as free extents.
* **Env knobs (v3):** `SQUEEZEFS_META_NODE_CACHE_MB` (node-cache budget, default 512), `SQUEEZEFS_META_CHECKPOINT_MAX_DIRTY_NODES` (dirty-node checkpoint cap = mount-replay working-set bound, default 4096); `SQUEEZEFS_META_FLUSH_INTERVAL_MS` is reused as the journal/checkpoint cadence (`0` = strict per-commit).
* **Stats fields (stats inode):** `meta_format_version` (`"2"`/`"3"` per volume); `meta_volume_atomicity` (contract class — `cow-checksummed` on v3) alongside `meta_volume_atomicity_physical` (the hardware probe); and the `meta_kv_*` family — `node_cache_{hits,misses,evictions}`, `node_appends`, `node_{append,rewrite}_bytes`, `node_{compactions,splits}`, `journal_{bytes,entries,full_stalls}`, `checkpoints`, `commit_smo_retries`, `node_dropped_tail_bsets`, `dentry_collision_overflows`, `delta_orphans`, `replay_{entries,dropped_torn,ms}`, `free_extents`, `pending_free`. v2-only counters (`meta_commit_sectors`, `meta_sector_lock_*`) emit only for v2 volumes.
...
* **Metadata crash contract**: explicit D0/D1/D2 levels (process crash / atomic-4KiB power loss / non-atomic power loss) — see `docs/design-wal-crash-consistency.md` §3 and README → Metadata durability. **v3 volumes strengthen this by construction** to whole-transaction atomicity + torn-write immunity, collapsing D1/D2 for KV-resident metadata (see Metadata format section + `docs/design-cow-kv-metadata.md` §4.10).
* Mount probes each meta volume's sector atomicity (sysfs; `meta_volume_atomicity` on the stats inode); `--strict-meta-atomicity` makes below-`atomic4k` classifications fail the mount loud (v2 volumes — on v3 the probe reports informationally as `meta_volume_atomicity_physical` while `meta_volume_atomicity` reads `cow-checksummed`).
...
4. MetaLV metadata-transaction locks, in sub-order (`src/stripe_locks.rs`):
   - **4a.** DLM `I{ino}` / `D{parent:name}` (per-object; `DlmLockManager`).
   - **4b.** *format v2*: dentry bucket lock (`dentry_bucket_locks`). *Format v3* (design-cow-kv-metadata §4.9 4b): **per-node write locks — the commit path takes leaf locks only, in ascending NodeId order, deduped, lock-then-revalidate-then-retry against SMOs; interior-node locks belong exclusively to the serialized per-volume checkpoint/SMO task (parent-then-child), which is what keeps the two lock populations acyclic. Node locks are never held across device I/O (commit apply is RAM-only; the journal entry write happens after unlock; writeback freezes under the lock and appends outside it; SMOs reserve in-window and write after release) and never held while waiting on ring space (ring admission happens before any node lock — §4.4 pt 5; the checkpoint task's own admissions never park, they drain-and-retry).**
   - **4c.** *format v2*: sector locks (`sector_locks`), ascending sector-offset order, commit-only. *Format v3*: the journal reservation — a wait-free atomic, not a lock; ordered inside 4b by protocol, it imposes no ordering edges.

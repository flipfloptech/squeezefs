...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_FUSE_PLACED_MERGE", Kind::Bool, "on", "Approach A — the FUSE placed-merge assembly (write-bandwidth program, 2026-08-09): eligible armed streaming WRITE chunks bridge WRITE_FIXED(slot -> per-(ino,block) memfd assembly) and the first merging handler ADOPTS the assembly's mmap view as the ActiveBlockBuf backing — the extraction destination (slot->bounce) and the NT merge copy are both deleted for placed bytes. Default ON per D17 (the measured posture; the A-B-B-A brackets in .benchmarks/2026-08-09-fuse-placed-merge.md are the basis). `0` = the pre-campaign extraction vehicle (the A/B control). Engagement: fuse3_zc_write_placements/_bytes; placed_adoptions/placed_merge_elides move on kernel rows; fuse3_zc_write_place_fallbacks counts bridge-failure/sealed fallbacks."),

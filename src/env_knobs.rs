...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_FUSE_ZC_READ_FUSION", Kind::Bool, "on", "Handler/worker FUSION for armed FUSE READs (e2e audit R-3, read board #3's zc-leg form): the READ handler runs on the reaping queue worker's fused lane, so the zc direct leg's fetch message, CQE resume and prefilled commit are same-thread — the three cross-thread hops the 4k-random attribution measured at 35-47 us each around a 40 us DMA (zc_bridge_phase_ns msg_hop / device_cq / wake_hop). Composed with R-2's fast dispatch as ONE law at the delivery CQE: (a) served inline if warm, (b) else fused here if zc-eligible + under the ceiling, (c) else handed to the lane homed on the queue's CPU — transport_fast_dispatch_serves + fuse3_zc_read_fusions + transport_fast_dispatch_demotes ≡ READs. `0` = arm (c) for every demoted READ (the R-2-only shape, the A/B control). Engagement: fuse3_zc_read_fusions ≈ every cold kernel READ on an armed session; fuse3_zc_read_fusion_demotions ≈ 0 steady-state."),

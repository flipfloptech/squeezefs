...
whose files a writer holds custody of does not hand over until the writer
releases (a grant never spans a handover). W1 never patches a
foreign-custody file in place (the co-writer shape — its blocks' references
live in the holder's tree). The PK4 `pack_group` batch-pack wire is NOT
...
- **Where the server is.** Under the plane the S9 custody protocol (join / renew / release / `T_self` / the custody epoch) is UNCHANGED; its SERVER for an inode object is the appender leasing the object's slot — resolved through tree 0's lessee + the `SlotHolderCache` (PR 6's `step_home` predicate: a declared region's slot is another appender's for every cross-owner decision), never a manager RPC. One JOINed custody client per holder endpoint (its renewal cadence on the `sqz-lease` lane; a second JOIN at one holder would revoke the first's grants, so the dial is single-flight); an OWN file's custody is the local arbiter (0 RPCs — gate 1's `dlm_rpcs == 0` on a solo armed mount, pinned); a foreign file costs exactly ONE lock round trip (`dlm_rpcs += 1`, `dlm_custody_via_slot_holder += 1`); a holder whose endpoint is not bound (the join ladder's census binding — PR 12) refuses loud naming it.
...
- **Custody across a handover** (§5.1.4): a slot whose files a writer holds custody of from this holder does NOT move — the handover is DEFERRED (the `Busy` class the slot-lease cadence retries at its next tick) until the writer releases, so a grant never spans a handover: no write lands under a stale holder's custody, no acked write is lost, and the file's next custody comes from the NEW holder (resolved through tree 0). Chosen over recall-or-carry because the S9 channel to the writer is PULL-based (a holder-initiated per-grant recall would be a protocol change) and a live grant is live work.
- **A dead holder**: the writer's lease at it runs the S9 law — at its own `T_self` (strictly before the holder's TTL) the writer POISONS its custody (PR 8's `FenceClass::RemoteCustody`, never the appender park), so no DMA lands after the holder may have re-granted; the holder's sweep at its TTL retires the grant. The holder side — a dead holder's slots re-leased — is PR 10's recovery.
- **W1 declines a foreign-custody file** (§5.4.3 law 2 from the custody path): a file whose custody this mount holds from its slot holder never patches in place — its blocks' references live in the holder's tree, which the durable probe cannot read (another process's RAM in production) — the CoW path, counted on `patch_ineligible_shared` like PR 7's SHARED clause; an own file keeps PR 7's durable probe.
...
- **Gauges** (`.stats`, in the `dlm_custody` object): `dlm_custody_via_slot_holder`, `dlm_custody_token_carried` — 0 unarmed and for every own file, pinned. The token-side faces (`dlm_token_grants_served`, `dlm_token_recalls`, the reader gauges of the writer's per-holder planes) are PR 5's.

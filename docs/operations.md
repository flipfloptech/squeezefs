...
- `SQUEEZEFS_FUSE_ZC_WRITE_FUSION` (**default on** — zc-write-fusion campaign, 2026-08-07): handler/worker fusion for small armed WRITEs — hold-candidate deliveries at or under the fusion ceiling run their handler future on the queue worker's own fused lane, so the store/extract bridge round trip pays zero cross-thread wakes (the rand-4k hop tax's fix). `0` = the pre-campaign handler-lane dispatch (the A/B control). Engagement: `fuse3_zc_write_fusions`/`_bytes`; `fuse3_zc_write_fusion_demotions` ≈ 0 in steady state (an arm-window burst is legal; sustained growth = lane saturation or a missing dispatcher).

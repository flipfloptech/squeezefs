...
| **Heap-full sweep** | `checkpoint_cycle`, after the flush pass, before the bitmap write / barrier (so this cycle's barrier covers the merges) | runs while `heap_full` is latched OR a `merge_backlog` stands; `KvTree::merge_underfull` walks this tree's RESIDENT leaves in key order (no device reads for the census; a candidate's sibling is demand-paged), exact `f` per leaf, the pair law per adjacent pair, greedy left-to-right packing (a successor keeps merging rightwards while the pair law admits), interior recursion + root collapse after each merge | O(resident leaves) fold walks per cycle, ZERO in steady state (the posture gates it); merges until the compaction floor refuses (`NoSpace` ⇒ backlog stands, sweep resumes next cycle) | forced retirement |

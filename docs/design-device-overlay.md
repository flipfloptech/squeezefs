# Design: The Device-Backed Visible Overlay (Approach B) — Rev 1
...
| **Date** | 2026-08-08 |
| **Status** | **Rev 1 — design only.** Formalizes Approach B of the write-bandwidth program adjudication (`docs/rc-manifest.md` §3f, third-party read-only investigation 2026-08-08, ACCEPTED). No implementation has started; the sequencing gates in §3f (counter confirmation on the existing binary, then Approach A as bounded falsification) run first. |
...
| **B-initial gate** (first landed increment, PR B5 closes it) | **materially above Approach A's best row AND ≥ 0.80× same-day raw** on the armed 1 MiB streaming shape | §3f; "materially" = outside the venue's A-B-B-A noise band, both bracket orders |
...
bracket rows as its baseline; if A lands, B's initial gate reads
"materially above A" against A's *measured* rows and B proceeds only if
the remaining gap to 0.85× still prices in. Either way A's instrument
work (the counter-confirmation battery, the per-leg reset discipline)
transfers verbatim. The custom **fixed-copy opcode** (a kernel opcode
copying fixed→fixed, which would let A's assembly skip the memfd
round-trip) stays **PARKED** with its condition stated in §8.
...
| `alloc_guard` | `InflightAllocGuard` | law 9's carrier + the fsck C2/C3 in-flight exemption (the `RewriteEpoch::guards` role, verbatim) |
| `completed` | the coverage union (shared pure core, §3) | law 3's ledger; the read protocol's serve authority |
| `inflight` | ranges submitted, CQE not yet seen — each stamped with the `generation` at submission | law 3's boundary; the fsync freeze's await set; law 6's overlap ordering |
...
| 9 | **never-return-unpublished-to-allocator-while-DMA-in-flight** | An unpublished overlay offset re-enters the free list only when the record can prove no `WRITE_FIXED` naming it can still land: `inflight` empty (all CQEs reaped, success or failure) **and** the record terminal. Abandoned handlers, superseded overlays and fence drops all wait out their in-flight set before the guard drops. The MEM-1 class (a re-armed buffer under an in-flight SQE) applied to device offsets: a reused offset with a straggler DMA in flight is silent cross-file corruption. | record teardown | `overlay_teardown_waits` (§9); `data_dma_epoch_refusals` for the custody face |
...
|---|---|---|
| `BlockAllocator::allocate_block` + `InflightAllocGuard` | `src/block_allocator.rs` | law 2's reservation + law 9's liveness proof; the fsck C2/C3 in-flight-registry exemption comes free (the `RewriteEpoch` precedent — a crashed daemon drops the guards and shields nothing) |
...
| `RewriteEpoch` / `rewrite_shadow_record` | `src/routing.rs` (design-rewrite-program §5) | **pending publication — the eventual consolidation (PR B8): a completed overlay FEEDS the rewrite shadow (records its dest as the block's B key), never a second durable-publish path.** The epoch's swap, deferred-free law, fencing law (KD-1.8), refetch-compose belt (KD-1.9) and supersession coherence (KD-1.11) all transfer verbatim; pre-B8, the overlay publishes through the same primitive the shadow swap uses (§6.2), so the consolidation is a call-site move, not a semantics change |
...
For any read touching a block with a live overlay record (registry
probe, lock-free):
...
4b. invalid   → RETRY from 1 (bounded), then the rebind ladder's
                serialized-settle arm (§3 row 5) — never EIO for
                legal writer churn, never a stale serve
```
...
Gap-seeding rationale (flagged in §Open Questions): seeding makes every
published overlay a **whole block** — one map op, no extent records, no
partial-block layout format. The alternative (publish partial coverage
via the W2 extent machinery) trades a bounded seed DMA for format
complexity on the hot path's durability boundary; rejected for v1.
...
|---|---|---|---|
| **B1 — pure state core** | `src/overlay_core.rs` (dependency-free): the record state machine, generation/coverage/inflight words, laws 1–3/6/9 as pure transitions; `record_write`'s union extracted into the shared `coverage_core` (`#[path]`, both consumers); property tests (proptest: arbitrary segment schedules never violate the laws); loom model for the snapshot/revalidate word protocol (§5.2 is lock-free on the reader side — the house extracted-core convention) | property + loom suites green; `active_block.rs` byte-identical behavior through the extracted core (the existing coverage suites re-run unmodified) | a law that cannot be expressed as a pure transition (would mean the state machine is under-specified — redesign before any I/O lands) |
...
| **B4 — gap seeding** | partial-overlay completion: seed uncovered ranges from `old_binding` bytes at fsync (§6.2 step 3); overwrite shapes (old binding present) arm HERE — law 8's deferred free of the displaced key | A-B-B-A overwrite bracket: amp ≤ 1.05, zero mid-row discards (elision composes); the seed pays only on partial overlays (`overlay_gap_seed_bytes` ≈ 0 on full-coverage streams) | seed traffic on real streams is material (kernel-split coverage rarely completes) — the eligibility predicate narrows or the ladder stops |
| **B5 — fsync/publication hardening** | the full §6.2 sequence under storms; freeze/successor-record semantics; unmount drain; fence drops (W5); **closes the B-initial gate** (§1.3) | fsync-storm suites; `overlay_fence_drops` 0 healthy; **the B-initial measurement row: materially above A's best and ≥ 0.80× same-day raw, sustained, reset-per-leg A-B-B-A** | the gate row fails ⇒ the residual is profiled and named before any further rung (the internal-time program's discipline) |
...
| **B8 — rewrite-shadow consolidation** | a completed overlay FEEDS `rewrite_shadow_record` (its dest becomes the epoch's B key); the overlay's own publish path deleted — **one durable-publish authority** (§3 row 3); overlay + epoch gauges reconcile | journal-entry-count equality vs B5 (consolidation adds no commits); the shadow suites green with overlay-fed epochs | — (refactor rung; falsifier is the equality pin) |
...
ACK-after-CQE; the accelerator moves ACK latency, not bytes. It lands
...
| Recovery | `overlay_recovered_offsets` (mount census reclaims) | nonzero after a crash is the design working; growth on clean remounts is a leak |
...
| KD-OV-6 | **Fsync seeds gaps to whole blocks** rather than publishing partial coverage via extent records. | One map op per block on the durability boundary; the seed DMA is bounded and pays only on partial overlays (`overlay_gap_seed_bytes` is the falsifier's instrument). Flagged as Open Question 2. |
| KD-OV-7 | **v1 acks after the CQE**; ACK-early is the retention accelerator's charter, not a rung. | Every ladder rung stays correct and measurable on stock-shaped timing; the accelerator moves latency, not bytes, and lands with its own kernel-series review (ruling D13). Flagged as Open Question 1. |
...
Places where the adjudication (rc-manifest §3f) was ambiguous and this
note had to choose — flagged for user/orchestrator review:

1. **ACK timing in v1 (chosen: ACK-after-CQE).** §3f names the
   retention patch as the ACK-early accelerator but does not state
   v1's ACK point. This note chooses ACK strictly after the store's
   CQE (laws 1+3 compose trivially; the slot is consumed before the
   handler returns, satisfying §5.4 severance by consumption). If
   leadership intended ACK-at-install from day one, PR B2 grows the
   retention dependency and the accelerator stops being optional.
2. **Gap-seeding venue (chosen: seed the overlay offset to a whole
   block).** The alternative — publishing partial coverage through the
   W2 extent-record machinery — avoids the seed DMA but puts extent
   records on the streaming fsync path. §6.2 states the rationale;
   reverse if partial-coverage fsyncs dominate real traffic.
3. **The B-initial gate when A is falsified.** "Materially above
   Approach A" is unambiguous only if A landed. This note reads the
   falsified-A case as: the gate's comparator becomes the extraction
   baseline (A's bracket rows as recorded evidence), with the 0.80×
   raw fraction unchanged. Confirm.
4. **Read-tier admission of overlay-served bytes (chosen: none while
   the overlay is open).** Install purges the block key from the read
   tiers (the displaced-key purge law); overlay serves are
   ledger-invisible until publication, after which ordinary admission
   applies. The alternative (admitting overlay serves under the
   unstable incarnation) fights the seqlock by design. Flagged because
   it makes hot re-reads of an open overlay pay device reads.
5. **Registry scope for the §5 probe.** Chosen: probe per read on inos
   with any live record (one lock-free map probe on the read hot path,
   ≈ the placed-sever registry cost). If the probe shows up in
   `read_serve_phase_ns`, an ino-level bloom/flag gate is the follow-on.
6. **Does the overlay open on staged-layout files?** Chosen: striped
   (and striping-eligible fresh) layouts only — staged/inline layouts
   keep their machinery; a staged→striped promotion drains staging
   first (the existing promotion path), then overlays apply. §3f is
   silent; this matches the rewrite program's KD-1.10 scope fence.
7. **Per-record vs per-range generation (chosen: per-record, stamps on
   inflight entries).** Cheaper words, coarser retries: a reader of
   range X retries when a disjoint range Y advances the generation.
   Acceptable at block granularity (4 MiB); revisit only if B3's
   falsifier fires on retry tax.
...
- Code anchors: `src/nvme_dev.rs` (`zc_write_fd`, `authorize_zc_store`),
  ...
  `publish_block`, `begin_free`/`finish_free`), `src/placed_sever.rs`,
  `src/free_grace.rs`, `crates/fuse3/src/raw/connection/zc.rs`,

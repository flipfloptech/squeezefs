...
| | **Random-Access Writes** | <span style="color:red">**CRITICAL GAP**</span> | **Currently ignores FUSE write offsets.** Needs offset-aware read-modify-write (RMW) logic. |
...
| | Tier 2 (System RAM) | **Complete** | Thread-safe LRU cache sized dynamically to 20% of host RAM. |
| | Tier 3 (NVMe Staging) | **Complete** | Writes buffered to local NVMe under `/tmp/squeezefs_staging`. |
...
* **Squeezefs Current State:** FUSE `write` passes offsets, but `routing.rs` processes the payload as a complete overwrite.
* **Solution:** Implement Read-Modify-Write (RMW) for partial writes inside the `DataRouter`.
...
- [ ] Modify `DataRouter::write_file` to accept `offset` parameter.
- [ ] Implement Read-Modify-Write (RMW) for staged files:
  - If a write is at an offset, retrieve the staged block, patch the bytes at the offset, and rewrite the block.
- [ ] Implement RMW for striped files:
  - Identify which 4MB blocks are affected.
  - Download the affected blocks, modify the bytes, and re-upload.
- [ ] Add integration tests in `tests/metadata_tests.rs` for partial writes and random access offsets.

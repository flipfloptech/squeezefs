...
|---|---|---|---|
| Single writer, single mount | Full POSIX; whole-tx atomic metadata; torn-write immune. Since the rung-10b flip the default format class is multi-writer-capable and **stamped-solo is measured within noise** with `dlm_rpcs == 0` | (i) | Shipped — `.benchmarks/2026-08-15-mw-s4-regate.md` + `.benchmarks/2026-08-16-mw-s4-residuals.md` |
| Concurrent PLAIN write mounts of one volume set | **Refused** (D0 flock + PR where available) — unweakened by the co-writer door (a co-writer needs the five-rung admission decision; the guard's own refusals are byte-identical) | (i) | Shipped — still the honest statement for un-armed fleets |
| One writer + N coherent readers | Readers take no leases; **bounded monotone staleness** = poll interval + the ≤ 1 s checkpoint ceiling, published live (`reader_staleness_bound_ms`); DATA half eliminated with the membership plane armed (freed-offset grace), bounded with it off (the default) | (i) | **Shipped** (DLM S5 + S6 + §6.8 item 3); 32-member census/journal rows measured — `.benchmarks/2026-08-16-mw-s6-arm.md` |
| Multi-writer, disjoint file sets (D1's shape) | Authority + N co-writers: shipped metadata, custody-authorized direct DMA, lane-partitioned allocation, shipped frees + harvest reuse; kill-9/failover/fencing crucibles with fsck + C8 green after every kill; zero acked-data loss across authority failover | (i) for the fencing/correctness rows (real mounts, real PR target); (ii) for fleet fan-out throughput (one box); (iii) for every 15 k number — the published arithmetic bounds aggregate co-writer ingest at ≈ 2.6 GiB/s through ONE authority | **Armed & proven, opt-in** — `.benchmarks/2026-08-16-mw-s{7,8,9}-arm.md`, `.benchmarks/2026-08-16-mw-publish-era-gate.md`; closing record `.benchmarks/2026-08-18-mw-program-closing.md` |
| Multi-writer, ONE shared file (the MPI-IO shape) | EX byte-range custody: block-aligned spans DMA directly, sub-block-shared blocks demote to the authority assembler, required never trimmed, budgets derived + refuse-loud | (ii) — pinned ior 4.0.0, 32 ranks, one box; **verdict MET** (A-B-B-A ≥ 0.8× both brackets: 1.411× / 2.273×; read-back exact; fsck + C8 clean) | **Shipped dark** (`SQUEEZEFS_RANGE_CUSTODY` default-off; the flip gates on the indirect-map width-N composition) — `.benchmarks/2026-08-18-s11-mpiio-row.md` + `.benchmarks/2026-08-18-s11-widthn-refs-fix.md` |
| Serial metadata latency on a co-writer | The honest product statement (spec R1's fallback, adjudicated): remote clients are throughput-oriented; latency-sensitive serial metadata work runs on the owner — `tar -x` at 250 µs RTT measured **6.73× the authority-local wall** with every S10 lever ON | (i) | Adjudicated — `.benchmarks/2026-08-17-s10-slot-placement.md`; the unlock (per-volume claim admission) is the residual board's |

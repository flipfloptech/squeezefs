...
| **Date** | 2026-07-17 |
| **Status** | **Approved** (rev 4 — review rounds 1–3 complete, 23/23 findings resolved, verdict "Approve" 2026-07-17; all six Open Questions resolved by **final user decision** 2026-07-17, folded into the plan in rev 4; flips to Implemented at program closure, PR 7) |
| **Repo** | `/home/justin/Source/squeezefs`, `dev` @ `49ad606` |
| **Intended home** | `docs/design-nvmeof-target-management.md` |
| **Evidence base (normative)** | `.agents/spdk-scoping/scoping-report.md` (full `nvmeof.rs` inventory, SPDK-vs-nvmet A/B, PR/PTPL matrix, ops notes, ranked OQs); `.benchmarks/2026-07-17-guard-pr-register-ladder.md` (S1 landed: register ladder, `wire_host_id()`, both-stack 10/10 kill-9 matrix) |
...
| **Related** | `tests/dev_substrate.sh` (ownership-namespacing conventions this program adopts), README §Single-writer mount guard guarantee table, QUICKSTART §4, `.agents/spdk-scoping/{rig-up,guard-smoke,pr-matrix,bench,teardown,snapshot}.sh` (fidelity-tier seed) |
...
The full inventory is `.agents/spdk-scoping/scoping-report.md` §1; the load-bearing facts:
...
- `.agents/spdk-scoping/scoping-report.md` — inventory, A/B, PR/PTPL matrix, ops notes, open questions, S1–S4 sketch (adapted here to N1–N7 under the keep-both decision).
...
- **Files**: `src/meta_backend/reservation.rs`, `src/meta_backend/kv/backend.rs`, `tests/mount_writer_guard_tests.rs`, `.agents/spdk-scoping/{rig-up,guard-smoke}.sh`, README guarantee table.
...
- **Gate**: root-tier install→start→status→stop cycle; loud-fail matrix G3 (no binary / no hugepages / dead RPC / version drift) message assertions; RPC framing/timeout unit tests; cargo gate. **Root legs execute via the `.agents/spdk-scoping/` rig scripts** until PR 5 supersedes them.
...
- **Gate**: unit refusal-class matrix (tests-first, injected live-state snapshots + fake RPC server); root-tier — adopt a hand-built pre-rebuild-style configfs share → `list` managed with provenance → clean `unshare` (incl. out-of-range port-id removal), **via the `.agents/spdk-scoping/` rig scripts** until PR 5 supersedes them; cargo gate.

**PR 5 (N5) — `test(nvmeof): dual-stack fidelity tier`**
- **Files**: new `tests/nvmeof_target_substrate.sh`, `tests/run_nvmeof_fidelity.sh` (quick/full), `tests/guard_smoke.sh` (rig script productized, parameterized), automated PR-matrix probe script; AGENTS.md test-tier table rows; `.agents/spdk-scoping/` scripts annotated as superseded-by-tests where applicable.
...
- **Files**: `QUICKSTART.md` §4 final polish (mechanical spellings already landed with PRs 2/4/4b); `README.md` (nvmeof utilities section incl. `adopt`, deployment-class table §6.3 incl. the seq-write no-preference row, host-identity normative paragraph, `fabric_*`/`target status` observability docs); new `.benchmarks/2026-XX-XX-nvmeof-dual-stack-ab.md`; `docs/design-nvmeof-target-management.md` (this doc) status → Implemented with landed-SHA table.

...
/// The composed field shape, end to end at the record level: a writer
/// remounting over records only it and its own offline verbs wrote — an
/// enrollment at era T, then its own gate (era T+1) — must find
/// `writer_term() > prior_term` when `arm_owner` recomputes the
/// predecessor era from the volume's records, and the arm must succeed
/// (the `mw_fleet --owners` self-refusal of 2026-08-23, whose attestation
/// half retired with the per-volume-owner recipe at PR 14).
#[tokio::test]
async fn a_writer_remount_over_its_own_records_arms() {
    let _serial = serial();
    let meta = NamedTempFile::new().expect("temp volume");
    meta.as_file().set_len(VOL_LEN).expect("size the volume");
    format_v3(meta.path(), VOL_LEN, &opts())
    ...
    // An offline verb's enrollment at its own era, clean exit.
    ...
    // The remount: the gate at this mount's own era.
    let re = KvMetaBackend::open(meta.path()).await.expect("remount");
    ...
    assert!(
        term > prior_term,
        "a writer's own records must never read as a newer predecessor \
    ...
    MembershipOwner::arm("owner-remount", term, prior_term, shipped_clocks(), clock)
        .expect("the remounted writer arms over its own records");
    re.shutdown().await.expect("clean shutdown");
}
...
// ---------------------------------------------------------------------------
// 12. The claim set's durable image — the retired per-volume-owner words
// ---------------------------------------------------------------------------
//
// The per-volume claim admission recipe (`docs/design-per-volume-claim-
// admission.md`, BANNERED) wrote `owner` / `successors` / `holder` into
// each volume's `claim_set` record. PR 14 retired the recipe with the
// `volume set-owners` verb (ownership is the slot lease under the
// symmetric forest — D19 reversed), and the record's image is the
// pre-program one again: the encoder never writes those keys, a pre-1.3.0
// record carrying them decodes with its assignment IGNORED (the tolerant
// `get(...).and_then(...)` law), and the next RMW writes the pre-program
// image — `enable-symmetric` reports such an assignment DROPPED (§7.3)
// off the raw record. The bit-14 gate and the last-member deletion are
// the laws that survive verbatim.

/// The **pre-program** `claim_set` image, frozen as a literal: exactly
/// what `dev`'s `ClaimSet::encode` produced before the retired recipe's
/// fields existed — and exactly what it produces again since PR 14. A
/// hand-written literal is the point: comparing against `encode()` would
/// pin the encoder to itself and could not catch a key that must not
/// appear.
...
/// The same set as a pre-1.3.0 binary wrote it with a `volume set-owners`
/// assignment (+ the KD-PV-17 holder attestation): what a converted
/// volume's record may still read like until its first RMW.
const PRE_1_3_0_ASSIGNED_CLAIM_SET: &str = concat!(
    r#"{"holder":{"boot":"boot-b","id":"node_0000000000000002.m00000001","pid":4242,"#,
    r#""writer_id":"w-uuid"},"#,
    r#""members":[{"boot":"","endpoint":null,"id":"node_0000000000000001.m00000001","#,
    r#""pid":0,"pr_key":0,"role":"writer","ts":1700000000},"#,
    r#"{"boot":"boot-b","endpoint":"10.0.0.2:7100","id":"node_0000000000000002.m00000001","#,
    r#""pid":4242,"pr_key":4660,"role":"writer","ts":1700000001}],"#,
    r#""owner":"node_0000000000000002.m00000001","#,
    r#""successors":["node_0000000000000001.m00000001"],"term":7,"v":1}"#,
);

/// The set whose encoding is [`PRE_PROGRAM_CLAIM_SET`]: the pid-less
/// roster form plus one live writer.
...
/// **The headline byte-identity law**: the record encodes to the
/// pre-program image, byte for byte — the retired recipe's keys are not
/// the encoder's to write.
#[test]
fn a_claim_set_encodes_byte_identically_to_the_pre_program_record() {
    let set = pre_program_set();
    assert_eq!(
        String::from_utf8(set.encode()).expect("utf-8 record"),
        PRE_PROGRAM_CLAIM_SET,
        "a claim_set must encode BYTE-IDENTICALLY to the pre-program image: the retired \
         per-volume-owner keys (`owner` / `successors` / `holder`) are never written"
    );
    let back = ClaimSet::decode(PRE_PROGRAM_CLAIM_SET.as_bytes()).expect("pre-program record");
    assert!(back.durable);
    assert_eq!(back.members, set.members);
    assert_eq!(back.term, 7);
}

/// Forward tolerance in BOTH directions: a pre-1.3.0 record carrying a
/// `volume set-owners` assignment decodes with the assignment IGNORED —
/// never a guess, never a refusal — its members and term intact; the raw
/// record still answers the assignment to `enable-symmetric`'s DROPPED
/// report; and an RMW of it (the enrollment upsert every mount performs)
/// writes the pre-program image, so the residue leaves the volume at its
/// first membership change.
#[tokio::test]
async fn a_pre_1_3_0_assignment_decodes_ignored_and_leaves_at_the_first_rmw() {
    let _serial = serial();
    let assigned =
        ClaimSet::decode(PRE_1_3_0_ASSIGNED_CLAIM_SET.as_bytes()).expect("the record decodes");
    assert_eq!(
        assigned,
        ClaimSet {
            durable: true,
        ...
        "the assignment is ignored: members and term are what the record says"
    );
    assert_eq!(
        String::from_utf8(assigned.encode()).expect("utf-8 record"),
        PRE_PROGRAM_CLAIM_SET,
        "re-encoded, the record is the pre-program image"
    );
    assert_eq!(
        ClaimSet::retired_owner_assignment(PRE_1_3_0_ASSIGNED_CLAIM_SET.as_bytes()).as_deref(),
        Some("node_0000000000000002.m00000001"),
        "the raw record still names the assignment for the conversion's report"
    );
    assert_eq!(
        ClaimSet::retired_owner_assignment(PRE_PROGRAM_CLAIM_SET.as_bytes()),
        None
    );

    // The first RMW on a volume: the enrollment upsert.
    let meta = engaged_volume().await;
    let be = KvMetaBackend::open(meta.path()).await.expect("open volume");
    be.setxattr_internal(1, CLAIM_SET_XATTR, PRE_1_3_0_ASSIGNED_CLAIM_SET.as_bytes())
        .await
        .expect("plant the pre-1.3.0 record");
    ...
    .await
    .expect("the upsert RMWs the record");
    let raw = be
        .getxattr(1, CLAIM_SET_XATTR)
        .await
        .expect("read")
        .expect("the record exists");
    let text = String::from_utf8(raw.clone()).expect("utf-8 record");
    assert!(
        !text.contains("\"owner\"")
            && !text.contains("\"successors\"")
            && !text.contains("\"holder\""),
        "the RMW wrote the pre-program image — the residue is gone: {text}"
    );
    assert_eq!(ClaimSet::retired_owner_assignment(&raw), None);
    let set = ClaimSet::decode(&raw).expect("decodes");
    assert_eq!(
        set.members.len(),
        3,
        "the two prior members + the enrolled one"
    );
    be.shutdown().await.expect("clean shutdown");
}

/// Projection purity: the singleton projection of `writer_claim` — what
/// every un-engaged volume answers with — carries the claim as its one
/// member and nothing else.
#[tokio::test]
async fn the_singleton_projection_is_the_claim_as_one_member() {
    let _serial = serial();
    let meta = formatted_volume().await;
    let be = KvMetaBackend::open(meta.path()).await.expect("open volume");
    be.setxattr_internal(
...
/// The bit-14 law: a durable record on an un-engaged volume is refused
/// loud and writes NOTHING, so no unstamped volume can grow one.
#[tokio::test]
async fn store_refuses_a_claim_set_without_bit_14() {
    let _serial = serial();
    let meta = formatted_volume().await;
    let be = KvMetaBackend::open(meta.path()).await.expect("open volume");
    let set = pre_program_set();
    let err = ClaimSet::store(&be, &set)
        .await
        .expect_err("storing a claim set without bit 14 must refuse");
...
/// A departing LAST member deletes the record, so a departed set presents
/// as unclaimed (the pre-program law; the retired recipe's "an assigned
/// memberless record is stored" exception left with the assignment).
#[tokio::test]
async fn a_last_member_withdrawal_deletes_the_record() {
    let _serial = serial();
    let meta = engaged_volume().await;
    let be = KvMetaBackend::open(meta.path()).await.expect("open volume");
    ...
    assert_eq!(
        be.getxattr(1, CLAIM_SET_XATTR).await.expect("read"),
        None,
        "the memberless record is deleted — a departed set presents as unclaimed"
    );
    assert!(
        !membership::withdraw_writer_member(&be, "node_0000000000000003.m00000001")
            .await
            .expect("idempotent"),
        "a second withdrawal has nothing to do"
    );
    be.shutdown().await.expect("clean shutdown");
}

/// The control records are invisible through the mount: `owner_assign:`
/// (the retired verb's pre-1.3.0 bracket, which the writable mount gate
/// still refuses on) is outside the VAL-2 positive allowlist like
/// `mw_upgrade:` / `job:` / `claim_set`, so none can be read, written,
/// listed or removed from an unprivileged shell.
#[test]
fn the_control_records_are_invisible_through_the_fuse_boundary() {
    for name in [
    ...
    assert!(
        ...
        "the record name is the pre-1.3.0 verb's — the mount gate and `enable-symmetric` \
         both name it"
    );
}

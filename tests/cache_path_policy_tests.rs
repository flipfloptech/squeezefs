...
impl Mount {
    fn unmount(&mut self) {
        ...
        // the noise (it is evidence only if the LAST attempt still
        // failed; the former inherited-stderr shape printed
        // "failed to unmount … busy" on PASSING tests).
        ...
        while Instant::now() < deadline {
            if let Some(status) = self.child.try_wait().expect("try_wait") {
                ...
                // must not absorb (the former `.is_some()` accepted any
                // corpse).
                assert!(
                    status.success(),

...
|---|---|---|---|---|
| 1 | `perf/owner-concurrent-verbs` — **concurrent-dispatch half landed** (D-1, `.benchmarks/2026-09-02-d1-owner-concurrent-dispatch.md`; publish plane D-1b) | D-1 | F-A: dispatch a frame's mutating verbs concurrently so they co-queue into ONE M7 pass. **Residual rung (open, measured 2026-09-03/04):** since C-2's instant-drain apply pass the co-queue is arrival-spread ÷ pass-latency — 4–8 passes per 64-verb frame (D-1 note), 3–14 per 24 publishes (C-2 note item 8) — so the un-held pass count is a venue ratio, not a mechanism property. The next rung is **one conveyor group per shipped frame**: stage every call's tx (prelude + DLM guard) concurrently, then enqueue the batch to the conveyor as ONE group so a frame is ONE pass by construction — which also makes the pass-per-frame contract assertable again (both D-1-family tests hold the pass pre-drain today) | K-writer fan-out: verbs/s per authority and ingest GiB/s vs the S9-a wall; `owner_phase_ns` sums; `shipped ≡ served`; fsck + C8 clean. Rung: `META_CONVEYOR_LEADER_PASSES` per served frame → 1 on the fleet rig, journal entries unchanged (one tx = one entry), A-B-B-A on tcp devsub then squeeze-test |

...
| 18 | sugg | Screen **rule 4**: `FrameScreen::appender_current` (tree 0's lessee; 0 = the manager's unleased tree), `g == g_current ∧ appender_id ≠ appender_current` → the breach class under a device fence (`is_breach_rule`); the overwrite probe looks for the lessee's OWN stamp. Pin + fuzz/proptest inputs. |

...
it, the fpp phases' `block_claim_anomalies` —
`.benchmarks/2026-09-07-cowriter-claim-anomaly-lineage.md`), `free_replays`
...
| `SQUEEZEFS_REWRITE_SUPPLY_CLOSE_PER_VOLUME` | bool | `on` | The supply-coupled close's PER-VOLUME plan (finding 15's file-per-proc residue, `.benchmarks/2026-09-07-cowriter-fpp-supply-residue.md`): a lane exhausts and refills per volume and a closed epoch's parked keys return to the volume each lives on, so the volume whose refill tick is short closes the epochs whose parked keys live ON IT — largest first until that yield covers its deficit; an epoch parking only on other volumes is no candidate, and a tick with parked keys only elsewhere declines instead of publishing an epoch that would restock a sibling. `0` = the mount-wide plan (largest epoch by total parked keys — on the fleet's ~64/36 per-volume supply skew about that share of every close's yield restocked the volume that was not short). Engagement `rewrite_shadow_supply_close_volume_blocks`; ledger `rewrite_shadow_supply_close_declined_offvolume`. |
...
| `cowriter` (object: `mw_role`, `custody_endpoint`, `admissions`, `admission_refusals`, `accounting_refusals`, `local_commit_refusals`, `unpublished_{abandons,recycles}`, `free_ship_own_lane_untracked`, `recomputed_retires`), `peer_volume_local_commit_refusals`, `peer_volume_unclaimed_{admits,refusals}` | The co-writer posture and the per-volume-owner gate ([Multi-writer co-writer mounts](#multi-writer-co-writer-mounts-dlm-stage-s9), [Per-volume metadata owners](#per-volume-metadata-owners-squeezefs-volume-set-owners)) |
...
| `rewrite_shadow_supply_close_volume_blocks` / `rewrite_shadow_supply_close_declined_offvolume` (co-writer side) | the per-volume plan's faces (`SQUEEZEFS_REWRITE_SUPPLY_CLOSE_PER_VOLUME`, finding 15's fpp residue): the parked A keys a supply close released that live ON THE VOLUME whose tick asked (⊆ `rewrite_shadow_supply_close_blocks` — the difference restocked a volume that was not short; on the mount-wide plan of the first landing that difference ran ≈ the fleet's 64/36 skew), and starving ticks that found parked keys on the mount but none on the asking volume (nothing closes — closing would restock a sibling; the per-volume face of `declined_no_parked`). Both 0 with the lever off |

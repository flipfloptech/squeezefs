...
**(a) The candidate law — the split's constant read backwards, no new one.** Let `C = NodeLayout::fold_capacity()` (the encoded record bytes one image holds — the compaction-vs-split line) and `f(N) = NodeSnapshot::fold_bytes_upper_with(&[], ·).0` (the K1 fold's byte upper bound over `N`'s serialized log + open delta + frozen delta — the §4.7 heap admission's own estimator; tombstones count their encoding because elision needs the durable tail). Two nodes `L`, `R` at one level, ADJACENT under ONE parent (`R.min = key_successor(L.max)` — K2's gap-free partition), are **mergeable iff**

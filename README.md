...
> **Concurrency scope, stated plainly.** What ships today is **one write mount per volume set** — enforced (not merely advised) by the single-writer mount guard: a second concurrent *write* mount is refused loudly — **plus N read-only mounts** (`-o ro` / `--read-only`), which take no lease and no claim and are therefore orthogonal to that guard. A reader's exact guarantee is documented rather than implied: it serves the state of the most recent checkpoint it has polled — **bounded, monotone staleness** whose bound is published live on the `.stats` inode (`reader_staleness_bound_ms`, 2 s with the shipped defaults) — and the data-block half of that bound is *bounded rather than eliminated*, because the freed-offset grace period is a later stage ([docs/operations.md → Read-only coherent mounts](docs/operations.md#read-only-coherent-mounts--o-ro--one-writer-plus-n-readers)). The 15,000-node scale target is the design's destination, and the program to get there — read-only coherent mounts first, then multi-writer — is specified in `docs/pre-rc-engineering-spec.md` §6 with its staging plan and gates. Every capability claim in this README describes shipped behavior; roadmap items say so.

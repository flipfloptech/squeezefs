...
**Code site:** `src/uring_fs.rs:1368/1586/1609` (the three uring-path opens — `.custom_flags(libc::O_CLOEXEC)` only) and `src/uring_fs.rs:2222/2247/2258/2274` (the `blocking_fallback_loop`'s opens — no custom flags: buffered too, so the finding holds on the fallback arm and PR 13i's fix must cover it or refuse it on a shared LUN); `src/nvme_dev.rs:744` (the data path's `open_opts.custom_flags(libc::O_DIRECT)` for the worker's device fd — the posture the metadata path lacks; the file's other three `O_DIRECT` opens are 1576 the DUR-2 probe, 1686 the read-only direct-leg fd, 1717 the D14 write-side direct-leg fd); the appender page's two images at `src/meta_backend/kv/backend.rs:10510–10514` (`page.grant.clear()` … `write_page`) and `write_wire_joiner_page_grant` ≈ `backend.rs:11096`.
...
**Found by:** the cloud row (§3.10), read off the same snapshots as F-C1 (the contemporaneous summary; §3.10's ledger): m60's `extent_grant_granted 72 = claimed 72, unclaimed 0`, one `ExtentGrant` ask per cadence tick (`joined_wire_verbs 356`), every ask a verbatim replay at the manager (`manager_verb_replays 2083`) — the 15:55 row's **"all 7 joiners write-dead from their join"**. The `claimed 72` is `RegionGrant::recover`'s classification of an EMPTY page word (every extent the tree-0 record names lands CLAIMED with no mint — the code's own comment, `backend.rs:20437–20441`), not a mint: exactly this finding's shape composed with F-C1's.

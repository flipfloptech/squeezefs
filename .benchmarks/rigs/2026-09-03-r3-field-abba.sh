#!/usr/bin/env bash
# .benchmarks/rigs/2026-09-03-r3-field-abba.sh — R-3 field A-B-B-A on
# squeeze-test (the 4k-random attribution note's venue and jobs).
#
# A = the box's attribution binary (/scratch/tmp/squeezefs.kvmap +
#     libsqueezefs_il.so.kvmap, dev 7fe9fde2 -dirty ⇒ SQUEEZEFS_IPC_ALLOW_DEV=1
#     on both ends for il rows);
# B = the R-3 rocky8 build (fb105d17: the zc-leg instrument, the NvmeBlockDev
#     funnel wake, READ fusion default ON) + its own shim (same commit, KD-7).
# One fresh cluster + one write_BW.job pass (arm A) mints the 24 × 8 GiB
# files; then A B B A remounts, each running: kern randread_iops (30 s +
# 10 s ramp), il randread_iops (LD_PRELOAD), kern read_BW (1 MiB qd16) —
# and on the SUSTAINED legs (B1, A2) a 60 s kern + il rand-4k pair. The
# second B and the last A mount with SQUEEZEFS_OP_TRACE=1 and drain
# `.trace` mid-row in their kern rand-4k leg (the re-attribution of the
# zc leg's per-op split, `zc_bridge_phase_ns`).
#
# Writes ONLY under /scratch/tmp/sqz-agent/r3/. Run as root on the box:
#   bash /scratch/tmp/sqz-agent/r3/2026-09-03-r3-field-abba.sh
set -eu
...
echo "== R-3 field A-B-B-A: $(date -u +%FT%TZ) out=$OUT kernel=$(uname -r)"
...
row() {  # $1 = tag, $2 = job (randread_iops|read_BW|write_BW), $3 = mode (kern|il), $4 = runtime, $5 = il so, $6 = trace (0|1)
  ...
  "${pre[@]}" fio "$JOBS/$job.job" --runtime="$rt" --output-format=json --output="$OUT/$tag.fio.json" \
...
row B1-rr4k-kern-60 randread_iops kern 60 "" 0
row B1-rr4k-il-60 randread_iops il 60 "$B_IL" 0
umount_arm
# B2 (traced)
...
row A2-rr4k-kern-60 randread_iops kern 60 "" 0
row A2-rr4k-il-60 randread_iops il 60 "$A_IL" 0
umount_arm

...
| `free_grace_recall_gated_frees`, `free_grace_timeout_deferrals` | **Recall-driven free-grace** (design-symmetric-metadata §5.7.3 — PR 5, dark; 0 on every mount without an armed token holder). Under GPFS-strict tokens the freeing publish recalled every reader's token before it committed and a reader acks only after its in-flight DMA on the ino drained and its block-key census was purged — so the recall IS the qualification: `recall_gated_frees` = terminal frees that published to the free list DIRECTLY (no ring, no deferral; `free_grace_hold_ms` for them is the recall round trip, `dlm_token_recall_rtt_ns.total`); `timeout_deferrals` = frees that rode the grace RING because a LIVE member's recall was unacked past the bound (`dlm_token_recall_timeouts_live`'s window, open for `T_owner` — the ring is the timeout path, released by its own law: the acknowledgement, the fence at `T_owner`); every ring deferral still counts `free_grace_deferrals`, so the closure `deferrals ≡ releases + offsets` holds on both paths. |

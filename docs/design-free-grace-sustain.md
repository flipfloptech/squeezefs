...
| **T0** (added 2026-09-07 — the stage BEFORE the loop's clock starts) | **Epoch parking (displacement → free).** A rewrite's displaced A key is PARKED in the ino's open rewrite epoch and freed only at an epoch close (design-rewrite-program §5.3 KD-1.6); `finish_free` — where `defer_checkpointed` is stamped — runs only then. On a co-writer rewriting a slice of a shared file no routine trigger fires DURING an iteration (full coverage unreachable, fsync/RELEASE at the boundary), and KD-1.7's ENOSPC close fires only after the `StorageFull` | up to the iteration's length; the s11 fleet's serve timeline showed the recycled supply arriving in bursts with **9–31 s gaps** aligned with the epoch closes, every ENOSPC storm inside a gap (`.benchmarks/2026-09-06-free-grace-term1-fleet.md` §3) | **the dominant term once T1–T8 fell** (hold 9.4 → 1.8 s, row still failing). Lever: the **supply-coupled close** — `SQUEEZEFS_REWRITE_SUPPLY_CLOSE`, `.benchmarks/2026-09-07-rewrite-epoch-supply-close.md`: the co-writer's refill tick closes open epochs when `lane_reachable < watermark` (L5's own threshold), largest first until the yield covers the deficit, bounding T0 by the lane's headroom instead of the iteration |

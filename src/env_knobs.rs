...
pub enum Kind {
    ...
    Int { lo: i128, hi: i128 },
    /// One of a fixed set of words. `retired` lists spellings that USED to
    /// be admissible and now refuse naming their successor (forward-only —
    /// the `Kind::Retired` law applied to one value of a live knob); each
    /// entry is `(value, why)`.
    Enum {
        allowed: &'static [&'static str],
        retired: &'static [(&'static str, &'static str)],
    },
...
/// `SQUEEZEFS_SYM_GRANT_EXTENTS`'s range: the SMO budget floor to the u16
/// slot namespace's width (`appender::GRANT_EXTENTS_{FLOOR,MAX}`).
const SYM_GRANT_EXTENTS_MIN: i128 = crate::meta_backend::kv::appender::GRANT_EXTENTS_FLOOR as i128;
const SYM_GRANT_EXTENTS_MAX: i128 = crate::meta_backend::kv::appender::GRANT_EXTENTS_MAX as i128;
/// `SQUEEZEFS_SYM_MINT_SLOTS`'s ceiling: the shipped mint-spread
/// granularity (`slot_lease_core::MINT_SPREAD`, tie-tested against
/// `meta_backend::MINT_SPREAD`).
const SYM_MINT_SLOTS_MAX: i128 = crate::slot_lease_core::MINT_SPREAD as i128;
/// `SQUEEZEFS_SYM_AFFINITY_MAX_MB`'s ceiling: `BYTES_MAX` in MiB — the
/// heap clamps it at open.
const SYM_AFFINITY_MAX_MB_MAX: i128 = BYTES_MAX >> 20;
...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_INLINE_MAX_BYTES", int(crate::routing::INLINE_MAX_FLOOR as i128, crate::routing::INLINE_MAX_CEILING as i128), "4096", "The inline-layout ceiling, bytes: a file up to it keeps its whole payload IN its layout record on the metadata volume — visible to every client of the set at the commit, no block, no promotion step; past it the file is staged (with a staging dir) or striped. DEFAULT one page (4096, the derived default — `routing::INLINE_MAX_FLOOR`): an inline payload rides the metadata plane TWICE per write (journal entry + CoW node append, ~2x its bytes) against the staged layout's fixed ~0.3 KB of metadata per file, so one page is where the record's payload costs about what a block mapping does; the phase-B sweep (.benchmarks/2026-09-09-inline-raise-sweep-local.md) priced a raised default OUT — 16 KiB inline: −34 % files/s and 59x the metadata bytes per file; 32 KiB: −68 %, 117x, and a 1 GiB metadata volume's heap exhausted and FAIL-STOPPED in 20 s. The RANGE's top is the FORMAT BOUND (`routing::inline_max_bytes_ceiling`: the volume's KV xattr value cap `min(64 KiB, node_size/4)` minus the layout wire's 4 KiB framing headroom — the largest payload one KV value holds; 61440 at the 256 KiB node; a smaller-node volume's bound applies to an explicit value above it, logged once). What the lever is FOR: an operator with generously sized metadata volumes AND a hard small-file cross-client-visibility need (an inline file is readable by every client at its commit; a staged one only after pressure or a clean unmount promotes it), and the measurement lever for the threshold rows. Published live as `inline_max_bytes`. Engagement: layout_inline_writes vs layout_staged_writes; layout_promoted_inline vs layout_promoted_block (the size-dispatching promotion engages inline only above the default)."),
    k("SQUEEZEFS_SMALL_FILE_PACKING", Kind::Bool, "on", "Small-file PACKING (docs/design-small-file-packing.md, PR PK2): a staged-layout file whose stored image's slot fits SQUEEZEFS_PACK_MAX_SLOT_BYTES promotes INTO the volume's shared OPEN PACK BLOCK — one LBA-aligned slot reserved by one fetch_add, one DMA of the image at base+off, one layout commit publishing the size-carrying `bk:off:len` mapping the read path already decodes, its C8 reference in the same tx — instead of taking one whole 4 MiB block per file (the 64x space law of `.benchmarks/2026-09-09-fsync-promote-staged-ab.md`: 140,183 promotions filled a 480 GiB set in 8 s). SCOPE: the PROMOTION arm only (every promoter — pressure, dismount, the fsync lever); the read funnel honours a packed mapping under either setting, so a volume packed under `1` reads byte-exact under `0`. Default ON since PK7's counted flip (2026-09-10, `.benchmarks/2026-09-10-packing-rows-squeeze-test.md`: A-B-B-A on squeeze-test — files/s +0.5 %, fsync −3.7 %, device/user bytes 1.00× on both arms, 96,000 fsync-promoted 16 KiB files = 375 blocks instead of 96,000; dismount 20,000 → 148; the legacy volume 20,000 → 147 with one `defrag --pack`); `0` = the A/B control — the one-block-per-file block arm, byte-identical to the pre-flip shape, `layout_promoted_packed` stays 0. Files ≤ the inline ceiling stay inline; a slot above SQUEEZEFS_PACK_MAX_SLOT_BYTES takes its own block (pack_own_block_promotions); striped files are untouched; a volume packed under `1` reads byte-exact under `0` (rollback = the lever). A CO-WRITER packs BATCH-SCOPED (PK4, §5.6): a promotion batch is partitioned by (owner endpoint, home meta volume), each partition fills private pack blocks in the co-writer's own lane, every tenant DMAs before any publish, and each pack's layout commits travel as ONE `pack_group` publish frame the owner serves as ONE conveyor group on ONE volume or refuses whole (its D-1c lever off → PUBLISH_PACK_GROUP_UNAVAILABLE, an online slot cutover → PUBLISH_PACK_GROUP_SPLIT; the fallback is one-block-per-file, counted pack_cowriter_group_{unavailable,refusals,splits}). Read once per process. Engagement: layout_promoted_packed (+ pack_own_block_promotions for the above-threshold class); the pack life cycle in pack_blocks_{opened,sealed_full,sealed_dismount,sealed_batch,abandoned}; the co-writer group in pack_batch_{frames,tenants,volume_splits}."),
    k("SQUEEZEFS_PACK_MAX_SLOT_BYTES", int(crate::routing::LBA_GRAIN as i128, crate::block_allocator::CHUNK_SIZE as i128), "derived CHUNK_SIZE/2 (2 MiB)", "The packer's OWN-BLOCK threshold, bytes (design-small-file-packing §5.5, KD-5): a stored image whose LBA-rounded slot exceeds it takes its own block (pack_own_block_promotions), a smaller one packs. DERIVED default CHUNK_SIZE/2 — the block economy's break-even: an own block wastes CHUNK − slot forever (a staged file never grows in place), a packed tenant costs one compaction copy of slot, and the saving covers the copy iff slot ≤ CHUNK/2 (the same law is the compaction trigger; tie test `pack_max_slot_default_is_half_the_chunk_and_the_grain_is_the_lba_law`). Range one LBA grain (4096 = one-LBA tenants only, the measurement floor) to the chunk (pack everything, the ceiling); explicit wins verbatim. A measurement lever, never an operational posture. Published live as `pack_max_slot_bytes`."),
    ...
    k("SQUEEZEFS_REWRITE_SUPPLY_CLOSE", Kind::Retired { successor: "(nothing — a rewrite epoch closes on its routine KD-1.6/1.7 triggers; the lane whose supply it coupled to is gone, PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the supply-coupled rewrite-epoch close on a laned CO-WRITER's ahead-refill tick. The S9 allocation lane retired with the co-writer posture — a writer's supply is PR 8's block grant, a freed block returns to the holder's bitmap — so the lever names no mechanism; any value refuses at startup. `.benchmarks/2026-09-07-rewrite-epoch-supply-close.md` is the retired mechanism's record."),
    ...
    k("SQUEEZEFS_ALLOC_LANE_RESERVE_BLOCKS", Kind::Retired { successor: "(nothing — allocation under the symmetric plane is a ranged block GRANT from the data volume's allocation-lease holder, sized by `block_grant` derivation; PR 8 / PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the per-lane durable reservation grain of the S9 data-plane allocation partition (`alloc_lane:` records). The partition retired with the co-writer posture; any value refuses at startup."),
    ...
    k("SQUEEZEFS_READ_TIER_ADMISSION", Kind::Enum { allowed: &["always", "second-touch", "never"], retired: &[] }, "second-touch", "R1b tier-admission policy."),
    ...
    k("SQUEEZEFS_FUSE_PIN_SCOPE", Kind::Enum { allowed: &["node", "core"], retired: &[] }, "node", "Transport thread affinity scope; `core` is the pre-campaign hard-pin posture."),
    ...
    k("SQUEEZEFS_TEST_PACK_COMMIT_STALL_MS", int(0, 600_000), "0", "Test seam: a PACKED staged promotion (design-small-file-packing §5.2) parks this many ms between its slot DMA and its layout commit — the kill-9 crash harness's deterministic window (§5.4: a slot no layout names is dead bytes, never a tenant). Never set in production."),
    ...
    k("SQUEEZEFS_IPC_DD_PACKED", Kind::Bool, "on", "Direct-drive READ prelude, the PACKED-tenant arm (PK8, design-small-file-packing §5.10): a promoted small file's size-carrying `bk:off:len` slot direct-drives as ONE grain-rounded window read inside its own slot on the mapping's backend (the same DMA the handler's staged arm issues — no tier admission either side); 0 = the pre-PK8 posture (the handler path, `ipc_direct_ineligible_meta`; the A/B control). Gauges ipc_direct_packed_{serves,bytes}, refusals ipc_direct_ineligible_packed_shape."),
    ...
    k("SQZ_STRIPE_WAVE_HOLDERS", Kind::Harness, "7", "PR 7b creator-wave row (tests/sym_dir_stripe_tests.rs `a_creator_wave_spreads_over_the_stripe_holders`): declared stripe holders N — 7 fits one 64 KiB directory extent of the declared-partition seam; 31 needs SQZ_STRIPE_WAVE_NODE_KIB=256."),
    k("SQZ_STRIPE_WAVE_CREATES", Kind::Harness, "2000", "PR 7b creator-wave row: creates into the striped directory (the 12,500-creator wave's local stand-in)."),
    k("SQZ_STRIPE_WAVE_NODE_KIB", Kind::Harness, "64", "PR 7b creator-wave row: the harness volume's node size in KiB (256 admits 31 declared holders per directory extent)."),
    k("SQZ_STRIPE_FLIP_LAW_SHAPE", Kind::Harness, "-", "PR 7b flip lock-law pin (tests/sym_dir_stripe_tests.rs `flip_lock_law_child_entry`): the child-process marker and its shape (`explicit` | `mkdir`) — the parent runs it under SQUEEZEFS_DLM_STRIPES=1, a width the 4a tables resolve once per process."),
    k("SQZ_SYM_HANG_FACTOR", Kind::Harness, "4", "The sym-forest matrix runner's per-suite watchdog (tests/run_sym_forest_suites.sh; PR 10 review round 3, Issue 30): a suite's bound is max(SQZ_SYM_HANG_FLOOR_S, this factor × its flat wall) — a suite past it is killed whole and reported HUNG, failing the leg loud."),
    k("SQZ_SYM_HANG_FLOOR_S", Kind::Harness, "600", "The sym-forest matrix runner's per-suite watchdog floor in seconds (the smallest bound any suite gets, whatever its flat wall)."),
    ...
    k("SQUEEZEFS_MULTI_WRITER", Kind::Retired { successor: "(nothing — every RW mount of a symmetric-forest set is a WRITER by the join ladder; design-symmetric-metadata §7.3, PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the S7/S9 multi-writer opt-in armed the device-enforced WERO hold, the served write custody and the shipped publish path on ONE declared authority for N co-writers. Under the symmetric plane the join ladder arms the device-enforced class on every RW mount itself (the D0 winner is the MANAGER, every later RW mount JOINS it over the wire), so the knob names no posture; any value refuses at startup. A pre-flip multi-writer-class set converts with `squeezefs volume enable-symmetric`."),
    k("SQUEEZEFS_MW_ROLE", Kind::Retired { successor: "(nothing — there are no roles under the symmetric plane: `mount_posture` reads `writer` on every RW mount and the lease gauges say what it holds; design-symmetric-metadata §7.3 / §11, PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the authority / co-writer / set-authority / partial-authority postures of the S9 multi-writer and per-volume-claim-admission recipes ceased with them — every RW mount of a symmetric-forest set is a writer through the join ladder (the manager or a joined appender), the per-volume ownership map (`volume set-owners`) is gone, and `enable-symmetric` reports a converted set's assignments as dropped. Any value refuses at startup."),
    k("SQUEEZEFS_MW_AUTHORITY", Kind::Retired { successor: "(nothing — there is no set authority to dial: a foreign object's holder is resolved through tree 0's lessee and the durable endpoint binding; design-symmetric-metadata §5.1.6, PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the co-writer's and the S5/PR-5 reader's declaration of the set authority's S8 listener. Under the plane every writer publishes its own listener into its claim-set entry at the join ladder's rung 7 (`resolve_holder_endpoint`), and a `-o ro` token reader binds per slot holder off tree 0. Any value refuses at startup."),
    k("SQUEEZEFS_MW_MEMBERS", Kind::Retired { successor: "(nothing — no roster: a writer is enrolled by JOINING, its membership lease is its admission; design-symmetric-metadata §7.3, PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the S9 authority's operator-declared co-writer roster. Under the plane the manager admits every joiner that walks the ladder (bits, membership lease, registrant, `JoinAppender`), bounded by the ring budget (`appenders_capacity`), never by a declared list. Any value refuses at startup."),
    ...
    k("SQUEEZEFS_COWRITER_LANE_PLACEMENT", Kind::Retired { successor: "(nothing — placement weights are the device-fill `health_effective` on every writer; the lane-reachable weight left with the lane, PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): lane-aware write placement + allocation failover on a laned co-writer (`.benchmarks/2026-09-07-cowriter-lane-aware-placement.md`). Any value refuses at startup."),
    k("SQUEEZEFS_ALLOC_LANE_HARVEST_AHEAD", Kind::Retired { successor: "(nothing — a writer's grant window tops up at 50 % consumption through the holder's wire sink, PR 8 / PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the ahead-of-stall lane refill's watermark tick. Any value refuses at startup."),
    k("SQUEEZEFS_ALLOC_LANE_REFILL_HINT", Kind::Retired { successor: "(nothing — the holder's grant reply is the supply, PR 8 / PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the co-writer lane refill's hint gate (`.benchmarks/2026-09-07-lane-refill-hint-gate.md`). Any value refuses at startup."),
    k("SQUEEZEFS_ALLOC_LANE_HARVEST_SINGLE_FLIGHT", Kind::Retired { successor: "(nothing — the grant window's top-up ask is single-flight by construction, `block_grant_topup_inflight`; PR 8 / PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the co-writer lane harvest's single-flight lever (`.benchmarks/2026-09-07-lane-harvest-single-flight.md`). Any value refuses at startup."),
    ...
    k("SQUEEZEFS_ALLOC_LANE_VOLUME_HINT", Kind::Retired { successor: "(nothing — a grant window is per data volume by construction, PR 8 / PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the co-writer lane refill's per-volume hint (`.benchmarks/2026-09-07-cowriter-fpp-supply-residue.md`). Any value refuses at startup."),
    k("SQUEEZEFS_FREE_GRACE_LANE_PUSH", Kind::Retired { successor: "(nothing — under read tokens the recall is a free's qualification and the ring is the timeout path alone; the lane hop it pushed is gone, PR 5 / PR 14)" }, "-", "RETIRED at the symmetric default flip (PR 14, forward-only): the freed-offset grace loop's lane-push lever — release on ack + the renewal grant's lane-supply hint (`.benchmarks/2026-09-06-free-grace-lane-visible.md`). Any value refuses at startup."),
    ...
    k("SQUEEZEFS_CLUSTER_WIRE_MAX_CONNS", int(64, 1 << 16), "derived: raw CPUs × 16, ceilinged by RLIMIT_NOFILE / 8", "Symmetric PR 13c (F-B3): the cluster-wire LISTENER's concurrent-connection cap (the S8 owner listener and the membership plane — every listener but the job wire's). Derived from the RAW affinity mask (a listener's load is the FLEET's width × each member's session demand, which N co-located daemons each serve whole — never the fleet-share-divided root, which shrank the cap exactly as the fleet grew: 64 on a 32-member fleet on 32 cores), floored at the shipped 64 and ceilinged by the fd budget (two fds per connection, a quarter of the soft RLIMIT_NOFILE — raised to the hard limit at startup). Explicit wins verbatim inside its range, railed by that fd ceiling only. A dial the cap refuses is retried bounded by the dialing member (`cluster_wire::RpcClient::connect`), then surfaces the typed EAGAIN class."),
    ...
    k("SQUEEZEFS_NVMEOF_TARGET_STACK", Kind::Enum { allowed: &["nvmet"], retired: &[("spdk", crate::nvmeof::SPDK_RETIRED_KNOB_VALUE_NOTE)] }, "nvmet", "Target stack selection (`--target-stack` flag > this env > nvmet — `nvmeof::resolve_stack`). The kernel nvmet target is THE target: `spdk` is a RETIRED value (R-SYM-8, docs/design-symmetric-metadata.md §5.8.1) — the startup gate refuses it naming nvmet and the re-share sequence; any other value refuses loud, never falls back to the default."),
    ...
    k("SQUEEZEFS_TEST_CORPSE_SWEEP_FAIL_DESTROY", Kind::Bool, "0", "Test seam: the mount-time corpse sweep refuses every chunk's `destroy_inodes` commit AFTER the chunk's corpses ran `delete_file` — the failed-destroy state (references released, records left live) the ledger gate must make harmless on the next mount (the generic/749 double-release repro, tests/corpse_sweep_tests.rs). Never set in production."),
    k("SQUEEZEFS_TEST_DESTROY_CHUNK_STOP_AFTER", int(0, u32::MAX as i128), "0", "Test seam: a single-ino CHUNKED destroy (one corpse whose record + xattrs + reference releases exceed the journal's whole-entry cap) stops after this many committed entries — the durable state of a crash between two of its chunks, which the next mount's sweep must finish (RECLAIM-ATOMIC residual B, tests/corpse_sweep_tests.rs). 0 = off. Never set in production."),
    k("SQUEEZEFS_TEST_FORMAT_FLAT", Kind::Bool, "0", "Test seam (PR 14 — the inverted matrix): a DEFAULT-class format request builds the `--single-writer` (flat) volume instead of the symmetric forest, so `tests/run_sym_forest_suites.sh flat` runs every default-formatting KV contract on the one flat writable class after the flip; the explicit builders (`format_v3_stamped_symmetric`, the single-writer and pre-flip multi-writer forms) ignore it. Never set in production."),
    ...
    k("SQUEEZEFS_TEST_META_BUFFERED", Kind::Bool, "0", "Test seam (PR 13i F-C1, design-symmetric-metadata §5.12): keep every registered metadata device path on BUFFERED I/O instead of O_DIRECT — the two-host pin's control arm, the pre-PR-13i posture in which a second host's kernel serves its own stale page cache of a block the manager rewrote (`tests/run_mw_matrix.sh sym-two-host`). Counted on `meta_io_buffered_fallback`; never set in production (the buffered fallback the product takes on its own is a REGULAR FILE on a filesystem that refuses O_DIRECT, tmpfs — loud, and never on a block device)."),
    k("SQUEEZEFS_SYM_RING_KB", int(SYM_RING_KB_MIN, SYM_RING_KB_MAX), "derived", "The per-appender journal ring size, KiB, on a symmetric-forest (bit 17) volume — the ring an appender ≥ 1 joins with (design-symmetric-metadata §1.6 'Per-appender ring', §6.1). DERIVED default `clamp(2 × ewma_commit_bytes/s × CHECKPOINT_MAX_AGE, 512 KiB, volume/64)`: two checkpoint ages of the appender's measured commit stream, never below the admissibility floor (the §4.4 pt 5 checkpoint reserve + one 128 KiB max entry, rounded to a power of two — `appender::SYM_RING_FLOOR_BYTES`), never above the solo ring's own derivation; a fresh join has no EWMA and takes the floor — and in PR 2 EVERY join is fresh (the appender's persisted commit rate, the EWMA input, lands with PR 3), so the derived default IS the floor today; growth on `journal_full_stalls` (≤ 8 segments) carries a BURSTY appender toward the ceiling — growth needs a drained ring, so an appender saturating its ring without pause stays at its size and every drain rides one cadence tick (the bound is `admissible bytes ÷ cadence` per region; this knob is the operator's lever for such an appender until PR 3's drain-then-grow arm). Explicit wins verbatim (clamped to the VOLUME's ceiling, logged). Range = the floor in KiB to the solo derivation's 32 MiB ceiling. Appender 0's ring is the format-time fixed extent and is not sized by this knob."),
    k("SQUEEZEFS_SYM_GRANT_EXTENTS", int(SYM_GRANT_EXTENTS_MIN, SYM_GRANT_EXTENTS_MAX), "derived", "The extent GRANT the manager of a symmetric-forest (bit 17) volume answers an appender with, in 256 KiB-class heap extents (design-symmetric-metadata §5.3.3, §6.1). DERIVED default `clamp(2 × ewma_smo_rate × manager_failover_bound_s, 8, free_heap / (4 × appenders))`: the images an appender needs to keep compacting through a manager failover at its measured SMO rate (`2 ×` = one EWMA window of under-estimate), never below the SMO budget floor (8 = ≤ 2 extents per compaction/split × 4 pending root swaps per cycle — `appender::GRANT_EXTENTS_FLOOR`), never above a quarter of the free heap spread over the appenders; refilled proactively at 50 % consumption. Explicit wins verbatim. Range = the floor to the u16 slot namespace's width."),
    k("SQUEEZEFS_SYM_ALLOW_NON_PR", Kind::Bool, "0", "KD-SYM-13: LOUD opt-in to arm the symmetric plane — a declared appender partition, a join, WERO on the metadata namespace — on a BLOCK DEVICE WITHOUT NVMe Persistent Reservations (a loop or null_blk volume; a regular FILE with no device over it is one kernel by construction and needs no opt-in — `reservation::single_kernel_substrate`), where fencing is detection-grade only and design-symmetric-metadata §5.8.2 names the acked-loss class a zombie can reach. Announced at every mount that uses it; never a default (lab use). `0` = the plane refuses to arm on a non-PR substrate."),
    k("SQUEEZEFS_META_PR_WERO", Kind::Bool, "1", "On a symmetric-forest (bit 17) volume whose symmetric plane is armed, the manager holds the metadata namespace under Write Exclusive – Registrants Only (rtype 3) and every other appender is a REGISTRANT under it, so a dead appender is fenced by preempting its registration (design-symmetric-metadata §5.8.1). `0` = the shipped Write Exclusive (rtype 1) — detection-grade for the other appenders — REFUSED on a PR-capable substrate unless SQUEEZEFS_SYM_ALLOW_NON_PR=1. Inert on every bit-17-absent (`--single-writer`) mount and on a single-kernel substrate."),
    k("SQUEEZEFS_PR_REGISTRANT_CAP", int(1, 65_535), "unset (learned from the device; unbounded on nvmet)", "A DECLARED NVMe Persistent Reservation registrant cap for a third-party PR array whose per-namespace registration table is vendor-bounded (design-symmetric-metadata §5.8.1, KD-SYM-18/23). Every registrant join reads the Reservation Report SIZED BY REGCTL and refuses LOUD at or past the cap in force — declared here, else learned from the first REGISTER the device refused with a non-conflict status, else unbounded — naming the count, the namespace and the remedy (the kernel nvmet target, THE target since R-SYM-8, has no cap; `pr_registrant_cap_refusals` is 0 on it by construction). Never a default: unset lets the device decide."),
    ...
    k("SQUEEZEFS_SYMMETRIC_META", Kind::Bool, "1", "The SYMMETRIC metadata plane — ON by default since the PR-14 flip (design-symmetric-metadata §7.2 / PR-plan row 14; KD-SYM-12): every RW mount of a symmetric-forest (bit 17) set is a WRITER through the join ladder — the D0 winner is the MANAGER, every later RW mount JOINS it over the wire (its own ring, page, checkpoint task, listener), every `-o ro` mount a member-reader + READ-TOKEN client (user-visible metadata exact at the next resolve, `reader_staleness_bound_ms` 0). The writer leases its native slot plus SQUEEZEFS_SYM_MINT_SLOTS rotor slots, mints by the bounded parent-slot affinity policy (KD-SYM-11/16), refuses a leaf mutation of a slot it does not lease (`meta_kv_leaf_lease_refusals`), homes the DLM on the live lease map (`dlm_mode` = `slot-homed`; `dlm_rpcs` 0 on a solo mount by construction) and serves the manager verbs on the S8 owner service. `0` on a stamped volume is the WRITABLE open's REFUSAL — the unarmed-forest posture retired with the flip, so `=0` names nothing a writer could run under; the reader / probe posture is `-o ro`, never a knob value. On a `--single-writer` (flat, bit-17-absent) volume the knob is inert: the flat solo posture, the one flat writable class after the flip. A multi-writer-class volume WITHOUT bit 17 (every default format between 2026-08-16 and the flip) refuses a writable mount whatever the knob says, naming `squeezefs volume enable-symmetric`."),
    k("SQUEEZEFS_SYM_MINT_SLOTS", int(1, SYM_MINT_SLOTS_MAX), "derived", "The ROTOR: how many non-native slots an armed symmetric writer leases at join for the inodes whose parent it does not lease (design-symmetric-metadata §5.1.2, §6.1 — PR 4). DERIVED default `M = clamp(W / (2 × writers_known), 1, MINT_SPREAD)` — `W` the derived routing width (65,536), `writers_known` the membership census at join (1 on a solo mount ⇒ 64 = `MINT_SPREAD`, the shipped granularity ceiling; 2 at the 12,500-writer operating point); the `2 ×` keeps half the width unleased for inheritance and churn. Explicit wins verbatim (the measurement lever); the manager refuses `AcquireSlots` above the size in force, and a forced shrink releases idle rotor slots down to it within one renewal beat. Range 1 to `MINT_SPREAD`."),
    k("SQUEEZEFS_SYM_AFFINITY_MAX_MB", int(1, SYM_AFFINITY_MAX_MB_MAX), "derived", "An explicit STATIC affinity ceiling, MiB, for the bounded parent-slot mint affinity of an armed symmetric writer (design-symmetric-metadata §5.1.2, KD-SYM-16 — PR 4), replacing the DYNAMIC derivation `A_max(t) = max(used_leaf_bytes(volume, t) / MINT_SPREAD, node_size)` — no slot tree holds more than 1/64 of its volume's CURRENT leaf bytes, never less than one extent: a child mints into its parent's slot only while that tree is below the ceiling, else into the rotor slot with the most headroom (`affinity_ceiling_spills`). The ceiling in force is published as `affinity_a_max_bytes`. The knob is the divisibility A/B lever (the design's row calls its floor 'the one-extent floor' — an extent is `node_size`, 256 KiB by default, so the registered floor is 1 MiB and the value is clamped at open to ≥ `node_size` and ≤ the heap). Soft: a mint is never refused by it."),
    k("SQUEEZEFS_SYM_T_IDLE_MS", int(1_000, MS_MAX), "derived", "The handover window `T_idle`, ms, of an armed symmetric writer (design-symmetric-metadata §5.1.4 — PR 4): the holder OFFERS a slot to ONE requester iff, over the same window, that requester's ships ≥ 2 × the holder's own ops AND ≥ `N_floor` (`slot_offer_n_floor` — the handover's own cost in ships); an idle tree (`ops_h` = 0) moves only to a requester that shipped ≥ `N_floor` ops into it, so a single touch never moves anything. DERIVED default `T_owner` (`SQUEEZEFS_MEMBERSHIP_LEASE_TTL_MS`, 45 s shipped) — the membership lease the slot lease lives as long as. Explicit wins verbatim (the measurement lever)."),
    ...
    match knob.kind {
        ...
        Kind::Enum { allowed, retired } => {
            if let Some((_, why)) = retired
                .iter()
                .find(|(value, _)| value.eq_ignore_ascii_case(raw.trim()))
            {
                return Some(format!(
                    "{}='{}' was RETIRED (forward-only — never a silent alias): {why}",
                    knob.key,
                    raw.trim()
                ));
            }
            core::parse_enum(knob.key, v, allowed)
                .err()
                .map(|e| e.to_string())
        }

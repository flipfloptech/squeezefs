...
- **What the forest does on a stamped volume.** Slot trees are minted lazily — a slot with no records owns no extent; every guest slot tree's root is published into tree 0 by the checkpoint task (one checkpoint-class journal entry, before the flush pass, so the same cycle's ledger record covers a slot tree's root swap — the dying-floor law one tree at a time); mount replays tree 0 first, opens every guest tree it names, then replays the window's slot-tree records by their keys' slots. The §4.10 digest walk runs over `(kind, legacy key, value)`, so a forest volume and a flat volume holding the same records digest EQUAL — the forest is a relayout, never a rewrite.

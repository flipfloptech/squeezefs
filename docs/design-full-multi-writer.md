...
2. When two live grants share a block (`FileCustody` sees this exactly — the stab window), the block **demotes to authority-assembled**: BOTH holders' writes to that block (including the first grantee's, symmetrically) **ship as extent records to the authority** over the publish wire (`PublishCall::WriteExtent`, publish schema 4 — the W2 extent-record vocabulary reused: offset-in-block, bytes, fencing token, `(lease_epoch, request_id)` dedup key). The authority merges via the existing extent overlay/fold and publishes once. One doctrine note stated against the module's own text: `publish.rs:33` declares the vocabulary **"No retry"** by default — `WriteExtent` explicitly joins the RETRIED class (the shipped-free precedent), which is exactly why it carries the dedup key; PR 17 states this against the no-retry doctrine rather than leaving it implied. **Why the authority and not a peer assembler**: the publish wire as built (`meta_ship/publish.rs`) is a client→authority channel and the custody plane has *no push backchannel* (the `dlm_custody_unknown_leases` law) — a peer assembler would need either an authority-relay lane (a second hop plus designation tracking) or peer-to-peer sessions (a NEW trust edge, contradicting §14); the authority already holds full data- and metadata-plane authority, is already every extent's publish destination, and its death is already the MW-3/S9-b failover story — a peer assembler buys only merge-CPU offload at the cost of a new topology, a new trust class, and a new crash actor. Priced as the exception path (D1: rarely-shared); the row that prices it (including the authority's merge CPU) is in §9.5. OQ-3 (assembler re-designation tiebreak) is thereby **resolved by construction** — there is no designation to move.
   ...
   - **§9.3a amendment — finding 16, the learning loop was structurally dark at scale (landed 2026-08-26, `.benchmarks/2026-08-25-s11-freeloop-stall.md` §Finding 16).** The s11-mpiio row's own ledger proved the two §9.3a channels above never fire on exactly the workload they were built for: under the 32-rank block-cyclic interleave the geometry cap churns grants FASTER than a renewal cadence, so 40 of 51 shrink notices died with their grant (`tail_shrink_fence_resolves`), the ceiling never learned (`stretch_ceiling_clamps` 0), and the row paid 8,610 desired trims + 5,449 full-budget conflict parks. Two halves: **(b) the trim teacher** — the acquire reply's OWN desired trim is a ceiling lesson (granted end below the stretched desire ⇒ the surviving stretch length teaches at the source, zero wire change; the same commit re-scoped `desired_trims` to FOREIGN clips only, since a same-scope wall is the admit-time merge's own bookkeeping, not contention), verified live: conflicts 5,449 → 505, grants churn −63 %, spans extend instead of re-minting; **(a) the carrier widening** — custody schema 6: the acquire reply became `AcquireReplyFrame` and the release reply `ReleaseReplyFrame`, each carrying the same demotion/shrink notice sets the renewal reply does (gathered after the acquire's own arbitration under the same `FileCustody` serialization — the composed-after-the-mark argument transfers verbatim), so a churn-shaped incumbent hears within ONE interaction and the renewal remains only the worst-case carrier for a quiet one. The ledgers, the fence column, and the barrier's `T_owner` backstop are all unchanged — the fix moves resolutions from the fence column to the ack column. Contracts: `the_acquire_replys_trim_teaches_the_ceiling_without_a_shrink_round` + `shrink_notices_ride_acquire_and_release_replies_not_just_renewals` (`tests/mw_ranged_lease_ladder_tests.rs`).
...
- **The owner's dispatch (D-5, 2026-09-08 — e2e perf audit DLM #7/#8, `.benchmarks/2026-09-08-d5-owner-hop-and-depth.md`)**: `meta_ship_owner_dispatch_ns` {`queue_hop`, `run`, `wake_hop`, `total`} — every owner-side dispatch on BOTH planes (the S8 verb frame; the S9 publish call / group / free / harvest) split at its two thread boundaries, exact-sum, always-on; `meta_ship.owner_dispatch_{inline,hops}` — which venue served it (`SQUEEZEFS_META_SHIP_INLINE_SERVE=1`: the served frame is polled on the accepting connection's own thread, so both hops read 0 and the S8 owner's `dispatch` IS its `run`; **ships OFF** — the 2026-09-08 squeeze-test fleet row priced the inline `run` 0.6–0.9 ms slower than the two hops it deletes, both orders; the `spawn_meta_join` hop is the default); `meta_ship_publish.ship_mux_frames` — publish frames that rode the endpoint's ONE pipelined session (`SQUEEZEFS_PUBLISH_SHIP_MULTIPLEX=1`: `ship_session_dials` reads 1 per authority instead of `SQUEEZEFS_PUBLISH_SHIP_DEPTH`; the owner's session lane serves the in-flight frames concurrently — ships OFF on measurement, the pool ran ≈ 1.5× at equal depth in-process because one session serves K frames on one owner thread; the capability arm for a connection-cap-bound fleet). The venue rule of §6.7 is untouched — never the conveyor's task — and its S8-era mechanical reason (the conveyor pass spawned on the committer's ambient runtime) is gone since rip-tokio-total.

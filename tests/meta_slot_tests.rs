...
use squeezefs::meta_backend::kv::checkpoint::{
    ...
    MEMBERSHIP_MAX_HOSTED_SLOTS, ROOT_LEDGER_SLOT_LEN,
};
use squeezefs::meta_backend::kv::superblock::{
    classify_volume, validate_meta_slots, FEATURES_INCOMPAT_KNOWN, FEATURE_INCOMPAT_KV_GUEST_SLOTS,
...
use squeezefs::meta_backend::{
    ...
    route_ino_width, volume_set_generation, Metadata, RoutedMetaBackend,
};
...
/// Format one LEGACY (default, stampless) v3 meta volume.
async fn format_legacy(meta: &Path) {
...
/// Kill-9 between (1) bit-2 set and (2) the first stamp-extended ledger
/// slot: the volume is bit-set + stamp-free. An OLD-mask binary must
/// refuse the mount at the superblock gate (naming bit 2); THIS binary
/// proceeds (legacy discovery — the stamp appears on the next stamped
/// write). The dangerous inverse (stamped-but-unbitted) is structurally
/// impossible to mount old: pinned below by the decode-as-absent proof.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_bit_before_first_stamp_kill9_window_old_mask_refuses() {
    let dir = tempfile::tempdir().unwrap();
    let meta = make_file(dir.path(), "meta", VOL_LEN);
    format_legacy(&meta).await;

    // (1) the bit lands durably; kill-9 strikes before any stamped slot.
    assert!(
    ...
    // The old-mask binary (bits 0|1|3) refuses loud, naming bit 2 — the
    // REAL gate code path (decode_sector_with_known), not a synthetic
    // "some bit set" assertion.
    ...
    std::fs::File::open(&meta)
        ...
        .unwrap();
    let old_mask = FEATURE_INCOMPAT_KV_V3
        | FEATURE_INCOMPAT_NODE_SEQ_WATERMARK
        | FEATURE_INCOMPAT_KV_VOLUME_LIFECYCLE;
    let err = squeezefs::meta_backend::kv::superblock::SuperblockV3::decode_sector_with_known(
        &sector, old_mask,
    )
    .expect_err("a pre-VL5a known-mask must refuse a bit-2 superblock");
    assert!(
        format!("{err}").contains("bit 2"),
        "refusal must name the unknown bit: {err}"
    );

    // THIS binary proceeds: the window is harmless forward (bit set + no
    // stamp yet ⇒ legacy-shaped discovery).
    squeezefs::meta_backend::kv::superblock::SuperblockV3::decode_sector_with_known(
        &sector,
        FEATURES_INCOMPAT_KNOWN,
    )
    .expect("this binary understands bit 2");
    let paths = vec![meta.display().to_string()];
    let disc = discover_meta_set(&paths).await.expect("window mounts here");
    assert!(!disc.stamped, "no stamp yet ⇒ legacy-shaped discovery");
    assert_eq!(disc.routing_width, 1);
}
...
/// A RoutedMetaBackend built the legacy way (`new`) routes EXACTLY as
/// before VL5a: W = volume count implicitly, identity for one volume.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_legacy_backend_routes_exactly_as_before() {
    let dir = tempfile::tempdir().unwrap();
    let meta = make_file(dir.path(), "meta", VOL_LEN);
    format_legacy(&meta).await;
    ...
    assert_eq!(routed.routing_width(), 1, "legacy W = volume count");
...
proptest! {
    ...
    fn prop_stamp_encode_decode_round_trip(
        ...
        n_slots in 0usize..=MEMBERSHIP_MAX_HOSTED_SLOTS,
...
/// The §5.5.1a space math made load-bearing: a worst-case record — 64
/// hosted slots (the documented cap) + 197 tree roots (64 × 3 guest + 5
/// native) — fits the 4096-B slot; a 65-slot stamp refuses LOUD at
/// encode (the cap exists precisely so the ledger slot can never
/// overflow).
#[test]
fn test_encode_slot_boundary_at_64_hosted_slots() {
    let uuid = [3u8; 16];
    let worst = LedgerRecord {
        membership_stamp: Some(MembershipStamp {
            set_uuid: uuid,
            set_epoch: u64::MAX,
            member_position: 63,
            member_count: 64,
    ...
    let img = worst.encode_slot().expect("64-slot worst case must fit");
    assert_eq!(LedgerRecord::decode_slot(&img).unwrap(), worst);

    let over = LedgerRecord {
        membership_stamp: Some(MembershipStamp {
            set_uuid: uuid,
            set_epoch: 1,
            member_position: 0,
    ...
    let err = over
        .encode_slot()
        .expect_err("65 hosted slots must refuse loud");
    assert!(
...
#[test]
fn test_meta_slots_bounds_enforced_at_format() {
    assert!(validate_meta_slots(0, 2).is_err(), "W = 0 refuses");
    assert!(validate_meta_slots(1, 2).is_err(), "W < volumes refuses");
    assert!(
        validate_meta_slots(129, 2).is_err(),
    ...
    assert!(
        msg.contains("64"),
        "refusal names the ledger-slot space cap: {msg}"
    );
}
...
fn test_plan_meta_slot_set_identity_map_and_hosted_slots() {
    ...
    for (pos, st) in plan.stamps.iter().enumerate() {
        ...
        assert_eq!(st.slots_hosted, expect, "identity distribution");
...
/// whose bytes re-encode identically stampless, and a config JSON with no
/// meta_* keys (KD-14's untouched-sets byte-identity law).
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_default_format_is_legacy_shaped_and_byte_identical() {
    let dir = tempfile::tempdir().unwrap();
    let meta = make_file(dir.path(), "meta", VOL_LEN);
    format_legacy(&meta).await;
    ...
    assert_eq!(
        ...
        0,
        "default format must never set bit 2"
    );
    ...
    assert!(
        ledger.membership_stamp.is_none(),
        "default format writes no stamp"
    );
    // Byte pin: the slot on disk is exactly the stampless encoding.
    ...
    assert_eq!(img, on_disk, "legacy slot bytes re-encode identically");
    ...
    assert!(cfg.meta_slot_map.is_none());
    assert!(cfg.meta_volumes.is_none());
    let out = serde_json::to_value(&cfg).unwrap();
    for key in ["meta_routing_width", "meta_slot_map", "meta_volumes"] {
        assert!(
            out.get(key).is_none(),
...
async fn test_reordered_uris_discover_same_set_and_same_generation() {
    ...
    // Contrast: LEGACY sets keep URI order authoritative — a reorder IS a
    // different generation (exactly today's behavior).
    let dirl = tempfile::tempdir().unwrap();
    let l0 = make_file(dirl.path(), "l0", VOL_LEN);
    let l1 = make_file(dirl.path(), "l1", VOL_LEN);
    format_legacy(&l0).await;
    format_legacy(&l1).await;
    let lf = vec![l0.display().to_string(), l1.display().to_string()];
    let lr = vec![l1.display().to_string(), l0.display().to_string()];
    assert_ne!(
        volume_set_generation(&lf).await.unwrap(),
        volume_set_generation(&lr).await.unwrap(),
        "legacy reorder stays a different metadata view"
    );
}
...
async fn test_repair_set_restamps_coherent_state_and_refuses_incoherent() {
    ...
    // Legacy set: nothing to repair, no writes, still legacy afterward.
    let l = make_file(dir.path(), "leg", VOL_LEN);
    format_legacy(&l).await;
    let paths = vec![l.display().to_string()];
    let restamped = squeezefs::config_ops::repair_meta_set(&paths)
        .await
        .expect("legacy set is a no-op");
    assert!(restamped.is_empty(), "legacy sets are never stamped");
    let disc = discover_meta_set(&paths).await.unwrap();
    assert!(!disc.stamped, "repair must not stamp a legacy set");
...
async fn test_stamp_survives_mount_write_checkpoint_remount() {
    ...
    for (i, m) in metas.iter().enumerate() {
        ...
        assert_eq!(
            ledger.membership_stamp.as_ref(),
            Some(&plan.stamps[i]),
            "member {i}'s stamp must ride every checkpoint's ledger record"
        );
    }
}

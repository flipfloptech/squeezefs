...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_FUSE_IO_URING_SPIN_US", int(0, 100_000), "derived (2 x the worker's park-gap EWMA, <= 200 us, while ops are in flight and box busy <= 80 %)", "Spin-before-park CAP for the FUSE-over-io_uring queue workers, us (e2e perf audit R-4, reap-thread economy): where a worker would block in submit_and_wait it first spins a bounded window watching IORING_SQ_TASKRUN / the CQ tail / the wake coalescer, so a device CQE or lane message landing inside it is reaped without a park + scheduler wake (the field's 35-60 us wake latency on 0.76 parks/op, 84 % of them <= 32 us). ADAPTIVE by default: the window derives per worker from its own observed gap EWMA (2x), engages only while that worker's queues hold ops in flight (an idle queue never spins), and is refused past the box's queueing knee (busy > 80 %, /proc/stat at the shared 100 ms cadence). `0` = never spin (the A/B control); an explicit value = the operator's verbatim cap (the box gauge does not refuse it). Engagement: transport_spin_{absorbed,expired,ns,refused_busy,window_us} — absorbed = parks deleted, ns = the CPU paid."),
    ...
    k("SQUEEZEFS_PUBLISH_SHIP_DEPTH", int(1, 64), "derived clamp(ceil(cpus/8), 2, 8)", "DLM S9 publish plane (E2E perf audit D-1b): publish FRAMES one co-writer keeps in flight per authority — one authenticated session each (the cluster wire is request/reply per connection). Derived on the owner RPC-lane slope (one per 8 cores, floor 2 = the minimum at which a frame's RTT overlaps a sibling's owner pass, ceiling 8 = the RPC-lane ceiling). `1` is the stop-and-wait A/B control (frames still batch concurrent publishes; nothing pipelines), never an operational escape. A frame is whatever is queued when a slot frees — no timer, no added delay on a serial stream."),
    ...
    k("SQUEEZEFS_ALLOC_LANE_HARVEST_AHEAD", Kind::Bool, "on", "The ahead-of-stall lane refill (finding 15 part 2, docs/design-free-grace-sustain.md §5.5 / PR 4): a laned co-writer's background task (one per volume, single-flight by construction, off the allocation path) harvests its lane's freed supply from the authority when the allocator is OWED blocks (Freed verdicts its shipped displaced frees brought back) and its lane-reachable stock sits below the derived watermark — ceil(claim-rate EWMA x refill horizon), capped at lane-share/4, where the horizon is the authority's own measured bound age carried on the harvest reply (schema 8, OQ 2) with the member-local derivation as the fallback. `0` = ENOSPC-triggered harvests only, the shipped shape verbatim (hint included). Engagement: alloc_lane_owed_blocks / alloc_lane_ahead_harvests / alloc_lane_harvest_watermark / alloc_lane_horizon_hints / alloc_lane_harvest_horizon_ms."),

...
- **Every RW mount is a writer; readers are exact.** The first RW mount of a volume set is its manager; every later RW mount joins it over the wire as a full writer with its own journal ring and leases (N unbounded by design), and any number of read-only mounts see every change exactly at their next lookup. A `--single-writer` volume keeps the one-writer mount guard, which refuses a second plain writer loudly, naming the holder.
...
**Scope, stated plainly.** Metadata authority is per *slot*: every metadata volume is a forest of slot trees, each slot leased by exactly one writer at a time (first-writer-takes-it, handed over by its holder when another writer dominates the work there, recovered by the set when a writer dies). Every RW mount of a set is a writer — the first is the set's manager, every later one joins it over the wire (`docs/design-symmetric-metadata.md`); each writer appends to its own journal ring, allocates data blocks from ranged grants its data volume's allocation-lease holder carves, and DMAs directly to the shared devices under custody its file's slot holder grants — on NVMe-oF storage with persistent reservations the device fences a dead writer. Any number of **read-only mounts** (`-o ro`) are read-token clients: a foreign change is visible exactly at the next lookup, never after a delay. A volume formatted `--single-writer` keeps the one-writer posture and refuses a second plain writer, naming the holder. All of this is proven on real mounts at lab scale (up to 32 co-located mounts, three daemons on a kernel nvmet target, two kernels on one nvme-tcp LUN, an 8-node AWS fleet over nvme-tcp); the 15,000-node design target rests on simulation and arithmetic, never a measurement — every scale claim carries its tier in [docs/rc-manifest.md](docs/rc-manifest.md).

...
**§5.3 second consumer (2026-09-09, the inline raise):** the same `xattr_value_cap(ino) − 4 KiB` derivation that bounds the inline BLOCK MAP now also bounds the inline DATA payload — `routing::derived_inline_max_bytes` is the inline-layout ceiling (60 KiB at the default node, 12 KiB at the 64 KiB floor; `SQUEEZEFS_INLINE_MAX_BYTES` overrides within 4096..=61440), replacing the free 4 KiB constant. A file up to it keeps its payload as the layout record's `data_key` (one KV value, ≤ the cap by construction; the layout-delta wire's `u16` data-key length is satisfied a fortiori), and `promote_staged_file` dispatches a staged file at or under it INTO that record instead of a whole block (`.benchmarks/2026-09-09-fsync-promote-staged-ab.md` §3–4). Nothing on disk changes: the record is the same `"layout"` xattr value the format already admits at this size.

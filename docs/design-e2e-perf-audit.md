...
| ~~7~~ | per-WRITE allocs — **LANDED 2026-09-08 (W-6)** | 4 Box + 4 Arc slot closures, per-block `String`s, ~30 atomics | 14.6 → 3.06 allocs/op; 17 shared-line RMWs striped; one `Arc` per zc slot |
...
| ~~9~~ | fsync flushes ALL data namespaces + 3 serialized meta legs — **LANDED 2026-09-08 (W-5)**, box row owed | was: no `fsync_phase_ns`; now `fsync_phase_ns` exact-sum + the `fsync_*` ledger (in-process: 99.8 % of an fsync is barrier wait) | touched-namespace barriers (stripe word stamped at the C8 closure + the in-place DMA shapes; write-through skipped) + joined legs; DUR-1 order kept |

...
//! | **D4** meta node occupancy | per meta volume: dead (superseded) records vs distinct keys in the serialized bset logs (`KvMetaBackend::dead_bset_census`), plus the UNDERFULL leaf count `mergeable_leaves` (fold ≤ ¼ capacity — design-cow-kv-metadata §4.6a) | `JobType::DefragMeta` — leaf compaction through the SMO serialization, then the §4.6a sibling merge sweep — bounded, cursor-resumed, one whole lap per job under the duty cycle (`defrag_merge_sweep`) |
//!

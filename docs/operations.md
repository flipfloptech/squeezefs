...
> **ℹ️ Staged-layout files and multi-client visibility.** On a filesystem WITH staging dirs, a file whose size lands in `(4 KiB, block_size]` takes the **staged layout**: its whole payload is one entry in the writing host's local staging ring, its metadata on the shared volume says "the bytes live in ring entry `<file_id>`", and the shared data backend holds **nothing** for it until the entry is **promoted** (one backend block + one metadata commit). `fsync(2)` makes such a file durable **on that ring** (it syncs the shard and commits the layout; it does not promote), and promotion is otherwise **staging-pool-pressure-driven** (the merge worker drains the ring past its 75 % high-water mark). Consequences: the same host remounting at the same mount point recovers every staged-layout file byte-exact (a moved mount point is reported as `MOVED-MOUNT-POINT STAGING RESIDUE` with its adopt/discard remedies); **every other client of the volume set** — another mount point, another host, read-only mounts, co-writers — reads an un-promoted staged-layout file as size-consistent **zeros**, silently (no error; the reader's `staged_payload_lost_reads` counts it and its log names the file). The daemon's dismount census reports the resident population at INFO as exactly that (`staged-layout files resident in local staging at dismount: N`); the WARN is reserved for unflushed **active write blocks**, which the unmount wait CAN drain. `squeezefs umount` offers its "[w] Wait" only for that second class. Evidence: `.benchmarks/2026-09-09-dismount-staged-residue.md`.

...
- `src/jobs.rs`, `src/nvmeof.rs`, `src/p2p.rs`, `src/config_ops.rs` — maintenance-job worker skeleton (throttle law + queue; the volume-lifecycle fabric extends it), NVMe-oF control, peer-to-peer, and the offline **guarded** admin verbs over durable on-volume state (cache-path policy, `volume add-data`/state — PR VL3). (Staging crash recovery lives in `src/cache/nvme.rs` — `NvmeStaging::new`; the former `src/recovery.rs` stub was deleted, ENG-12.) The historical `/dev/shm` runtime-config file is DELETED (VL3): live `enable`/`disable` health overrides ride the admin lane (`BackendRouter::set_health_override`); offline enable/disable is durable `DataVolumeRecord.state`.

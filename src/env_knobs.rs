...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_FUSE_ZC_WRITE_FUSION", Kind::Bool, "off", "Handler/worker FUSION for small armed FUSE WRITEs (zc-write-fusion campaign). Default OFF: field-falsified 2026-08-08 — at fabric RTT the fused lane's bounded polling collapsed armed rand-4k writes to 0.45x (vs 0.78x fusion-off), inverting the local +12% bracket; re-enables by default only after the RTT-tolerant re-derivation proves itself on the fabric-emulated venue. `1` opts in. Engagement: fuse3_zc_write_fusions/_bytes."),
    ...
    k("SQUEEZEFS_IPC_DD_EAGER_FLUSH", int(0, 1 << 12), "derived (kernel_inflight/8, floor 1)", "Direct-drive issue cadence: explicit K > 0 enters inline once a lane's unflushed SQE count reaches K (wins verbatim); 0 = sweep-only flush (the pre-r4 A/B posture — at fabric RTT it was the per-lane depth ceiling); absent = the r4 adaptive threshold bounding the un-issued tail to 1/8 of the lane's live pipeline."),

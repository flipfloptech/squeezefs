...
async fn test_checkpoint_writeback_and_multipart_upload() {
    ...
    let upload_id: Option<String> = con.hget(&active_mp_key, "upload_id").await.unwrap();
    assert!(
        upload_id.is_some(),
    ...
    let upload_id_post: Option<String> = con.hget(&active_mp_key, "upload_id").await.unwrap();
    assert!(
    ...
    let block_map_id_opt: Option<String> = con.hget(&meta_key, "block_map_id").await.unwrap();
    let block_map_id = block_map_id_opt.unwrap();
    let block_map_key = format!("block_map:{}", block_map_id);

    let b0_key: Option<String> = con.hget(&block_map_key, "0").await.unwrap();
    let b1_key: Option<String> = con.hget(&block_map_key, "1").await.unwrap();
    let b2_key: Option<String> = con.hget(&block_map_key, "2").await.unwrap();

    assert!(b0_key.unwrap().contains("s3_single"));
    assert!(b1_key.unwrap().contains("s3_single"));
    assert!(b2_key.unwrap().contains("s3_single"));
...
async fn test_checkpoint_writeback_existing_file_fallback() {
    ...
    let overwrite_data = vec![0xCC; 1024 * 1024]; // 1MB
                                                  // Open the file again
    fs.open(req, ino, 0).await.unwrap();

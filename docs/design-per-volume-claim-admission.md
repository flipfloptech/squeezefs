...
| **Date** | 2026-08-21 |
| **Status** | **PROGRAM CLOSED (acceptance complete 2026-08-24)** — PRs 2–7, 7b and 8 landed; PR 8's rows all PUBLISHED with oracles clean at binary `367ad855` (closing record `.benchmarks/2026-08-22-pv-claim-admission.md`): gate (a) **0.98× of S0** (≤ 1.10× MET — vs the 6.73× the S10 gate adjudicated), the §5.12 funnel **0.92×** with the shipped-free ledgers closing live, the D18 refusal table at its designed rates, the R14 price published per posture, census growth 0. Thirteen findings fixed red-first during acceptance (the run log's numbered list — the udev flock wait-out, KD-PV-17 era attestation, co-located WERO adoption, grace-window enrollment, lost-FORGET corpse sweep, corpse-census correction, drift-gauge feeding law, debt-drain pacing, and the finding-13 owner-partitioned shipped-free ledger read among them). The named follow-ons re-filed on `docs/design-full-multi-writer.md`'s residual board: cross-owner slot migration (D19), offline subtree re-homing (OQ 3), the R15 purge half, R14's W1 recovery on partial authorities |

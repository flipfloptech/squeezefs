...
- **What is NOT held.** No foreign 4a guard travels (an as-built deviation from §5.6's `acquire_remote` line — `dlm_rpcs` stays 0 on the cross-owner path): a guard held at the holder beside the initiator's stripe-canonical `lock_many` has no common order with a peer performing the inverse op (`A: local X → remote Y`, `B: local Y → remote X` is a cycle, and Lustre's FID-ordered acquisition would need every 4a site to order by ino); the holder's apply under ITS guards plus the witness give the same isolation at commit. A two-holder create therefore costs exactly ONE shipped step (the `InsertDentry`) and zero lock round trips — pinned.
...
- **Gauges** (`.stats`): the Cross-owner family — `xv_cross_owner_intents_{minted,retired,open}` (closure `minted ≡ retired + open`), `xv_cross_owner_steps_{shipped,served}`, `xv_cross_owner_intents_stuck` (must-stay-0), `xv_cross_owner_phase_ns` (`plan / intent_barrier / ship_rtt / retire / total`), `dir_rename_lock_{acquires,wait_ns}` — every one 0 on an unarmed or bit-17-absent mount, pinned; the S3.5 `crossvol_tx_*` family keeps its meaning (a shipped step's outcome is counted by the HOLDER).

...
| Knob | Accepts | Default | Purpose |
|---|---|---|---|
| `SQUEEZEFS_NVMEOF_TARGET_STACK` | spdk / nvmet | `spdk` | Target stack selection (`--target-stack` flag > this env > spdk — `nvmeof::resolve_stack`); a malformed value refuses loud, never falls back to the default. |
...
squeezefs nvmeof unshare <subnqn> [--force] [--accept-version-drift]
                                           # stack resolved from the share ledger;
                                           # --force overrides the live-consumer refusal
squeezefs nvmeof list [--json]             # managed/down/pending/removing/foreign, both stacks
squeezefs nvmeof restore [--target-stack spdk|nvmet] [--accept-version-drift]
                                           # replay ledger; reconcile intents (the stack flag is a FILTER)
...
squeezefs nvmeof target install [--version <pinned-tag>] [--with-pkgdep]   # pinned SPDK v26.05 build, sha-verified
squeezefs nvmeof target setup [--hugemem-mb 2048 | --restore-prior] [--target-stack spdk|nvmet]
squeezefs nvmeof target start [--core-mask 0x..|--cores N] [--dpdk-mem-mb 1024] [--accept-version-drift] [--target-stack ...]
squeezefs nvmeof target stop [--force] [--target-stack ...]      # save_config -> TERM -> grace -> KILL
squeezefs nvmeof target status [--json] [--target-stack ...]     # RPC liveness, version+drift, reactors, ledger
squeezefs nvmeof target systemd-unit [--core-mask 0x..|--cores N] [--dpdk-mem-mb 1024] [--target-stack ...]
                                           # emitted to stdout, never installed; bakes the start flags given

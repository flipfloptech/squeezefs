...
| `dlm_custody` (object: `mode`, `dlm_custody_held`, `dlm_custody_{grants,renewals,releases,releases_deferred,conflicts,reclaims}`, `dlm_revokes_{issued,expired}`, `dlm_custody_unknown_leases`, `dlm_custody_self_fences`, `dlm_custody_{quarantined_offsets,drain_proofs}`, `dlm_custody_grace_conflicts`, `dlm_rpcs_custody`, `dlm_custody_range_{acquires,extensions}` (the client's wire-face range ledger), `dlm_custody_notice_poll{s,_notices,_failures}` (the standing notice poll: rounds = channel liveness, notices = the quiet-incumbent engagement, failures = rounds that died — session churn), `dlm_custody_grant_census` (opt-in)), `dlm_custody_generation`, `dlm_custody_epoch_advances`, `dlm_custody_phase_ns` (`rtt`, `arbitrate`, `adopt`, `renew`) | Remote write custody on an armed multi-writer plane — all `0`/`off` on a single-writer mount by construction ([Multi-writer data plane](#multi-writer-data-plane-dlm-stage-s9)); `releases_deferred` = release verbs held behind the inode's publish drain (the ordering fix's engagement); `reclaims` = grace-window reclaims admitted on a successor |

...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_SYM_ALLOW_NON_PR", Kind::Bool, "0", "KD-SYM-13: LOUD opt-in to arm the symmetric plane — a declared appender partition, a join, WERO on the metadata namespace — on a substrate WITHOUT NVMe Persistent Reservations (a file-backed or loop volume), where fencing is detection-grade only and design-symmetric-metadata §5.8.2 names the acked-loss class a zombie can reach. Announced at every mount that uses it; never a default (lab use). `0` = the plane refuses to arm on a non-PR substrate."),
    ...
    k("SQUEEZEFS_SYMMETRIC_META", Kind::Bool, "0", "Arms the SYMMETRIC metadata plane on a symmetric-forest (bit 17) volume (design-symmetric-metadata §5.1, PR 4): the writer that wins the D0 ladder leases its native slot plus SQUEEZEFS_SYM_MINT_SLOTS rotor slots through the manager's `AcquireSlots`, mints new inodes by the bounded parent-slot affinity policy (KD-SYM-11/16), refuses a leaf mutation of a slot it does not lease (`meta_kv_leaf_lease_refusals`), homes the DLM on the live lease map (`dlm_mode` = `slot-homed`, `dlm_rpcs` still 0 on a solo mount by construction) and serves the manager verbs on the S8 owner service. `0` (the shipped posture until the PR-14 flip) = the PR 1–3 dark forest EXACTLY — every slot the mount's, the shared mint rotor, `dlm_mode` = `solo`. `1` on a bit-17-ABSENT volume refuses the mount loud naming `squeezefs volume enable-symmetric` (PR 11). Read once at the writer's open. A `-o ro` mount reads it too (PR 5, §5.7.2): `1` makes the reader a member-reader + READ-TOKEN client of the volume's holder — user-visible metadata exact at the next resolve, `reader_staleness_bound_ms` = 0, refused loud without a membership lease, a declared holder endpoint (SQUEEZEFS_MW_AUTHORITY) or the cluster secret; `0` = the S5 bounded-staleness reader verbatim."),

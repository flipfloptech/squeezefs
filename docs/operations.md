...
| `meta_kv_forest_slot_trees_minted`, `meta_kv_forest_root_publishes`, `meta_kv_forest_key_violations`, `meta_kv_forest_reader_window_skips` | **The slot-tree forest** (design-symmetric-metadata §5.2, incompat bit 17 — PR 1, dark until the PR-14 flip; a volume without the bit moves none of these). `slot_trees_minted` = guest slot trees minted on a slot's first record (one extent each — the lazy-mint engagement gauge); `root_publishes` = `slot_state` records the checkpoint published into tree 0 (one per guest slot whose root moved since its last publication; ≤ checkpoints × slot trees); `key_violations` **must stay 0** — a record that reached a forest volume's commit, replay or READ path under a key the §5.2.1 codec refuses (a kind byte that is another tree's id, a wrong length, a by-block prefix other than refs): the partition-violation class of "a kind byte is never another tree's id" — the kind-routed walks SKIP such a record (so a census is never truncated by one) and fsck's forest C1 walk reads the slot trees raw to report and repair it; `reader_window_skips` = window records a NON-WRITER open (a `-o ro` reader, a co-writer, a probe) skipped at replay because their slot tree had no published root yet — served at the poll after the writer publishes (S5 bounded staleness); 0 for the life of every write mount by construction. |

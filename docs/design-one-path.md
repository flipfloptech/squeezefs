...
| **Date** | 2026-08-09 (Rev 3 same day — review round 3 folded) |
| **Status** | **Rev 3 — design only, review rounds 1–3 folded.** Formalizes the product ruling of 2026-08-09: too many specialized "vehicles" exist so we can hit certain benchmark numbers; the system must work at maximum performance in both read/write and IOPS *all the time, regardless of pattern*. This is **not** "one memcpy/DMA sequence for every shape." It **is** one admission, always the cheapest legal action, never a named posture / A/B lever on the hot path. Rev 3: P0 ACK partition is three handler-return arms (overlay / W1 / accumulate); whole-block inplace is an **upload** gauge, not an ACK increment; mermaid `INL --> ACK`; F0 fusion cap is computed once per session (recompute if streaming-hold flips), mount-log at most once. OQ1–OQ5 remain product calls. |
| **Repo / branch** | local `dev` @ `c987ce0b` (`perf(daemon): O_DIRECT overlay extracts at delivery, Bytes ACK-early`). 11 commits ahead of `origin/dev`, not pushed. |
| **Intended home** | `docs/design-one-path.md` |
...
| 3 | 2026-08-09 | **Review round 3 folded (3 items).** ACK partition is overlay / W1 / accumulate only; `write_through_inplace_overwrites` is upload engagement (out of the ACK sum) — whole-block inplace is decided in `pipeline_upload_parked_block` after ACK and must not double-count accumulate. `INL --> ACK`. F0 computes the cap once at session arm (recompute if `set_zc_hold_streaming` flips the bound); hot-path `fusion_ceiling` only reads the cached value; mount-log at most once. |
...
Several cells are **structurally unmeasurable** until later PRs / OQ answers (§8). P0 and P1 do not claim this sentence.
...
The user (product owner) stated they have too many specialized vehicles so we can hit certain benchmark numbers. They want the system to work at maximum performance in both read/write and IOPS **all the time, regardless of pattern**. The 24.7 GB/s A-leg is the exhibit, not the goal. The write strategic end state is still `docs/design-device-overlay.md` §1.3: **≥ 0.85× same-day raw, sustained**, with overlay PR **B5** closing the overlay program's own **B-initial** gate (≥ 0.80× and above the §1.4 comparator). This program's P2-flip uses the **A-leg bandwidth number** as *one* conjunct of its AND-gate; it does not rename or replace B5.

The user's "~40" is unit-ambiguous (see §2.2). Until clarified, the *fraction* against same-day raw is the bar, not a fixed GB/s.
...
2. **The default is not the best recipe.** Overlay — the fresh-path store the write-bandwidth program named as the strategic end state — ships `SQUEEZEFS_DEVICE_OVERLAY` default OFF because B2 ACK-after-CQE lost 0.77×. Until the ACK-early A-leg beats accumulation **and** the rest of the P2-flip AND-gate, "max write all the time" is a lie on a default mount.
...
4. **Dead code wearing a lever.** FUSE placed-merge default-OFF is not enough (AGENTS.md no-dead-code) *if* OQ2 chooses delete. The held-GUP snapshot path is not a product lever. `fuse3_zc_write_lazy_extractions` growing on a steady row is the hold gate regressing, not a second vehicle.
...
5. **Overlay becomes the fresh path after the P2-flip AND-gate**, not after a renamed "B-initial." This design does **not** flip the default. Overlay PR B5 still owns the overlay program's B-initial close.
6. **Overwrite stays one store family, not a second zoo.** Sub-block exclusive = W1 (already). Whole-block exclusive is a **three-way counted row** (inplace vs today's CoW residual vs overlay-B4-after-B4-is-built). One-path does not delete CoW and does not build B4.
7. **One client data plane *story*.** IL when the ring is cheaper, FUSE when it is not — **live cost**, not `bs > 64K ⇒ kernel`. OQ3 picks the end state; until then the hybrid *model* stays and the dual advertisement does not.
...
One data plane. IL when the ring is cheaper, FUSE when it is not — **live cost**, not `bs > 64K ⇒ kernel`.

Today `lane_gate.rs` approximates live cost with a startup memBW probe × a measured 2.3 µs lane-RTT delta, clamped to `[slab, max_op]`. That is a *model of* live cost, and it is the right kind of thing — a derivation, not a recipe. On field memBW the derived interior is ~140–164 KiB, so **1 MiB already routes kernel**. Advertising IL as a universal fast path while that is true is the dual story this design forbids.

OQ3 picks the end state (P4-A kill the gate after 1 MiB IL matches kernel, or P4-B declare kernel FUSE the large path). Until OQ3, there is **no P4 code PR** — only a measurement note (§8). The hybrid model stays in P0's "legitimate derivation" bucket.

Long-term the kernel lane is POSIX compatibility (open/stat/readdir/mmap/locks/fsync/the fd capability), not a second fast path. That is D14's fleet-posture corollary made honest.
...
No unlabeled edge leaves a write-admission node. Drain is the FUSE **read** handler (and the IL demote target), never a write epilogue. Worker-side HOLD/extract/place is a **conservative projection** of the handler (§5.2), not a second admission. Fusion is a *scheduling* detail of a **held** W1 at ≤ `min(explicit, hold_candidate bound)` (default derivation remains `payload/8`). FUSE placed-merge does not appear as a product path. The hybrid lane gate, until OQ3, is a client-side approximation of live cost, not a second filesystem.
...
* `SQUEEZEFS_ZC_ACK_EARLY_ODIRECT` leaves the product surface when OQ2's sibling cleanup lands (rename to `SQUEEZEFS_TEST_*` or delete). Production overlay O_DIRECT does not HOLD.

### 5.4 Overlay is the fresh path — P2-prep then P2-flip

`SQUEEZEFS_DEVICE_OVERLAY` default OFF is the D17 measured posture: B2 ACK-after-CQE lost 0.77× with engagement exact; shipping it default-ON would make every default 1 MiB ingest *slower* than today's accumulation. `device_overlay_enabled`'s comment still says "default OFF until the **B5 gate row**." This program does **not** redefine "B-initial." Overlay PR B5 still closes B-initial (fsync-storm + B4 coexistence + the §1.3/§1.4 number).
...
1. **The A-leg number** — ACK-early + at-delivery extract, overlay ON vs shipped accumulation, reset-per-leg A-B-B-A, nvmet-tcp or field fabric, ≥ 60 s flat, engagement exact (`overlay_store_bytes` ≈ user bytes, extract ≈ 0, `nt_copy` ≈ 0, `fuse3_zc_write_lazy_extractions` = 0). Materially above the extraction control **and** ≥ 0.80× same-day raw (device-overlay §1.4 case 2 — Approach A is dead evidence). Written as a `.benchmarks/` note (the 24.7 GB/s comment is not this row).
2. **OQ4 resolved, and the corresponding work landed:** either overlay B3 (lock-free compose) is in, **or** a written restriction that acceptance **read** cells only run after coverage-complete/fsync on full-block files. Drain-on-read + zero-seed of the unwritten 3 MiB of a 1 MiB-into-4 MiB overlay is a structural read-cell regression; it is not "read cells might move."
3. **Overlay §8's ACK-early read/fsync restatement implemented** — wait-or-serve-retained, not "comment says instant." `await_overlay_inflight` is load-bearing; the yield-spin is not the retained-source serve the future law also allows.
4. **B2 fsync-storm contracts green on the ACK-early binary** (fresh/hole only). Name the existing `device_overlay_tests` fsync / seed-zeros / `overlay_unpublished_at_fsync == 0` pins plus a storm under ACK-early (`overlay_fence_drops == 0`). That is the fsync work a **fresh/hole** default needs. It is **not** overlay B5.

**Overlay B5-with-B4 is not a P2-flip conjunct.** B5 (`docs/design-device-overlay.md` §8) is the full §6.2 sequence under storms *with rewrite-shadow default-ON and the B4 coexistence arm engaged*. B4 is overwrite overlays (not built; `try_device_overlay_store` still `Ok(false)` on mapped blocks). One-path does not build B4 (KD-OP-9) and does not wait on it. Overlay B5 remains the overlay program's **B-initial** close. If the overlay program later amends B5's criteria, that is their note, not a silent wait in this ladder.

Until P2-flip, every claim that a default mount is "max write all the time" is refused. This design will not launder the flip. OQ1 (flip now vs wait) remains a product call; the design recommendation is wait.
...
### 5.7 Client — measurement now, product PR after OQ3

```mermaid
flowchart LR
    App --> Shim{IL bound fd?}
    Shim -->|no / bail-out| FUSE[kernel FUSE]
    Shim -->|yes| Cost{ring cheaper than FUSE<br/>for THIS op?}
    Cost -->|yes| Ring[IPC ring]
    Cost -->|no| FUSE
    Ring --> Admit[daemon handler admission]
    FUSE --> Admit
```

Two honest end states (**OQ3 — not picked**):

* **P4-A — kill the hybrid.** 1 MiB IL matches kernel. `SQUEEZEFS_IL_KERNEL_LANE_MIN` default becomes `0` and then the knob is deleted. Every bound data op rides the ring. Kernel FUSE remains the compatibility/fallback lane.
* **P4-B — declare kernel FUSE the large-write path.** Keep the derived cut; stop advertising IL as universal. Scoreboard/charter: large seq is a kernel-FUSE claim; IL is the IOPS/rand-small claim. The IL 1 MiB acceptance cell is labeled "kernel lane via hybrid, not a ring claim."

Until OQ3 and a 1 MiB IL-vs-kernel row on the post-P-minus-one (ideally post-P2-flip) binary, **there is no P4 implementation PR**. A measurement note can run any time; on pre-P2-flip it measures accumulation vs kernel, not overlay vs kernel, and must say so.
...
|---|---|---|
| `SQUEEZEFS_DEVICE_OVERLAY` | default **OFF** (B2 0.77×); comment says until B5 | **P2-flip** may default ON after the AND-gate. Then `=0` is class (3) (A/B that proves fallback). **Not flipped in this design.** OQ1 |
...
| `SQUEEZEFS_ZC_ACK_EARLY_ODIRECT` | default OFF — residual held-GUP snapshot | Class (2) or (1) when that cleanup PR lands (depends on OQ2's sibling). Production overlay O_DIRECT does not HOLD |
| `SQUEEZEFS_FUSE_PLACED_MERGE` | default OFF, falsified ~25 % | **OQ2.** If delete: class (1) of the **FUSE-only** surface. If keep: class (3) `=1` instrument until a written B4 design names memfd assembly. IL `placed_sever` is not this knob |
...
| `SQUEEZEFS_IL_KERNEL_LANE_MIN` | derived | legitimate live-cost model until OQ3. Explicit `0` class (3). No P4 code change until OQ3 |
| `SQUEEZEFS_DIRECT_DEVICE_TRUE` | default OFF | class (3) **measurement escape**, never a second read path |
| `SQUEEZEFS_PATCH_MAX_BYTES` | derived block/8; `0` = A/B | `0` stays the W1 off lever (OQ5). Default stays derived. Class (3) |
...
| `docs/design-device-overlay.md` Resolved Question #1 / §8 accelerator | v1 is ACK-after-CQE | Distinguish B2 *measured* ACK-after-CQE from tip *code* ACK-early; §8 future law is P2-flip AND item 3 |
...
**fuse3:** no new opcode. `odirect` stamping stays (P-minus-one). `fusion_ceiling` hold-bound cap is **`fix(fuse3)`**, not P0. FUSE `zc_write_place` is deleted **or** left behind `=1` **after OQ2** — do not `cfg(test)` it (fuse3 is a separate workspace; that does not hide it from the daemon).

**Shim:** no ABI bump in P0–P3. No P4 code change until OQ3.
...
| kernel × seq 1 MiB × read | dest-lease / zc serve on a **published** map | P0 on non-overlaid files; **P2-flip item 2** (OQ4) for overlay-covered | Drain+zero-seed of a partial overlay is not dest-lease |
...
| IL × seq 1 MiB × write | **kernel FUSE via hybrid** on derived interior < 1 MiB | labeled **P4-B** until OQ3; **P4-A** only after 1 MiB IL matches kernel | Not a ring claim today. `ipc_lane_gate_kernel_bytes` must account for the row |
| IL × seq 1 MiB × read | same hybrid | same as write | consume-copy tax is why the gate exists (28 vs 49.2 GB/s class) |
...
| **KD-OP-4** | **Overlay is the fresh/hole store after the P2-flip AND-gate, not after a renamed "B-initial."** Do not flip `SQUEEZEFS_DEVICE_OVERLAY` in this design. Overlay PR B5 still closes B-initial and is **not** a P2-flip conjunct (item 4 is B2 fsync-storm on the ACK-early binary). | B2 ACK-after-CQE lost 0.77×. B5-with-B4 would stall default-ON on unbuilt overwrite overlays this program refused to absorb. |
...
| **KD-OP-7** | **FUSE placed-merge is dead as a product path** (25 % capture). Whether the FUSE *code* dies is OQ2. IL `placed_sever` stays. P1 is not opened until OQ2 is decided. | §3f Approach A falsified. Deleting `placed_sever`'s IL arm would kill 1-copy ring writes. |
| **KD-OP-8** | **W1 is not a vehicle; it is the exclusive-mapped in-place arm.** `patch_ineligible_*` stays the decision ledger. `try_patch` before `try_overlay` stays until P2-flip (ledger hygiene, not a bug). | Deleting W1 is the 2,500× RMW regression. Reordering before default-ON churns the W1 series. |
...
| **KD-OP-11** | **Client: live cost, not a size recipe.** The hybrid *model* stays until OQ3. No P4 code PR until OQ3. Large-seq IL is already kernel via the derived interior. | D14 + 49.2/35.3 vs 594 k/28 GB/s. Dual advertisement is the lie. |
...
Rejected as a silent choice. P4-B is the honest form for *large* ops only, after OQ3.

### Alt 4 — IL-only data plane (hybrid gate off forever)

Rejected as a silent choice. P4-A is the honest form — *after* 1 MiB IL matches kernel.

### Alt 5 — Flip overlay default ON now, measure after

Rejected **in this design** as a silent flip (OQ1 may still choose it). B2's counted row is 0.77× and 0.495× raw.
...
IL stays behind the §5.2 daemon fd screen (KD-6 of design-preload-interception). OQ3 does not widen bind rights.
...
`overlay_store_bytes` stays the **CQE / device-engagement** face (`finish_ack_early_*` after CQE, and only if `stored`). P2-flip item 1's "store_bytes ≈ user bytes" on a completed ≥ 60 s row is still that face. P0 pins `bytes_overlay_ack_early_returns_before_device_cqe`: at ACK, `admit_write_overlay_bytes == pwrite len` and `overlay_store_bytes` may still be 0.
...
**Client:** `ipc_ops_*` / `ipc_bytes_*` (ring) vs `ipc_lane_gate_kernel_{routes,bytes}` (hybrid). An il row still requires engagement exact. The IL 1 MiB cell must show kernel-lane bytes, not ring ops, until OQ3/P4-A.

### 12.2 What dies or becomes inverted

| Gauge | Fate |
|---|---|
| `fuse3_zc_write_placements` / `_bytes` / `placed_fuse_claims` | 0 forever after OQ2-delete; or instrument-only if OQ2 keeps `=1`. IL `placed_merge_elides` stays |
...
| `overlay_read_drains` | B2 drain-tax; → 0 on dest-armed overlay reads after B3 (compose) or after full-block-only restriction |
...
| **P1** | unchanged | **Not opened until OQ2.** Then either FUSE-only delete or `=1` instrument. Lazy-extract tripwire may land with P0 if independent |
| **P2-flip** | overlay default ON | **AND-gate in §5.4.** Then `DEVICE_OVERLAY=0` is class (3) |
| **P3** | inplace absorbed or class (3) | Three-way row; CoW residual stays unless the field number kills it. No B4 |
| **P4** | per OQ3 | **Not opened until OQ3.** Measurement note can run any time |
...
| # | Risk | Sev | Mitigation |
|---|---|---|---|
| R1 | Field A-leg with ACK-early still loses to accumulation | High | P2-flip does not merge. Profile the residual; do not "patch the posture" |
| R2 | Default-ON overlay regresses read cells via drain+zero-seed | High | P2-flip AND item 2 (OQ4). Not optional |
| R3 | OQ2-delete then a later B4 wants FUSE memfd assembly | Med | OQ2. Keep `=1` only if a **written** B4 design names it. IL `placed_sever` is independent |
...
| R6 | P4-A ships before 1 MiB IL matches | High | No P4 code PR until OQ3 + the row |
| R7 | `fuse3_zc_write_lazy_extractions` noisy on cold-start cache misses | Low | Tripwire is *steady-row* after warmup |
| R8 | ACK-early read of in-flight overlay serves old gap or blocks on a yield-spin | High | P2-flip AND item 3 (overlay §8 restatement). Drain wait is not serve-retained |
| R9 | Unconditional streaming-hold + stale-TRUE gate = 1 MiB HOLD+late-extract | High | KD-OP-16 pins before arming |
| R10 | Calling the A-leg number "B-initial" **or** requiring B5-with-B4 as P2-flip item 4 stalls default-ON on unbuilt overwrite overlays | High | A-leg ≠ B-initial. Item 4 is B2 fsync-storm. B5 stays the overlay program's close |

R1/R2/R5/R6/R8/R9 are **entry criteria** on P2-flip / P3 / P4, not a list at the back.

---

## 15. Open Questions

These are product calls. This design does **not** silently decide them. Structural scheduling (split P2, three-way P3, do not open P1/P4 as if they were decided) is already applied.

1. **Flip `SQUEEZEFS_DEVICE_OVERLAY` default ON now** (before the field A-leg number) **vs wait** for the P2-flip AND-gate?  
   *Design recommendation:* **wait** (KD-OP-4). B2's counted 0.77× is the last *counted* overlay row; P-minus-one has no field note. A flip now makes every default 1 MiB ingest slower than accumulation.

2. **Delete FUSE placed-merge code now vs keep behind `SQUEEZEFS_FUSE_PLACED_MERGE=1` as a counted instrument until a written B4 design names memfd assembly?**  
   *Design recommendation:* **delete the FUSE product path** if no written B4 names it; **keep IL `placed_sever`** regardless. Default-OFF FUSE code is dead code. Do not open P1 until this is decided.

3. **P4: kill the hybrid (all IL; 1 MiB IL must match kernel) vs declare kernel FUSE the large-write path and stop pretending IL is universal?**  
   *Design recommendation:* **no pick without the 1 MiB IL-vs-kernel row** on the post-P-minus-one (ideally post-P2-flip) binary. Lean **P4-B as the honest story until P4-A's number exists**: large seq is already kernel via the derived gate. Killing the gate before 1 MiB IL matches 35-vs-28 is R6. No P4 code PR until this is decided.

4. **Does P2-flip require overlay B3 (lock-free read compose) in the same flip, or is a written restriction that acceptance read cells only run after coverage-complete/fsync on full-block files enough?**  
   *Design recommendation:* **one of those two is required** (P2-flip AND item 2). Drain-on-read + zero-seed of the unwritten 3 MiB is not an acceptable default-mount read path for the seq-1 MiB write-then-read cell. Which of the two is a product/schedule call; "neither" is not on the menu.

5. **After P3, is `SQUEEZEFS_PATCH_MAX_BYTES=0` still a legitimate acceptance A/B, or does turning W1 off become unrepresentable on a default mount?**  
   *Design recommendation:* **keep as the W1 A/B** (`0` = acceptance lever, already the registry purpose). Turning W1 off must remain representable or the 2,500× RMW pin dies.
...
| Silently flip `DEVICE_OVERLAY`? | **No** — OQ1. |
| P3 deletes CoW in favor of inplace-or-B4? | **No** — three-way; B4 is not a one-path PR (Issue 1). |
| P2 A-leg number = overlay "B-initial"? | **No** — B-initial is PR B5. |
| P2-flip waits on B5-with-B4? | **No** — item 4 is B2 fsync-storm on the ACK-early binary (Rev 2). |
...
| Schedule P1/P4 as if OQ2/OQ3 were decided? | **No** (Issue 7). |
...
### PR P1 — after OQ2 only

Two independently reviewable shapes; pick one after OQ2:

**P1-delete (if OQ2 = delete FUSE path):**
...
- **Dependencies:** P0; **OQ2 = delete**.
- **Description:** No-dead-code on the FUSE 25 % path. Do not `cfg(test)` across the fuse3 workspace boundary. Do not touch IL 1-copy writes.

**P1-keep (if OQ2 = keep `=1`):**

- **PR title:** `docs(fuse3): FUSE placed-merge is a counted instrument, not a product path`
- **Files/components:** registry purpose class (3); mount-log when `=1`; test that default mount places 0 bytes.
- **Dependencies:** P0; **OQ2 = keep**.
- **Description:** Default-OFF stays unreachable as a posture. Code remains only if a written B4 design names memfd assembly.

### PR P2-flip — Overlay default ON after the AND-gate

- **PR title:** `perf(overlay): default ON after A-leg + read/fsync restatement`
- **Files/components:** `device_overlay_enabled` / `src/env_knobs.rs` default; `set_zc_hold_streaming(true)` on zc-armed sessions (pins already green); `try_overlay` no longer a knob check; optional `try_patch`/`try_overlay` reorder **with** `patch_ineligible_*` suite updates; field evidence note `.benchmarks/20YY-MM-DD-one-path-overlay-default.md`; `docs/operations.md`. Overlay B3 / §8 restatement / **B2** fsync-storm as **dependencies**, not files this PR invents. **Not a dependency:** overlay B5 / B4.
- **Dependencies:** P0; P2-prep; P-minus-one (already on `dev`); **AND-gate §5.4** (A-leg note + OQ4 work + §8 restatement + B2 fsync-storm green on the ACK-early binary). Does **not** merge without that note.
- **Description:** Allowed to flip only with the number **and** the read/fsync conjuncts. Not "B-initial." Not B5.
...
### PR P4 — after OQ3 only

**P4-measure (any time; not a product flip):**

- **PR title:** `docs(bench): 1 MiB IL vs kernel on post-P-minus-one binary`
- **Files/components:** a `.benchmarks/` note. Label whether overlay was on. If pre-P2-flip, say the write cell is accumulation vs kernel.
- **Dependencies:** none.
- **Description:** The row OQ3 needs. Not a code change.

**P4-A or P4-B (one line, after OQ3):**

- **P4-A title:** `perf(il): kill hybrid; 1 MiB IL matches kernel`
- **P4-B title:** `docs(il): kernel FUSE is the large path; IL is IOPS`
- **Files/components:** P4-A — `lane_gate` / `IL_KERNEL_LANE_MIN` delete after the match row. P4-B — charter §3 / scoreboard labeling / operations.md; gate stays.
- **Dependencies:** OQ3; P4-measure; P4-A also needs the match row ≥ 0.97× kernel on the relevant binary (post-P2-flip if we are claiming overlay).
- **Description:** One story. Not both.

**Program acceptance:** the §8 matrix, after the cells that depend on P2-flip / OQ3/P4 are measurable. Cited with instrument, substrate, same-day raw, and admission-ledger closure.

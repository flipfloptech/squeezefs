...
/// The PRODUCTION W1 inval hook (§5.6.2, PR L4-6): pushes
/// `FUSE_NOTIFY_INVAL_INODE` through the fuse3 [`Notify`] handle
/// captured post-mount into `cell` (pre-mount fires are skipped — no
/// kernel cache exists yet). Factored out of the mount path so the
    ...
    Arc::new(move |ino, scope| {
        if let Some(notify) = cell.load().as_ref() {
            ...
            let (off, len) = match scope {
                InvalScope::Whole => (0, -1),
                InvalScope::AttrsOnly => (-1, 0),
            };

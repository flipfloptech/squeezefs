...
- **The verbs (§5.3.5, §6.3).** `ManagerCall::{JoinAppender, ExtentGrant, ReturnExtents}` ride the S8 owner service as one verb family (`CLUSTER_WIRE_SCHEMA` 4 → 5 — a same-commit-fleet change, KD-7); every verb is idempotent against DURABLE state — a `JoinAppender` replayed against this manager OR a successor answers `Joined{already}` off the joiner's `Live` page, a return of extents the record no longer grants answers `already` — counted on `manager_verb_replays`; `manager_verb_refusals` (**must stay 0**) counts the durable witness contradicting the caller (appender 0 asked for a grant; a join past `appenders_capacity`, which refuses naming the metadata volume COUNT as the lever). `JoinAppender` reuses the lowest `Free` directory page or grows the chain by one extent (header written LAST, then the previous header's `next`, each barriered), carves a ring of ≤ 8 coalesced extents, writes the page `Live` into both directory slots (the directory-first law) and mints the initial grant. `manager_service_ns` is exact-sum `admit / execute / reply / total`; `manager_load_pct` is MEASURED Σ service ÷ wall; `manager_verbs_per_s` the rate.

...
| S11 | Byte-range custody + the W1 seventh clause | **LANDED; §9.5 rows RUN — gate closes CONDITIONALLY (two adjudicated standing-reds)** | The full ladder (rungs 15–18): the distributed range wire, the demotion barrier + authority assembler, and the §9.5 closing rows on **pinned ior 4.0.0** (`.benchmarks/2026-08-18-s11-mpiio-row.md`). Adjudicated live: the **block-cyclic row** (Issue-19: grants ≈ blocks, ZERO cap refusals, 1.52× its same-width disjoint control), the **tiny-ranges bounds row** (coalescing 4,096 byte-granular asks → 2 spans, foreign-client collateral 1.02×), the **demotion-barrier row + price table** (ledger closed, ≈ 3 ms authority CPU/extent — the priced exception, D1 holds), the **range kill matrix** (holder ×3 + authority-mid-assembly ×2, custody/era laws held, re-driven passes exact). The rung convicted and fixed **ten product bugs red-first** (fold-settle, ranged POSIX-5, desired-stretch v2, idle-reap reconnect, extent chunking, geometry-source size, divergence-retry classes, own-lease fencing at width, three assembler-coherence faces). **Standing-red (rung 19's, adjudicated in the legs' own gates)**: the width-N same-ino publish REFS composition (one swapped C8 pair per race) and the sub-block fourth dangling-take face — the MPI-IO throughput row is BLOCKED on the former at width 32 (engagement machinery proven; probe aggregates 0.7–2.0 GiB/s). `SQUEEZEFS_RANGE_CUSTODY` stays default-OFF (the third upholding); `patch_ineligible_range_shared` keeps the W1 ledger honest |

...
async fn make_fs(test_id: &str, block_size: &str) -> Harness {
    ...
    let routed = Arc::new(squeezefs::meta_backend::RoutedMetaBackend::new(vec![
        open_v3_meta(meta_temp.path(), 256 * 1024 * 1024).await,
    ]));
...
mod storm {
    ...
    impl Drop for Mount {
        fn drop(&mut self) {
            ...
            // Drop-time double-unmount: already-unmounted is the EXPECTED
            // case — silence the mtab noise; the explicit unmount path
            // stays loud.
            let _ = Command::new("fusermount3")
                .arg("-uz")
                .arg(&self.mnt)
    ...
    fn mount_fs(tag: &str) -> Mount {
        ...
        // Since PR K6b the CLI-formatted v3 volume serves FUSE read-write
        // directly (the K6a-era v2 re-format shim is gone): this suite's
        // storm + sideband traffic now exercises the FUSE-over-io_uring
        // transport AGAINST the v3 commit pipeline — the production shape.

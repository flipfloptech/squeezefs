...
| `xv_cross_owner_intents_{minted,retired,open}`, `xv_cross_owner_steps_{shipped,served}`, `xv_cross_owner_intents_stuck`, `xv_cross_owner_phase_ns` (`guard_rtt`, `plan`, `intent_barrier`, `ship_rtt`, `retire`, `total`), `xv_cross_owner_guard_rpcs`, `xv_cross_owner_guards_parked`, `xv_cross_owner_guard_expiries`, `dir_rename_lock_acquires`, `dir_rename_lock_wait_ns` | **The Cross-owner family** (design-symmetric-metadata §5.6 / §11, incompat bit 17 + `SQUEEZEFS_SYMMETRIC_META=1` — PR 6, dark; every one 0 on an unarmed or bit-17-absent mount, pinned). `intents_minted` = cross-owner intents this process came to know as open (minted here, or found open at a scan — an adopted intent counts at discovery), `intents_retired` = retired by this process (the live path, the mount's recovery, the roll-forward cadence), `intents_open` = the register's population — closure `minted ≡ retired + open`; `steps_shipped` = steps this initiator shipped to a foreign slot holder, `steps_served` = steps this holder served for a foreign initiator (the two ends of the wire; the S3.5 `crossvol_tx_steps_*` ledger counts a shipped step's outcome at the HOLDER); `intents_stuck` **must stay 0** — an open intent older than the grace window (the 45 s membership TTL) that no holder serves: the holder is down and its slots not yet recovered (PR 10's remedy), or its endpoint never bound; `xv_cross_owner_phase_ns` = exact-sum `guard_rtt` (Σ `xv_guards` round trips — taken in the arm before the transaction's `total` starts) / `plan` (guards → `tx0` committed) / `intent_barrier` / `ship_rtt` (Σ shipped round trips) / `retire` / `total`; `guard_rpcs` = `xv_guards` verbs this initiator shipped (one per foreign holder table per op — the design's "foreign-home guards travel: `dlm_rpcs += 1` each" face; a same-process holder costs none), `guards_parked` = guard scopes this holder parked for remote initiators, `guard_expiries` **must stay 0** — scopes the lease-expiry sweep released because their initiator died without releasing (the 45 s grace window); `dir_rename_lock_acquires` / `dir_rename_lock_wait_ns` = set-wide directory-rename lease takes and the wait for one (a file rename never takes it; a same-slot directory rename does). See [cross-owner transactions](#symmetric-metadata-program--cross-owner-transactions-pr-6-dark). |

...
// ---------------------------------------------------------------------------
// DLM S9 / rung-10 finding #5 — the block plane under an ENGAGED partition
// ---------------------------------------------------------------------------

/// **Rung-10 finding #5** (found live by the s9-fanout row's trailing
/// fsck: 512 C2-lost + 2 C6-drift VERIFIED findings on a HEALTHY
/// multi-writer fleet — every one a live co-writer's minted block): the
/// authority's allocator census covers only the lanes it OWNS, so a peer
/// writer's blocks are durably referenced and structurally untracked HERE
/// — and the C2/C6 zero-FP ladder had no lane exemption, so they verified
/// as "lost" (untracked across both scan epochs, exactly what a foreign
/// block looks like forever).
///
/// The law: under an ENGAGED allocation partition, a referenced offset in
/// a lane this mount does not own is a PEER's to adjudicate — C2's lost
/// arm exempts it (counted, `fsck_foreign_lane_exempted`), C6's capacity
/// census DECLINES entirely (its used-vs-tracked arithmetic is
/// single-writer by construction), the cross-writer oracle stays C8 (the
/// durable ledger, which runs and gates), and the lane-aligned
/// fleet-parallel fsck is rung 10c's. The class stays LIVE for the
/// mount's OWN lanes — an own-lane phantom still reports.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_c2_and_c6_decline_foreign_lanes_under_an_engaged_partition() {
    let _serial = serial().await;
    let dir = tempfile::tempdir().unwrap();
    let meta = make_file(dir.path(), "meta", 256 * 1024 * 1024);
    let oss1 = make_file(dir.path(), "oss1", 4 << 30);
    ...
    // Engage a 4-way partition on the data allocator: this mount is lane 0.
    for alloc in fx.fs.router.backend_router.lane_allocators() {
        alloc
            .engage_alloc_lanes(
                squeezefs::meta_backend::kv::journal::AppendPartition::new(4, 0)
                    .expect("partition"),
            )
            .expect("the partition engages");
    }

    // Two phantom references, both untracked and in-capacity: one in a
    ...
    assert!(
        !report
            .findings
            .iter()
            .any(|f| f.object.contains(&foreign_off.to_string())),
        "a FOREIGN-lane referenced offset is a peer's to adjudicate — never a finding here: {:?}",
        report.findings
    );
    assert!(
        !report.findings.iter().any(|f| f.class == "C6"),
        "C6's single-writer capacity arithmetic must DECLINE under an engaged partition: {:?}",
        report.findings
    );
    assert!(
        report.counters.foreign_lane_exempted >= 1,
        "the exemption is counted (fsck_foreign_lane_exempted engagement)"
    );
    fx.close().await;
}

...
pub static KNOBS: &[Knob] = &[
    ...
    // -- Copy economy ----------------------------------------------------
    k("SQUEEZEFS_WRITE_SHARED", Kind::Bool, "off", "Shared-mode write admission (design-write-inode-convoy): fully-mapped within-EOF striped overwrites take the inode READ guard (classify → acquire-as-classified → revalidate → at most one upgrade). Default OFF pending gate 8.3c (2026-08-11 bracket: kern wash, il loss — the guard convoy fell, the wall was co-limited by the transport pass funnel); `=1` is the measurement A/B lever."),

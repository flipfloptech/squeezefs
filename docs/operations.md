...
> **ℹ️ Staged-layout files and multi-client visibility.** On a filesystem WITH staging dirs, a file whose size lands in `(4 KiB, block_size]` takes the **staged layout**: its whole payload is one entry in the writing host's local staging ring, its metadata on the shared volume says "the bytes live in ring entry `<file_id>`", and the shared data backend holds **nothing** for it until the entry is **promoted** (one backend block + one metadata commit). `fsync(2)` makes such a file durable **on that ring** (it syncs the shard and commits the layout; it does not promote). Promotion happens at two points: **the writing mount's clean unmount** — the dismount teardown promotes every resident staged-layout file before its census, so a clean unmount is a durability boundary for this class exactly as for active blocks and overlays (`.stats` `dismount_promoted_{files,bytes}` / `dismount_promote_failures`; the log line `dismount promoted N staged-layout file(s)`) — and, while the mount is up, **staging-pool pressure** (the merge worker drains the ring past its 75 % high-water mark). Consequences: **while the writing mount is up**, every other client of the volume set — another mount point, another host, read-only mounts, co-writers — reads an un-promoted staged-layout file as size-consistent **zeros**, silently (no error; the reader's `staged_payload_lost_reads` counts it and its log names the file); after that mount's clean unmount every client reads it from the shared backend. A mount that dies without its teardown (crash, `kill -9`) leaves the population in its staging root: the same host remounting at the same mount point recovers it byte-exact (a moved mount point is reported as `MOVED-MOUNT-POINT STAGING RESIDUE` with its adopt/discard remedies), and other clients read zeros until that remount's clean unmount promotes it. The dismount census reports anything the promotion could not move at INFO as exactly that (`staged-layout files resident in local staging at dismount: N`, with the failed/skipped split); the WARN is reserved for unflushed **active write blocks**, the class an unmount wait drains. `squeezefs umount` offers its "[w] Wait" only for that second class. Evidence: `.benchmarks/2026-09-09-dismount-staged-residue.md`.

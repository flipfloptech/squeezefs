...
**Scope, stated plainly.** Metadata authority is per metadata volume: at any moment each metadata volume has exactly one *owner* mount that commits changes to it — one owner for the whole set by default, or up to 16 owners with the offline `squeezefs volume set-owners` split. Any number of **read-only mounts** (`-o ro`) see the owners' changes within a published, bounded delay. Any number of **co-writer mounts** (opt-in: `SQUEEZEFS_MULTI_WRITER=1` on NVMe-oF storage with persistent reservations) write file data directly to the shared devices and send each metadata change to the owning mount to commit — built for throughput-shaped writing, not latency-sensitive metadata-heavy work. A second plain writer of an owned volume is refused, naming the holder. All of this is proven on real mounts at lab scale (up to 32 co-located mounts; a 4-node AWS fleet over nvme-tcp); the 15,000-node design target rests on simulation and arithmetic, never a measurement — every scale claim carries its tier in [docs/rc-manifest.md](docs/rc-manifest.md).

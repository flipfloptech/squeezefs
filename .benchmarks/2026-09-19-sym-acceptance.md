...
**Fix round 1 (review `2bcc0e21`: OPEN 9, BUGS 2 — every issue closed one commit each, RED pin first for every product change; `3bab6850` … `488469e2`, eleven commits):** Issue 1 `3bab6850` (F-C4 at the joiner's page writer — `write_appender_pages` takes `grant` before `page`; the joiner pin `sym_n_daemon_tests::a_joiners_stats_reader_never_deadlocks_against_its_own_checkpoint_page_writer` RED 3/3 at its bound → GREEN in 1–2 s; the static rail `tests/appender_lock_order_tests.rs`, RED on the unfixed source naming `backend.rs:11594 → :11625`; the matrix grows to 44 suites), Issue 2 `ebeddb30` (the two-host pin holds the guest's bdev OPEN across its listings — the base RED **8 → 8 vs 12**, the reader's stale cache; the earlier ×5 re-attributed to the writer's write-behind), Issue 3 `803a21db` (§4.4av — the deferred head alignment; `meta_io_direct_tests::a_ring_recovered_within_max_pad_of_full_opens_through_the_guarded_cycles` RED `JournalReserveExhausted { needed: 25 }`), Issue 4 `973ee3b7` (the reverse-compat statement), Issue 5 `ad11d555` (the constants tied; `admit_meta_io_grain` refuses a grain > 4096 — `a_grain_the_page_arithmetic_cannot_honour_refuses_the_registration`), Issue 6 `633ca918` (the window arithmetic on the two constants; §7 (f)), Issue 7 `7d35284f` (the testkit ledger — under the buffered seam + `REQUIRE_CAPABILITY` six contracts FAIL `[REQUIRED-CAPABILITY]`; the torn-PAD pin `a_pad_whose_checksum_fails_is_a_torn_entry_the_scan_resyncs_past`; the reserve's `debug_assert`), Issue 8 `33d47369` (`meta_io_mode` = one lock-free load + one probe, `the_posture_lookup_resolves_a_path_once_and_never_on_a_hit`; bench `meta_io_mode` hit 36 ns / remembered miss 40 ns, dev-box scoping), Issue 9 `8a74321d` (the F-C4 pin's watchdog `max(60 s, pilot × 300 × 8)`; `joined::classify_join_reply` — `sym_n_daemon_tests::a_join_the_manager_defers_is_the_retryable_class_never_busy`; the 4,092 B boundary-crossing pad case). **The round's fleet legs found F-C2's SECOND FACE (§4.4as, `ca7fbb80`):** `sym-storm` round 2's rejoin refused `appender page cannot name 11 grant runs (max 4)` — the reply's word copied into the RAM page verbatim; the page names `page_runs_of(word)`, pin `a_joined_reply_word_wider_than_the_page_lands_whole_in_ram_and_largest_on_the_page` (seam `TEST_JOIN_FRAGMENT_REPLY_GRANT`, RED `cannot name 72 grant runs`). **Rails on the final tree (`488469e2`; product code = `ca7fbb80`):** fmt, both clippys, rustdoc `-D warnings`, `task check:loom` + the four `journal_core` loom models RUN against the rebuilt core, the markdown link check GREEN; **the 44-suite matrix flat THEN stamped GREEN from zero (run 4 of 4 on the round's tree — run 1 aborted as a mixed tree when `ca7fbb80` landed mid-run; runs 2 and 3 on `ca7fbb80` each RED at ONE pin, PR 13g's `sym_n_daemon_tests::a_joiners_extent_supply_under_a_create_storm_grows_its_ring_and_recycles_its_grant`, once under the round's concurrent fleet + qemu load and once on the idle box after thirty suites' compile bursts (returns +95 / +112 over 927 / 972 compactions against its 10× law), the same code GREEN 3/3 standalone, 2/2 in-suite, 1/1 in-suite under `--all-features`: this box sheds clock at Tctl ≈ 93–100 °C with `scaling_max_freq` untouched (a 40 s all-core burst: 3.9 → 3.0 GHz, `host_clock_throttled` blind), so `488469e2` gives the testkit the package-temperature word (`host_package_temp_millic` / `host_thermally_throttled`, hwmon Tctl) and the pin judges its two RATE laws only on an unthrottled host — venue-attributed otherwise, the owner's ruling — its structural laws unchanged; run 4 on that tree GREEN; **review round 2, Issue 10:** the non-verdict is a LEDGERED partial skip since `SkipClass::Venue` (`##SQUEEZEFS-SKIP##` on the uncaptured handle with the Tctl reading and every unjudged reading in its reason; promoted to a failure by `SQUEEZEFS_TEST_REQUIRE_VENUE=1` on the box), and the throttle point is `temp1_crit` ONLY — never k10temp's fixed 70 °C `temp1_max`; a sensor exposing no `crit` (this box's) is NO word and the laws judge)**; on `ca7fbb80`'s release binary from zero: **`sym-storm` GREEN 2/2** (ACKED 9,046 / 15,314, LOST 0, both rejoins landed), **`sym-crash` GREEN 2/2**, **`sym-two-host` GREEN** (9 → 12 ≡ 12/12, phase 2, census `{}`, zero residue) and the pre-F-C1 base RED at phase 1 (8 → 8 vs 12) — every leg torn down to zero residue.
...
**Found on the cloud row (§3.10):** every joiner `granted 72 = claimed 72, unclaimed 0` from its join — write-dead — with the manager replaying each ask verbatim (`ExtentGrant`'s idempotency answered the page's word). The manager's own doc on `write_wire_joiner_page_grant` states the contract: the grant's runs reach the joiner ON THE REPLY; the page write is the durable home for the joiner's own restart. `open_joined_appender` (`backend/joined.rs`) ignored `ManagerReply::Joined { grant }` and ran `RegionGrant::recover(record, &page.grant)` off the device — honest only while the page read is (F-C1). **Fix:** `JoinedOpen.grant` carries the reply's runs from `wire_join_appender` into `open_appender_regions`, which recovers `record ∩ the reply's word`; the RAM page carries the reply's word; `unclaimed_remainder_of`'s page read for the §5.3.5 replay is honest under F-C1's fix and its doc states the dependency. **Pin (RED on the base with the cloud row's numbers — `grant_unclaimed: 0, grant_claimed: 72`):** `sym_n_daemon_tests::a_joiner_honours_the_joined_replys_grant_word_not_its_page_read` with the seam `TEST_JOIN_STALE_PAGE_READ` (a joined region open reads its page's PRE-GRANT image). **Fix round 1 — the fix's SECOND FACE, found by the round's `sym-storm` round 2 (a rejoin after the manager's recovery of the killed joiner):** the first F-C2 build copied the reply's word into the RAM page's `grant` VERBATIM, and a fresh join's carve is a POOL of `want` extents in as many runs as a churned heap's free bitmap gives (11 on the fleet), so the joiner's first page write (`join_joined_region`) refused `appender page cannot name 11 grant runs (max 4)` and the rejoin FAILED — the mount refused. The page names `page_runs_of(word)` — the largest `GRANT_RUNS_MAX` runs, the manager's own law for the page it writes for the joiner (PR 13g's pool) — and the RAM grant takes the whole word. Pin `sym_n_daemon_tests::a_joined_reply_word_wider_than_the_page_lands_whole_in_ram_and_largest_on_the_page` (the seam `TEST_JOIN_FRAGMENT_REPLY_GRANT` fragments the reply's word into one-extent runs; RED with `cannot name 72 grant runs (max 4)` on the first build, GREEN fixed: the join lands, the RAM grant's extent set is the record, the page names ≤ 4 runs inside it, the first create mints off the pool with no wire refill).
...
**Fix (`appender.rs`):** ONE order — `grant` before `page` — is the region's lock law (stated on `AppenderRegion`'s doc; neither guard is ever held across an await); `AppenderSet::stats` follows the writers. **Fix round 1, Issue 1 (the reviewer's find — the law was violated by a THIRD site):** the checkpoint task's own page writer `write_appender_pages` took `page` THEN `grant` for every region ≠ 0 — a joined writer's own region on every cycle, a declared-region manager's too — so F-C4's deadlock stood at the joiner against its `.stats` poll and its commit path's refill (`name_remainder_on_page`, grant → page); the remainder is now read under the grant guard BEFORE the page lock; pin `sym_n_daemon_tests::a_joiners_stats_reader_never_deadlocks_against_its_own_checkpoint_page_writer` (a joiner's 200 create + cycle rounds against a tight `appender_stats` poll; RED 3/3 at the 100 s bound unfixed, GREEN fixed); and a STATIC rail `tests/appender_lock_order_tests.rs` walks every file that touches both mutexes and refuses a `page` guard whose block acquires `grant` (RED on the unfixed source naming `backend.rs:11594 → :11625`, GREEN fixed, its own teeth pinned) — a fourth site cannot rot the law before a schedule finds it. **Pin:** `sym_appender_tests::the_stats_reader_never_deadlocks_against_a_grants_page_update` — a std thread polls `appender_stats` in a tight loop while the manager carves and returns a one-extent grant 300× (the answered runs returned before every ask so §5.3.5's idempotency never answers verbatim); RED 3/3 on the unfixed order within its 60 s watchdog (the verdict on the UNCAPTURED stderr + `exit(101)` — a panic would hang the runtime's drop on the worker parked in `Mutex::lock`; the driver is SPAWNED so the watchdog is polled by a thread that is not it), GREEN 5/5 fixed. **The growth contract itself was re-read** (its premises, not its law): growth is judged as a DELTA across the in-flight window (the storm's kick cycles may legally grow the ring before the doomed write), the doomed write reserves under a test-held SMO mutex (`test_try_hold_smo` — a maintenance-pass SMO racing it for the ring's head took the armed sector once in ~25 runs: the SMO's write was the one the shim held, under the mutex the cycle needs, a second hang shape), and the closing growth is driven by a second storm's own stall; 100/100 alone after the rewrite.

**Fix round 1, Issue 3 (the reviewer's find — a mount-refusal face the pad law reintroduced):** `align_head_for_writing` ran at `open_inner` right after replay and needed `try_admit(0, Checkpoint)` = `max_pad` (4,117 B on a 4 KiB grain) of TOTAL ring space; a ring recovered within `max_pad` of 100 % full — the checkpoint class consumed its whole reserve before the crash, the wedged-tail escalation's shape; a ring the pre-13i binary wrote unpadded, so its head stands mid-sector — answered `JournalReserveExhausted` and the open FAILED, every later open identically (nothing drains a ring no writer can open — the face `preclaim_ring_recovery` exists to prevent, P2 2026-07-26 §9, which the pre-13i binary recovered from because its first pre-claim cycle needs no ring write). Fix (`backend.rs`, `backend/joined.rs`): the open DEFERS a refused alignment (`align_ring_head_or_defer`, a WARN naming the numbers — safe by the arithmetic: the same fullness admits nothing else, every admission claiming its entry plus the pad slack, so the pad stays the ring's first write and a write reaching a mid-sector head is `aligned_ops`' loud `Corrupt`) and lands it as the FIRST step of `preclaim_ring_recovery` (`align_own_rings_for_writing` — try every own ring's pad; while one is refused, `checkpoint_cycle(&mut smo, true)` under the SMO mutex and retry, bounded by `COVER_CYCLES_MAX`; the joined appender runs the same helper for its own ring before its checkpoint task spawns). Pin `meta_io_direct_tests::a_ring_recovered_within_max_pad_of_full_opens_through_the_guarded_cycles` — a 512 KiB ring filled UNPADDED (an unregistered path, the pre-13i bytes) in the checkpoint class to its last bytes (four xattr names re-put round-robin so the replayed dirt folds to four records and the guarded flush needs no SMO record), the head mid-sector; the writer open must succeed with every planted entry replayed, the pad written, the head aligned, a user commit landing and surviving a remount — RED on the first build with the reviewer's exact class (`JournalReserveExhausted { needed: 25 }`), GREEN fixed on both matrix legs.
...
### 4.4ax PR 14 — FIXED (PR 5's frame fence × the D0 gate's ring-recovery preflight; every forest volume — the DEFAULT since the flip; an acked-writes LOSS on the death path): a writer whose crash left a replay window that exhausts the ring flushed the replayed records under the structural `(0, 0)` stamp INSIDE its claim gate — rule 3 screened them at the next open

**Found by the inverted matrix's STAMPED leg (`crash_kill_tests::test_kill9_remount_soak_v3` — the v3 kill-9 soak, unchanged since Rollout 4, run for the first time against a volume that is the armed forest by default): the replay-twice digest law `d1 == d2` read RED in ~1 of 5 rounds — records PRESENT on the recovered mount m1 (every acked name resolved, the D0 contract green) were ABSENT after m1's clean `shutdown` on the next mount m2.** The tape (the ZZ instrumentation the previous agent left mid-attribution, removed before the commit): the LOST set was the dead child's LAST acked creates (`f65`/`f66`, inos 67/68 — their inode records, dentries and `user.crash` xattrs, plus the root's times), `META_KV_FOREIGN_FRAMES_SCREENED` +1 over m2's open, and with `RUST_LOG=warn` the two lines that attribute it: `meta volume …: recovered replay window exhausts the journal ring (head=741104, reusable_upto=4072) — running pre-claim recovery checkpoint cycles` at m1's open, then at m2's open `node 0x140000: bset frame at offset 36864 (appender 0, slot generation 0) screened FOREIGN by rule 3 (…the lessee's current generation is 0) — the log ends before it`. Every failing round had a replay window ≥ ~170 entries on the soak's 1 MiB ring (177 / 180; the passing rounds 18–138) — under PR 13i's pad law a serial commit costs a ring page, so the soak's window reaches the preflight's threshold (`logical_len − reserve − max_pad − MAX_ENTRY_LEN`) about one round in five.

**The defect (PR 5's `prime_frame_stamps` law with one site left outside it):** the frame screen (§5.8.2) needs every frame a writer appends to a slot-tree leaf to carry the slot's lease generation, and PR 5 round 4 primed the fence + tree 0's lease table at `writer_bring_up`, BEFORE the bring-up cover — the open's first frame writer as the round knew it. But a crash whose window exhausts the ring's user slice runs `preclaim_ring_recovery` INSIDE `writer_guard_gate` — before `writer_bring_up` — and its bounded `checkpoint_cycle`s (and `align_own_rings_for_writing`'s) flush the replayed dirt into the leaves with NO fence installed: `NodeCache::frame_stamp_for` answers `FrameStamp::default()` = `(0, 0)`, below the leaf's earlier frames' `g ≥ 1` (the dead writer's, at its lease generation). The recovering mount served every record from RAM (its own replay), its clean leave recorded tails past that frame and nothing wrong; the NEXT load — a writer's pre-arm load runs rule 3 alone (no plane answers yet), the armed screen every rule — ends the log at the `(0, 0)` frame: the dead writer's acked records flushed by the recovery, and everything the recovering mount appended after them, gone. Silent to fsck (the probe screens the same way), invisible until a read of the lost names. Reachable on EVERY forest volume since PR 1 by a kill-9 with a near-full ring followed by a remount — the shape the flat soak has run for months, which is why the flip's matrix found it: the default volume is a forest now.

**The fix (`backend.rs`, `preclaim_ring_recovery`):** the stamps are primed at the writer's FIRST frame writer — `prime_frame_stamps()` runs before the preflight's guarded cycles and before the deferred alignment's (`prime_frame_stamps`'s doc states the two sites; every caller idempotent: `load_slot_leases` re-loads, the fence install is the refused second one; the bring-up's prime stays for the offline verbs whose opens never run the gate). The stamp the pre-claim frames carry is the arm's: a re-adopted `Leased { 0, g }` stays `g`, an `Unleased { g }` native slot is `g + 1` (the acquire's law) — monotone with the dead writer's frames and with everything after. **Pin (RED-first):** `sym_coherence_tests::a_writer_recovering_an_exhausted_ring_inside_its_gate_stamps_the_leases_generation` — an armed writer commits acked creates with the cadence parked until the gate's own admission probe (`try_admit(MAX_ENTRY_LEN.min(user slice − max_pad), User)`) refuses, dies (the in-process drop); the successor recovers the ring inside its gate, resolves every acked name, leaves cleanly; the third open resolves every acked name and `META_KV_FOREIGN_FRAMES_SCREENED` is unmoved — RED on the unfixed tree at `acked-135 lost after the successor's clean leave`, GREEN fixed. **The soak's premise beside it:** a clean leave of an ARMED mount leaves ONE control entry in the window (the region's `Unleased` batch, §5.1.3 — pinned since Slice A on `sym_forest_tests` / `sym_convert_tests`), so `crash_kill_tests`' "empty replay window" assertion reads `clean_leave_entries(&m2)` (1 on a forest, 0 on `--single-writer`). **From zero on the fix:** `crash_kill_tests` GREEN ×5 on the stamped leg (100 rounds of each soak) and once on the flat leg; the whole `sym_coherence_tests` GREEN. Every layout the fix touches is the forest's — a flat volume has no fence and stamps nothing (byte-identical).

### 4.4ay PR 14 — FIXED (PR 8's allocation lease × the allocator's `statfs` arithmetic; every armed writer — the DEFAULT since the flip): `df`'s used count only ever GREW — a terminal free never reduced it

**Found by the require-mount gate on the flipped default (`statfs_tests::test_statfs_free_tracks_write_then_delete_reclaim`: `free must recover after delete+reclaim: before=8 GiB after_del=7.39 GiB`; `pack_compaction_tests`' live-mount contract: `used_chunks` 50 where 2 anchors + 1 pack were expected — the 24 legacy blocks the compaction FREED still counted, 24 new packs beside them).** `BlockAllocator::get_used_blocks` — `statfs`'s `used`, the placement table's fill ratio, the drain preflight's `avail`, the D1 census frontier — was the `--single-writer` arithmetic `highest_block − free_list.len()`. On a grant-armed allocator the bitmap IS the free list (PR 8: `finish_free` clears the block's bit at the holder and `publish_free_list` never inserts into the local list; a freed block returns only through a carve), so the local list stays EMPTY for the mount's life and `used` never fell: every default mount's `df` reported deletes as never reclaimed, the placement penalty read a fill that only rose, a drain's capacity preflight under-counted `avail`. Reachable on every armed writer since PR 8; the flat suites never saw it because the flat class has no grants, and the armed suites read `free_supply_blocks` (the funnel's own term, correct) rather than `statfs`'s.

**The fix (`block_allocator.rs`):** on a grant-armed allocator whose allocation lease THIS process holds, `used` = the holding's bitmap population (every referenced block ∪ every open grant — the other writers' windows counted as RESERVED, honestly) minus this writer's own unconsumed window; the unarmed allocator keeps the shipped arithmetic byte-identically. **A grant-armed NON-holder (a joined writer) keeps the arithmetic** — its truth is at the holder and no word it reads carries the population yet (PR 14b: the holder's population on the grant reply / the renewal carriage — a joiner's `df`). **Pin (RED-first):** `sym_block_grant_tests::an_armed_holders_used_count_follows_its_bitmap_not_the_single_writer_arithmetic` (a fresh armed volume reads 0 used; the whole volume minted reads `DATA_BLOCKS`; 100 terminal frees read `DATA_BLOCKS − 100` with `used + free_supply ≡ DATA_BLOCKS` — RED at `4096 vs 3996`); the two mount contracts GREEN on the fix.

### 4.4az PR 14 — the mount-class contracts' presumptions the flipped default moved (test-shape, no product change)

Beside §4.4ay the require-mount gate read four more mount suites RED, each a flat-class presumption made seam-blind: **`pack_compaction_tests` / `pack_tenant_ops_tests`** — "N tenants → ONE pack" presumed the flat class's one pack scope per data volume; under PR 7's law 1 a pack is scoped per `(writer, slot, data volume)` and the rotor spread root-level tenants over N slots (6 tenants → 6 packs; 24 legacy blocks compacted into 24 packs); the tenants live in a GATHERED directory now (`setfattr -n user.squeezefs.gather -v 1` — KD-SYM-17, the operator's lever for "these files share one pack"; a flat volume accepts the name and packs as before). **`corpse_sweep_tests`** — "the live file re-minted the corpse's freed offsets" (`alloc_from_freelist ≥ 1`) presumed the free list serving before any fresh mint; a grant-armed allocator mints from its window and re-carves a hole below the grant frontier only once fresh space is exhausted (`BlockGrantLedger::carve`), so the premise reads posture-aware (`alloc_from_freelist == 0` on an armed mount) and the law under test — the stale releases skipped BEFORE the free funnel, the live file intact — is the same either way. **`format_guard_tests`** — "the whole journal extent is zero after the force reformat" presumed the flat ring; on the forest the extent's first four pages are appender 0's page slots and the format writes its `Free` page there — the contract reads the RING through `KvMetaBackend::fixed_ring_extent` (the one place the mount reads the geometry). **`sym_convert_fuse_tests`** — the live-FUSE conversion leg formats its source `--single-writer` (the ONE flat writable class since the flip) and `enable-symmetric` refuses it naming `enable-multi-writer` first (the forest presumes the nine multi-writer bits); the leg runs the operator's two verbs in order. **`meta_slot_migration_tests`** (the matrix's stamped leg, `eb774215`) — "a slot-1 directory exists among `dir0..dir3`" presumed the flat class's rotation over each volume's hosted slots; on the armed forest a child of `/` mints into its parent's slot under the affinity cap or the volume's ROTOR, and a two-volume width-4 set's slot 1 is volume 1's NATIVE slot, which no rotor names — `populate` presets `dir{d}` into routing slot `d % W` (`seed_dir_in_routing_slot` via `allocate_local_ino_in_slot`, so a preset never collides with a policy mint). Every re-shape is seam-blind: green on both legs.

### 4.4ba PR 14 — FIXED (a REGRESSION of this rung's own §4.4aw census bound, `95881877`): the manager's online census bounded a JOINER's slot at the grant-time cursor word and skipped every ino the joiner minted — false C2 "leaked" on live files after a manager failover

**Found by the `sym-crash` leg on the flip binary's fleet: round 1 GREEN, round 2's online fsck at the successor read false C2 `leaked block: allocated (tracked) with zero referencers` — 10 on a 7-writer fleet (with and without a 512-create burst added to the leg), 4 on the recorded 3-writer shape — reproducible at the idle successor on every later run.** A throwaway probe over the two meta volumes (`open_probe` + `block_ref_scan`) named every one a LIVE file's block: durable references present (C8 drift 0, `data_alloc_bitmap_drift` 0 — no hazard), owners `0x1030000000103` / `0x2030000000103` / … — regular files, `nlink 1`, size 5,242,880 = the joiners' `after-failover-r*/striped.bin` (2 blocks each) in the joiners' rotor slots; the census had `inodes_scanned` 69, then **5**, against 1,863 before the failover.

**The defect is THIS RUNG's:** §4.4aw (`95881877`) bounded the online census at the per-slot ino watermarks read at its start (`KvMetaBackend::slot_ino_watermarks`, the population AS OF ITS START), and the first build folded EVERY word the manager holds into that map — the lease table's entries for slots OTHER appenders lease, tree 0's `Leased` snapshot words, and a guest cursor left from before a grant. At the manager every such word is the GRANT-time cursor (KD-SYM-3: a leased slot's cursor travels with its lessee; the manager's table never sees a mint), so every ino a joiner minted since read "past the watermark" and the census SKIPPED the joiner's files: their blocks had zero referencers, 679–969 C2 suspects rose per census, the zero-FP ladder cleared most (the epoch/registry arms) and let a few through. Round 1 read clean by timing (the successor's grant-time words were fresh enough for the joiners' first files). The pre-14 tree had no bound and walked every projection whole.

**The fix (`backend.rs`, `slot_ino_watermarks`):** the map names only the keyspaces whose LIVE cursor this mount holds — the native keyspace, its guest cursors, the slots THIS appender leases (the table's own entries), and tree 0's UNLEASED words (nobody mints into an unleased slot, so the recorded cursor is exact); a slot another appender leases is EXCLUDED from every source (the foreign set is computed first and `raise` refuses it), so the census walks it whole — the function's own doc's law ("a slot with no cursor here is absent — walked whole"), now true. **Pin (RED-first, `Some(3)` — the grant-time word):** `sym_n_daemon_tests::the_managers_census_watermarks_never_bound_a_slot_a_joiner_leases` (a joiner acquires `SLOT_A` first-touch and mints 24 inos; the manager's map has no entry for `SLOT_A`, the lessee's names it above every minted local, the manager's native keyspace stays bounded); §4.4aw's own pin (`fsck_c12_tests::an_online_census_is_bounded_at_the_ino_watermarks_it_started_with`) stays green. **The leg**: the re-dial law reads conditional on a verb (a `mkdir` + a file need no manager verb — the dentry ships on the S8 lane, the mints land in leased slots; a joiner that asked the successor anything reached it through a re-dialed wire, `joined_wire_failures` 0 either way). The `sym-crash` count restarts from zero on the fixed binary.

### 4.4bb PR 14 — FIXED (the S8 lane's writer-era gate × PR 6's shipped steps; every armed writer — the DEFAULT since the flip): a joiner's FIRST shipped step after the SECOND manager failover failed `EINVAL` — the lane presented the dead manager's era, the successor refused the frame whole, and the step surfaced the refusal instead of retrying it

**Found by the `sym-crash` leg on `dc8fa6f9` (3 writers, round 1 GREEN): round 2's post-failover `mkdir after-failover-r2-m60` under `/` failed `Invalid argument` at every joiner, once.** The joiner's log: `S8: owner …:45999 refused the batch — it named a stale writer era; the owner is now in era 4`; its `meta_ship.era_relearns` 1, the successor's `stale_term_refusals` 1. The shape: a cross-owner create under `/` (the manager's native slot) ships its dentry step over the S8 lane to the manager's endpoint; the lane presents the writer ERA it learned from the previous manager's replies (`ShipLane::learn_term`); the fleet's manager address is the ONE stable port, so the successor is the SAME lane; the successor bumped its durable term before arming and the owner's mutating-frame gate refused the frame WHOLE (`STATUS_STALE_TERM` — nothing applied, "so the caller's retry is admissible" in the router's own words) and the lane learned the new era from the refusal — but `ShipLane::interpret` typed that refusal `InvalidOperation` (EINVAL) and the cross-owner ladder, which retries only the typed `SlotMoved` / `StaleHolderView` classes, surfaced it as the op's error. **The FIRST failover never shows it**: a lane that has shipped nothing presents `0` ("unknown, tell me"), which the gate admits — so every round-1 leg, the in-process failover pins and the `sym-crash` counts before this rung ran with one relearn and no refusal; the second failover is the first time a lane holds a LEARNED era that is stale. Pre-flip this shape was a `SQUEEZEFS_MULTI_WRITER=1` co-writer's alone (the retired posture); since the flip it is every joiner's second failover.

**The fix (`error.rs`, `meta_ship/router.rs`, `crossvol_tx.rs`):** the stale-era refusal is the typed retryable class `RefusalClass::StaleOwnerEra` (wire word 6, EAGAIN — PR 13's `SqueezefsError::Retryable { class }` law: no text decides a retry), and every cross-owner ship site (the travelling guards' acquire, `XvRelease`, `LookupExact`, the shipped step, the record-level `ship_meta_call`) rides ONE door, `crossvol_tx::ship_ops_relearning_era`: a ship refused for a stale era is resent ONCE with the same request ids on the era the lane just learned (the owner's `(client_epoch, request_id)` dedup window makes a resend of a frame that was refused whole safe by construction — nothing of it was applied), counted on `xv_cross_owner_step_stale_era_retries`; a second refusal is the retryable class the application sees. **Pin (RED-first with the fleet's exact text):** `sym_n_daemon_tests::a_joiners_first_shipped_step_after_a_failover_retries_the_stale_era_once` — a manager + a joiner whose first cross-owner create under `/` teaches its lane era T; the manager leaves with the joiner's shipper (its lanes, its learned era) OUTLIVING it (`HoldersVenue::tear_down_keep_shipper`) and the successor stands up at the predecessor's ADDRESS (`HoldersVenue::stand_up_at`, the fleet's one manager port) in era T + 1; the joiner's next create lands with `era_relearns` +1 and `step_stale_era_retries` +1, the successor names both dentries exactly (`lookup_dentry` — the records are the joiner's slot's), the third create moves neither gauge. Two premises the pin found on its way: a successor at a FRESH port is a fresh lane at term 0 (no refusal — the reason the in-process failover pins never saw it), and `HoldersVenue::tear_down` uninstalls the process's cross-owner shipper (fresh lanes again). `posix_errno_tests` carries the class's wire round trip. The `sym-crash` count restarts from zero on the fixed binary.

### 4.4bc PR 14 — FIXED (PR 9's scoped custody fence × PR 12b's per-holder token planes; every armed writer — the DEFAULT since the flip): after a manager failover at its own address, every joiner paced its refused custody renewals to `T_self` and then stopped its token planes DEAD under a LIVE successor — the successor's next commit on `/` waited the whole recall deadline and fired `dlm_token_recall_timeout_live`

**Found by the `sym-crash` leg on `47431963` (3 writers, from zero): rounds 1–2 GREEN, round 3 `invariant_tripwires=1` on the successor.** The successor's log carries two `INVARIANT TRIPWIRE 'dlm_token_recall_timeout_live'` lines — the round-2 successor at 16:45:48 (joiners m62 + m61, after its own checks had passed) and the round-3 successor at 16:46:34 (m60): "reader … holds a LIVE membership lease and did not ack the recall of object 1 inside 15s". The joiners' logs explain the 15 s exactly. At each failover the successor came up AT THE SAME ADDRESS in a new era; every joiner's PR 9 per-holder custody client at the manager renewed its old lease epoch there and was answered `unknown lease` ("revoked, swept past its TTL, or granted by a previous authority — it must self-fence and re-join") — and PR 12b round 4's renewal loop (Issue 22: a failed renewal re-resolves the holder once per beat and retries at the derived pace; a MOVED holder fences at once) read the holder UNMOVED (same endpoint) and paced the refused renewal to `T_self`: ≈ 13 s of retries, each `note_lease_lost` advancing the process custody generation (42 advances per joiner over three failovers — every DMA authorization in flight under any holder retired each time), then `fence_holder_scoped` at `T_self` (16:45:27–29 in round 2, 16:46:14–16 in round 3) — whose arm half `holder_fenced` stopped the holder's token planes DEAD (`TokenReaderPlane::stop_dead`: no release travels, "the holder answers nothing" — PR 9's law for a holder that DIED). But this holder was alive: between its membership re-assertion at the successor (≈ 5 s after the failover) and the fence, the joiner had fetched fresh tokens from the successor (`/` for its `mkdir`), REGISTERED there under its member id; the dead plane left them with no poll to hand a recall to, and the successor's next commit on `/` (another joiner's `mkdir`, ≈ 4 s after the fence) recalled them, waited its deadline (15 s = the lease TTL, the member alive throughout) and fired the tripwire; the joiner's fresh plane (dialed at its next read, together with the custody re-join 5–20 s after the fence) arrived at or after the deadline. The `T_self` contract (`a_dead_holders_t_self_fence_is_scoped_to_its_own_custody_and_driven_by_the_cadence`) and every in-process failover pin stand a successor at a FRESH port (the plane re-points, the old listener answers nothing) — the same-address shape is the fleet's, and pre-flip it was a `SQUEEZEFS_MULTI_WRITER=1` co-writer's alone.

**Two laws, both in `data_grant.rs`, both red-first:** (1) **the authority's `unknown lease` word is definitive** — a renewal never re-creates a lease, so a `SlotHolder` client whose renewal was answered `unknown lease` (`WriteCustodyClient::lease_gone_by_word`, set by `renew_all`) fences on the FIRST refusal (`self_fence` — the MOVED arm's law: "the owner's word that the lease died"), `renew_retries` unmoved, `T_self` never reached, ONE generation advance; the set-authority scope keeps the shipped path. (2) **a fence never orphans a registration at a holder that may be alive** — `holder_fenced` retires the planes through the new `TokenReaderPlane::stop_released` (serves stop now, the held tokens are RELEASED at the holder through the clean leave's path — the recall's drain + purge, then `Release` — then the channel task stops), spawned on the meta executor; a holder that is in fact dead answers the best-effort release with a transport error and the plane ends exactly as `stop_dead` did. `ClientStats` gained `renew_retries` (the JSON face already had it); `dlm_custody_holder_fences` is no longer must-stay-0 on a fleet WITH failovers (it counts the word-driven fence too — its doc says so). **Pin (RED-first at the fence wait — the loop paced retries for the whole 20 s bound):** `sym_custody_tests::a_holders_unknown_lease_word_fences_the_client_at_once_and_releases_its_tokens` — holder A stays UP at its address and REVOKES the writer's lease (the successor-in-a-new-era word, minted by the authority's own revocation); the client fences on the next renewal with the manual clock standing still (a fence there can only be the word's), `renew_retries` unmoved, `holder_fences` +1, `self_fences` unmoved, the generation advanced, the mount live; the retired plane's token is released at the holder (its `outstanding` falls, `releases` rises); the holder's `setattr` on the file completes inside 2 s with `timeouts_live` 0 and `recalls` unmoved (nothing left to recall); holder B untouched; a fresh acquire at A's address joins anew. The `sym-crash` count restarts from zero on the fixed binary.
...
> **Status (PR 14, `feat/sym-default-flip`, 2026-09-26 — the default flip is BUILT; it LANDS after the owner's cloud decision):** the ONE act landed in four commits — Step 0 (`2144b121`, the four commits lost with the dev machine redone red-first), Slice A (`222cbd3c`: `format` stamps bit 17 by default with `--single-writer` the flat opt-out and `--symmetric` a no-op; the multi-writer class presence-required at the WRITER's door — readers / probes / fsck / `claim clear` / the conversion admitted, the conversion QUIESCING a set not cleanly unmounted first, NO knob admitting the class; `SQUEEZEFS_SYMMETRIC_META` default `1` with `=0` a writable-open REFUSAL on every stamped volume — the §9 `=0` law CONFIRMED, `refuse_unarmed_writer_of_an_armed_forest` deleted because the door refuses before the ladder; the seam `SQUEEZEFS_TEST_STAMP_SYMMETRIC` deleted for the inverted `SQUEEZEFS_TEST_FORMAT_FLAT=1`), Slice B (`063a640e` + `95881877`: the co-writer / per-volume-owner / lane-partition / PK4 posture DELETED under the no-dead-code law — `cowriter.rs`, `partial_authority.rs`, `data_alloc_lane.rs`, `alloc_lane_grant.rs`, the OwnerMap assignment recipe, M1/M2/M3, the co-writer and peer-owned doors, `set-owners`/`get-owners`, the `pack_group` wire, the lane verbs, the `BlockRefPopulation` read, the S5 reader class; `CLUSTER_WIRE_SCHEMA` 6, `PUBLISH_SCHEMA` 18, `CUSTODY_SCHEMA` 9; the four posture knobs + eight lane knobs `Kind::Retired`; `shipped_free.rs` + `member_id.rs` the surviving client halves, the shipped-free executor and the released-block probe installed on EVERY armed writer). **§7 items this rung moved:** item 3's F-B1 stays the box's (no laptop number holds merit); **§4.4aw LANDED** (the online census is the population as of its start — `slot_ino_watermarks()` per forest slot, a post-start record skipped and the cursor jumped to the next slot; pin `fsck_c12_tests::an_online_census_is_bounded_at_the_ino_watermarks_it_started_with`, RED 2701 vs 701 unbounded; the C8 oracle's durable side and the dentry pass stay unbounded — the same class as before, owed); **F-R2 is MOOT in production** (no production client ships a `CreateWithRdev` over S8 since the daemon verb-router hook went — the S10 plane survives as the S8 client API's, its retirement whole a PR 14b item); **the 13i economy item (f) RE-FILED** (a checkpoint-pass journaling refactor; its re-derivation waits on the box's serial-commit reading); **PR 7's un-share and the manager's zero-census open are POST-flip board items** (row 14 names neither). **Two product defects the mount-class migration found, each red-first:** the allocation lease's same-node takeover judged "the predecessor is page 0" off a directory read taken AFTER this open's join had rewritten page 0 (every clean `umount` + remount at another mount point of one host refused `Busy` — `AppenderSet::page0_at_open` is the witness; `sym_block_grant_tests::a_clean_leave_then_a_mount_at_another_mount_point_of_the_same_host_arms`), and the packing suite's four mount contracts re-shaped to PR 7's law 1 (one sealed pack per slot the rotor spread a population over; the kill-9 leg's tenants in a GATHERED directory). **A THIRD, found by the inverted matrix's stamped leg — an acked-writes LOSS on the death path of every forest volume, fixed red-first (§4.4ax):** the D0 gate's ring-recovery preflight (`preclaim_ring_recovery`) flushed a crashed writer's replayed records into the slot-tree leaves BEFORE PR 5's frame fence was primed, so those frames carried `(0, 0)` below the leaf's leased generation and rule 3 screened them — and everything after them — at the next open (the v3 kill-9 soak's replay-twice digest, RED ~1 round in 5 with a 1 MiB ring under the pad law); the stamps are primed at the open's FIRST frame writer now (`sym_coherence_tests::a_writer_recovering_an_exhausted_ring_inside_its_gate_stamps_the_leases_generation`; the soak GREEN ×5 from zero on the stamped leg). **A FOURTH, found by the require-mount gate — every armed writer's `df` (§4.4ay):** `get_used_blocks` was the single-writer arithmetic `highest − free_list`, and on a grant-armed allocator the free list never fills (the bitmap IS the free list), so `statfs`'s used count, the placement fill and the drain preflight's `avail` only ever grew; a holder's used count is its bitmap's population minus its own unconsumed window now (`sym_block_grant_tests::an_armed_holders_used_count_follows_its_bitmap_not_the_single_writer_arithmetic`; a joiner's `df` is PR 14b's — the population on the grant reply). The mount-class presumptions the flip moved (§4.4az) are seam-blind on both legs. **A FIFTH — this rung's OWN regression, found by the `sym-crash` leg (§4.4ba):** §4.4aw's census bound folded the grant-time cursor words of slots OTHER appenders lease into the manager's per-slot watermarks, so the successor's census skipped every joiner-minted ino and read the joiners' live blocks as false C2 after a manager failover; the map names only the keyspaces whose live cursor this mount holds now (`sym_n_daemon_tests::the_managers_census_watermarks_never_bound_a_slot_a_joiner_leases`). **A SIXTH, found by the same leg's round 2 on the fixed binary (§4.4bb):** a joiner's first shipped step after the SECOND manager failover failed `EINVAL` — the S8 lane presented the dead manager's writer era at the successor (the fleet's one stable manager address = the same lane), the owner refused the frame whole and the cross-owner ladder surfaced the refusal instead of retrying it (the first failover never shows it: a fresh lane's era `0` is admitted); the refusal is the typed `RefusalClass::StaleOwnerEra` now and every cross-owner ship resends ONCE on the relearned era (`xv_cross_owner_step_stale_era_retries`; `sym_n_daemon_tests::a_joiners_first_shipped_step_after_a_failover_retries_the_stale_era_once`). **A SEVENTH, found by the same leg's round 3 on that binary (§4.4bc):** after a manager failover at its own address every joiner's per-holder custody client paced its `unknown lease` refusals to `T_self` (≈ 13 s, one custody-generation advance each) and then stopped its token planes DEAD under a LIVE successor, orphaning the tokens it had fetched there since re-asserting — the successor's next commit on `/` waited the whole 15 s recall deadline and fired `dlm_token_recall_timeout_live` (`invariant_tripwires` 1); the authority's word fences the scoped client on the FIRST refusal now and the fence RELEASES the planes' tokens at the holder (`TokenReaderPlane::stop_released`; `sym_custody_tests::a_holders_unknown_lease_word_fences_the_client_at_once_and_releases_its_tokens`). The rails, the fleet legs from zero and the two-host leg on the flip binary are the rung's report's; the box brackets of every gate on the flip binary and the cloud re-run wait on the owner.

> **Status (PR 13i, `fix/sym-shared-lun-coherence`, 2026-09-24 — the cloud row's three findings, each FIXED red-first; §3.10, §4.4ar–at):** **F-C1** — metadata device I/O is `O_DIRECT` on every host with a derived grain and an aligned form for every shape (the journal ring's sector-pad law the one aligned-form change; design §5.12), pinned by the two-host qemu/KVM fixture (`sym-two-host`, RED ×6 on the base — the writer's write-behind ×5, the reader's stale cache ×1 with the bdev held open) and `tests/meta_io_direct_tests.rs`; **F-C2** — the joiner honours the `Joined` reply's grant word; **F-C3** — the fan-out keeps the error class (`KvError` / `SqueezefsError` are `Clone`; the flattening helper deleted). **New PR 14 inputs:** (a) the ring-capacity cost of the pad law on a 4 KiB-grain device — a page per WINDOW, so a SERIAL workload's ring runway is its page count (8,192 windows on 32 MiB); the flat-path scoping rows on the laptop are this rung's, the box bracket on the flip binary judges the serial-commit shapes (`mdstorm`, `w_fresh`); (b) the joiner floor ring (512 KiB = 64 pages of windows) churns cycles under serial creates until PR 13g's growth catches up — `appender_pressure_cycles` on the two-host leg reads it; (c) **the cloud re-run runs on a binary whose `sym-two-host` is GREEN first** — the fixture is the cloud row's shape on two kernels for the price of a laptop. (d) **§4.4aw — the online fsck census's liveness under a creator that outpaces its walk** (pre-existing; RED on the base): bound every online census walk at the ino watermark captured at its start (per slot on a forest; the C8 oracle's durable side by the same owner watermark) so a census costs O(records at start); until then an online `squeezefs fsck` on a volume with a hot creator has no bounded duration and `fsck_c12_tests`' storm contract keeps a bounded lead over the census. (e) **§4.4av F-C4** is FIXED here (the region's `grant`-before-`page` lock law) — the lock-order rail over the appender region's mutexes (`tests/appender_lock_order_tests.rs`, the `kv_loader_lock_style_tests` shape) landed in fix round 1 (Issue 1), so this belt is no longer owed. (f) **The SMO-record window economy (review round 1, Issue 6):** under the pad law the byte-denominated ring derivations bind in WINDOWS — on a 4 KiB-grain device every reservation occupies whole pages, and the checkpoint task journals ONE entry per SMO (`smo_replace` reserves per record), so the §4.4 pt 5 reserve `max(256 KiB, ring/64)` is 64 checkpoint-class SMO records per pass at the floor (256 KiB ÷ 4,072 B) and 128 on the 32 MiB ring before the deferral class engages, and a floor ring's admissible slice (≈ 259 KiB) is ≈ 63 conveyor batches; on 512-grain the bound is `reserve ÷ (entry + 533 B)`. The economy that removes most of the term: coalesce one flush pass's SMO records into ONE window — one pad per pass instead of one page per SMO, the same batching the conveyor already gives user commits (`submit_entries_batch(window, …)` takes a window of N entries; the pass's SMOs would reserve once, in order, and complete as one). Both constants' doc lines now state the window arithmetic (`journal::checkpoint_reserve_bytes`, `appender::SYM_RING_FLOOR_BYTES`); the re-derivation with the grain waits on the box bracket's reading of the serial-commit shapes ((a) above).
...
> **Status (PR 14, `feat/sym-default-flip`, 2026-09-26): the flip is BUILT — every act of design §12 item 3 landed on the branch, pinned (`tests/sym_default_flip_tests.rs`; the inverted 43-suite matrix flat THEN stamped from zero; the mount-class suites under the new default; the solo re-gate on a plain `format` + `mount` as "it works"), and it LANDS on `dev` after the owner's cloud decision — the box brackets of every gate on the flip binary (gate 1's serial-commit shapes under the pad law, F-B1's tripwire on 32 writers, gate 3's per-NODE law on the cloud `mw` preset) are what the owner reads before that word. The `=0` law reads as §7.2 states it: `SQUEEZEFS_SYMMETRIC_META=0` on a stamped volume REFUSES the writable open, naming the knob and `-o ro`; the reader / probe posture is `-o ro`, never a knob value. **What the flip's matrix found and this rung fixed before it could ship as the default (§4.4ax): the pre-claim ring recovery's unstamped frames — a kill-9 with a near-full ring followed by a remount LOST the dead writer's last acked creates at the open after the recovering mount's clean leave, on every forest volume since PR 1; the flat soak that has run for months found it the first time its volume was a forest.** Nothing below is re-decided by this rung; the list above (§7's PR-14 status) says what moved.**

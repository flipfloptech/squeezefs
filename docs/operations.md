...
- **Owed by the later rungs** (the PR plan): appender pages and per-appender rings (PR 2), the manager lease (PR 3), slot leases and the `apply_locked` lease gate (PR 4), tokens (PR 5), the fsck ONE-walk census over the mixed leaves (today the kind-routed walkers read a forest's slot trees once per kind, and a malformed slot-tree key surfaces as a C1 walk finding for that kind rather than a record-level, in-place-repairable one), `format --symmetric` (PR 11), and the squeeze-test solo re-gate A-B-B-A (the note's §7). The lazy mint still runs one device write on the conveyor pass task (once per slot per volume, before any node lock); design §5.3.3 moves the extent to the appender's grant in PR 3.

...
1. **Unreclaimable pressure arm.** The sampler reads cgroup-v2 `memory.stat` per tick and windows the *kill-relevant* set — `anon + file_dirty + file_writeback + shmem + unevictable + slab_unreclaimable` (clean file cache deliberately excluded: the kernel drops it rather than kill) — in the same 5-slot decaying window as RSS. `pressure = max(Σ gauges, windowed statm RSS, windowed unreclaimable)`. This is the gauge-undercount defense: dirty page cache is invisible to statm, and anon floods in un-inventoried components (the PR 7 KV-core residual) are invisible to gauges; the kernel's own accounting is not.

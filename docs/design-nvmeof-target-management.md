...
> **STATUS BANNER — SPDK retired as a target (2026-09-12, owner ruling R-SYM-8; landed by PR 16 of `docs/design-symmetric-metadata.md`, §5.8.1 / KD-SYM-23, forward-only).** The kernel `nvmet` target is THE supported NVMe-oF target. SPDK's compiled-in `SPDK_NVMF_MAX_NUM_REGISTRANTS = 16` was the only hard registrant ceiling SqueezeFS ever shipped, and the fencing-group machinery designed to fit 12,500 hosts under it left the symmetric-metadata program with it. What this means for the document below — which is preserved verbatim as the program's history and is NOT rewritten: every SPDK arm it specifies (the JSON-RPC client, the pinned v26.05 build/install, hugepage setup, the pidfile lifecycle, `save_config`/`load_config` persistence, `ptpl_file` pinning, the SPDK preflight ladder, the cross-stack live-state duplicate guard, `adopt_ambiguous`, the SPDK fidelity legs and the PTPL power-cycle leg) is **deleted from the tree**; `--target-stack spdk`, `SQUEEZEFS_NVMEOF_TARGET_STACK=spdk`, `nvmeof target install` and the retired knobs `SQUEEZEFS_SPDK_TGT_BIN` / `SQUEEZEFS_NVMEOF_RUN_DIR` **refuse loud naming nvmet and the re-share sequence**; the §6.4 ledger schema is unchanged and an SPDK record stays decodable — listed by `nvmeof list` as retired (with its state-dir residue path), skipped by `restore` (which exits nonzero while one remains), removed ledger-only by `unshare`, never re-presented. The §6.3 deployment-class table, the G5 A/B and the SPDK rows of the gate ledger stand as measured history; the retirement is a registrant-ceiling ruling, not a re-measurement. Operator surface: `docs/operations.md` §NVMe-oF operations; contracts: `tests/nvmeof_retire_spdk_tests.rs`; evidence: `.benchmarks/2026-09-13-sym-retire-spdk.md`.

...
| `free_grace_lane_push_wakes` / `alloc_lane_pushed_harvests` / `free_grace_lane_supply_hint` (co-writer side) | the lever's co-writer faces: hints that woke a refill, the pushed harvests they produced (⊆ `alloc_lane_harvests`), and the last hint learned (blocks of this lane on the authority's list — 0 = nothing owed there right now). Wakes with no pushed harvests = the allocator is owed nothing (`alloc_lane_owed_blocks` 0) |

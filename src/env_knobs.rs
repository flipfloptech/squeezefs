...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_FUSE_ZC_WRITE_FUSION", Kind::Bool, "on", "Handler/worker FUSION for small armed FUSE WRITEs (zc-write-fusion campaign, D16's fast-track): hold-candidate writes at or under the fusion ceiling run their handler future on the queue worker's own fused lane, so the store/extract bridge round trip pays zero cross-thread wakes. `0` = the pre-campaign handler-lane dispatch (the A/B control). Engagement: fuse3_zc_write_fusions/_bytes."),

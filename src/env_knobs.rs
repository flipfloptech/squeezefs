...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_OVERLAY_OVERWRITE", Kind::Bool, "off", "The overlay OVERWRITE arm (design-overlay-overwrite, PR B4): aligned single-block overwrites of mapped striped passthrough blocks ride the zero-copy slot->device overlay with a fresh CoW dest (never in-place, KD-B4-2) and publish by FEEDING the rewrite epoch (arm (a), KD-B4-1). Default OFF — FIELD-PENDING (user ruling 2026-08-15, supersedes KD-B4-9's ON): the local B4d A-B-B-A failed the above-control gate both orders (ON 783-940 vs OFF 1149-1161 MiB/s sustained on the device-bound zram-tcp venue, where the control's BDP-depth pipelining wins; engagement exact, rewrite_amp 1.0000, daemon CPU/GiB -30%), so the falsified-lever rule ships it as a documented FIELD lever: `1` on CPU-bound fabric fleets (the 96.6% merge-share wall this arm exists to delete) pending the B4e field adjudication. SQUEEZEFS_DEVICE_OVERLAY=0 disables the whole overlay including this arm. Engagement: overlay_overwrite_installs / overlay_overwrite_bytes (the CQE-counted overwrite subset of overlay_store_bytes)."),

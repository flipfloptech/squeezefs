...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_ZC_ACK_EARLY", Kind::Bool, "on", "ACK-early for device-overlay stores (kernel 0029; design-zc-write-kernel-v2 §6.2-6.4, design-device-overlay §8's accelerator): eligible overlay stores reply while the DMA runs against the RETAINED zc slot — the B2 falsification's named depth-cap release. Engagement additionally requires SQUEEZEFS_DEVICE_OVERLAY, a retention-negotiated session (fuse3_zc_retention_negotiated=1), and the §3.4 class gate (page-cache-sound writes; O_DIRECT needs SQUEEZEFS_ZC_ACK_EARLY_ODIRECT), so the ON default is inert on every shipped posture. Coverage/publication/fsync/read-wait stay CQE-anchored; acked custody retries forever (overlay_ack_early_retries), fence is the one drop (overlay_fence_drops). `0` = the A/B lever. Engagement: overlay_ack_early_stores/_bytes + fuse3_zc_retain_commits."),
    k("SQUEEZEFS_ZC_ACK_EARLY_ODIRECT", Kind::Bool, "off", "The §3.4 UNSTABLE-WRITE opt-in: lets O_DIRECT/GUP-class writes ACK early. A post-ACK reuse of the application buffer persists scribbled bytes (the NFS UNSTABLE/COMMIT contract) — the bytes that land are sampled at DMA time. Off by default BY LAW; arm only on workloads that never reuse a write buffer before fsync (labeled benchmark postures, append-only producers)."),

...
"$SQUEEZEFS_BIN" format \
    sqmeta:///dev/shm/squeezefs_dma_meta \
    sqdata:///dev/shm/squeezefs_dma_backend \
...
{
    echo "leg: $LEG"
    echo "instrument: fio psync randread bs=4k direct=1 numjobs=$THREADS runtime=${SECS}s time_based (via LD_PRELOAD shim; elbencho disqualified: static binary)"
    echo "substrate: /dev/shm file-backed sqmeta+sqdata (A/B-relative only; not absolute-comparable to the devsub numbers)"

...
| S3.5 | Cross-volume transaction machinery (ruling D4) | **LANDED** | `SQZXTX01` intent rides step 0's commit; roll-forward recovery at mount through one witnessed applier; **DUR-7 closed** (unlink/rmdir, link, cross-parent rename incl. whiteout, exchange) with the two-volume commit-boundary seam suite; no incompat bit, +1 entry per cross-volume op. S8's cross-volume verbs call `crossvol_tx::execute`; a REMOTE participant still waits on S8's wire |

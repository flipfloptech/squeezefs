...
| `meta_ship` (object incl. `owner_dispatch_{inline,hops}`), `meta_ship_publish` (object incl. `ship_frames`, `ship_framed_calls`, `ship_depth_waits`, `ship_session_dials`, `ship_mux_frames`, `served_{frames,frame_calls,chains}`, `free_{shipped_blocks,served_blocks,replays,stale_refusals,ship_failures}`, `lane_free_notices_{queued,shipped}`, `map_{shipped,served,refused}`), `meta_ship_intent` (object), `meta_ship_placement` (object), `meta_ship_phase_ns` (`route`, `queue_wait`, `encode`, `rtt`, `decode`), `meta_ship_owner_phase_ns` (`admit`, `dispatch`, `execute`, `reply_encode`, `total`), `meta_ship_owner_dispatch_ns` (`queue_hop`, `run`, `wake_hop`, `total` — D-5's split of the owner's dispatch, exact-sum per dispatch on both planes) | Metadata function shipping and its publish/intent/placement planes ([Metadata function shipping](#metadata-function-shipping-dlm-s8)) |
| `cowriter` (object: `mw_role`, `custody_endpoint`, `admissions`, `admission_refusals`, `accounting_refusals`, `local_commit_refusals`, `unpublished_{abandons,recycles}`, `free_ship_own_lane_untracked`, `recomputed_retires`, `lane_free_notices`, `lane_free_notices_reminted`), `peer_volume_local_commit_refusals`, `peer_volume_unclaimed_{admits,refusals}` | The co-writer posture and the per-volume-owner gate ([Multi-writer co-writer mounts](#multi-writer-co-writer-mounts-dlm-stage-s9), [Per-volume metadata owners](#per-volume-metadata-owners-squeezefs-volume-set-owners)) |
| `alloc_fresh_mints`, `alloc_from_freelist`, `alloc_lane_writers` (0 = unpartitioned), `alloc_lane_id`, `alloc_lanes_owned`, `alloc_lane_{reservations,shipped_reservations,adoptions,raise_refusals,enospc_refusals,stranded_bytes}`, `alloc_lane_{harvests,harvested_blocks,ahead_harvests,pushed_harvests,owed_blocks,reachable_blocks,supply_blocks,harvest_watermark,harvest_horizon_ms,horizon_hints}`, `alloc_lane_{share_needed_blocks,headroom_pct}` (the capacity law, live), `alloc_lane_visible_phase_ns` + `alloc_lane_visible_unplaced` + `alloc_lane_release_marks` (the released → served → visible hop, finding 15 term 2), `alloc_lane_stale_records_{pruned,folded}` | Block allocation and the multi-writer allocation partition ([Multi-writer capacity planning](#multi-writer-capacity-planning--the-data-plane-allocation-partition)); the harvest family is the laned co-writer's refill from the authority; stale-record hygiene runs once per affected set |
| `membership_mode` (word), `membership_{members,readers,writers}`, `membership_{lease_ttl,self_deadline,grace_remaining}_ms`, `membership_min_acked_free_epoch`, `membership_{joins,renewals,renew_refusals,registration_commits,self_fences,evictions,grace_refusals,grace_reclaims,census_serves}`, `membership_renew_sched_lag_ms` (max-gauge), `membership_renew_phase_ns` (`carry_wait` / `rtt` / `total`), `membership_renew_serve_ns`, `membership_renew_lane_calls`, `membership_renew_cadence_ms` (member) | The membership plane ([Membership plane](#membership-plane--lease-based-liveness-dlm-s6)); `renewals` grows while `meta_kv_journal_entries` does not — that is the point. The phase tables say WHERE a renewal's time went (member venue / wire / authority serve) and the cadence gauge whether the beat itself is the hole |
| `free_grace_mode` (word), `free_grace_{deferrals,releases,offsets,bytes}`, `free_grace_bound`, `free_grace_{fence_bound_ms,fence_bound_base_ms,pressure_bound_ms,ring_cap}`, `free_grace_reader_acks`, `free_grace_{forced_releases,laggard_fences,alloc_stalls}`, `free_grace_pressure_pct`, `free_grace_{prods,prod_renew_ms,bound_tightenings}`, `free_grace_demand_{pct,prods}`, `free_grace_bound_refreshes`, `free_grace_pass_{prods,interval_ms}`, `free_grace_ack_pipeline_depth`, `free_grace_acked_lag_ms`, `free_grace_hold_phase_ns`, `free_grace_hold_{unplaced,ms}`, `free_grace_checkpoint_{marks,cycle_ms}`, `free_grace_member_ack_lag_ms` (+ the gated `_census`), `free_grace_bound_refreshes_on_ack`, `free_grace_bound_scan_ms`, `free_grace_ack_renewals`, `free_grace_lane_push_{releases,hints}` (+ the co-writer's `free_grace_lane_push_wakes` / `free_grace_lane_supply_hint`) | The freed-offset grace period and its pressure valve ([Freed-offset grace period](#freed-offset-grace-period-spec-68-item-3)); closure law `deferrals ≡ releases + offsets` |

...
| (g — found by the rebased ledger, a REAL defect on PR 5's own surface) the pre-arm frame stamp under PR 6/8's open | The first ledger attempt read `sym_cross_owner_tests` 10/27 red on BOTH layouts (C9 "unreferenced inode", one "no reply within 10 s"); `git bisect run` landed on PR 5's first commit (`5b410fd3`, frame v2 + the screen). The open's bring-up cover and the join's cycle flush the D0 claim's native-slot leaf BEFORE the arm, so `frame_stamp_for_slot` (reading the GATE-filtered `slot_leases()`) answered the manager's `(0, 0)` — below the `g` the leaf's earlier frames carry from the previous armed session — and rule 3 screened the frame at the next load: the log ENDED there, every later commit of that open invisible to the remount and to fsck (the C9 findings were the dentries whose inode records sat past the screened frame). Fix (`7c7ce96a`, three parts of one law — a slot's generations are ONE sequence, the stamp never moves backwards): `prime_frame_stamps` loads tree 0's lease table and installs the fence BEFORE `cover_bring_up_residue`; `frame_stamp_for_slot` reads the appenders' table whether or not the gate is armed and answers the manager's NATIVE slot with `g + 1` for an `Unleased { g }` before the arm (the arm acquires it unconditionally at `g + 1`, so a pre-arm frame at `g` would sit past `g`'s recorded tail — rule 2's zombie shape); `frame_g_floor` keeps a per-slot monotone floor for the mount's life (the leave drops the plane before the shutdown's final flush passes). A first attempt — deferring slot-tree leaves in the checkpoint while unarmed — broke the bring-up cover ("did not cover within 64 cycles") and was reverted. `sym_cross_owner_tests` 27/27 after. | `commits_after_a_reopened_writers_join_survive_the_next_remount_unscreened` (three armed opens × four creates, a read-only remount resolves every name, `META_KV_FOREIGN_FRAMES_SCREENED` unchanged) — RED with the prime short-circuited ("lost across the remounts"), green with it |

...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_META_SHIP_INLINE_SERVE", Kind::Bool, "on", "D-5 (e2e perf audit DLM board #7): the OWNER-side dispatch venue on both shipped planes (the S8 verb frame; the S9 publish call / group / free / harvest). `on` (the default): a served dispatch is polled on the accepting connection's own thread (`sqz-clw-conn`, dedicated and parked for exactly that reply) — the two cross-thread hops onto and off the shared sqz-meta lanes are gone (0 by construction) and every wake inside the work unparks that thread directly instead of queueing behind the lanes' serve/publish storm; C-2's fleet attribution measured the hop at 2.0–2.3 ms per verb vs ≤ 32 µs quiet. `0` = the shipped `spawn_meta_join` hop — the same-binary A/B control, never an operational escape. Read once per served frame (one getenv per wire round trip). Engagement: meta_ship.owner_dispatch_inline ≡ every dispatch on the default (owner_dispatch_hops carries them under `0`); attribution: meta_ship_owner_dispatch_ns.{queue_hop,run,wake_hop,total}."),
    ...
    k("SQUEEZEFS_PUBLISH_SHIP_DEPTH", int(1, 64), "derived clamp(ceil(cpus/8), 2, 8)", "DLM S9 publish plane (E2E perf audit D-1b): publish FRAMES one co-writer keeps in flight per authority — one authenticated session each on the D-1b session pool (SQUEEZEFS_PUBLISH_SHIP_MULTIPLEX=0), all on ONE pipelined session by default (D-5). Derived on the owner RPC-lane slope (one per 8 cores, floor 2 = the minimum at which a frame's RTT overlaps a sibling's owner pass, ceiling 8 = the RPC-lane ceiling). `1` is the stop-and-wait A/B control (frames still batch concurrent publishes; nothing pipelines), never an operational escape. A frame is whatever is queued when a slot frees — no timer, no added delay on a serial stream."),
    k("SQUEEZEFS_PUBLISH_SHIP_MULTIPLEX", Kind::Bool, "on", "D-5 (e2e perf audit DLM #8's single-connection half): the co-writer's in-flight publish frames toward one authority ride ONE pipelined cluster-wire session (calls carry their ids, a per-session reader thread `sqz-clw-mux` demultiplexes the replies, the authority's session lane serves the in-flight frames concurrently) instead of one request/reply session per frame — so SQUEEZEFS_PUBLISH_SHIP_DEPTH no longer costs that many connections against the authority's max_connections cap (F-B: one owner thread per connection). Default ON. `0` = the D-1b session pool (depth sessions per authority) — the same-binary A/B control, never an operational escape. Read once per publish lane at its first frame. Engagement: meta_ship_publish.ship_mux_frames ≡ ship_frames on the default (0 under `0`); ship_session_dials reads 1 per endpoint at steady state instead of depth."),

...
- **Payload-arena cost scales with the negotiated ent size**: `queues × depth × max_write` registered anon bytes under the same `min(mem-budget/8, 2 GiB)` cap — the depth leg degrades 32→4 first, and only past that does `max_write` itself degrade back toward the 1 MiB base (never below it). Field shape 32 queues × 4 MiB ⇒ depth 16 at the 2 GiB cap.

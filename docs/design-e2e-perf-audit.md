...
| ~~15~~ | **Write #9 — fsync economy — LANDED 2026-09-08 as W-5 (`.benchmarks/2026-09-08-w5-fsync-economy.md`); the box row is OWED** | `fsync` flushed ALL data namespaces (one coalesced device Fsync per registered device, serially, write-through included, clean fsyncs included) + 3 serialized meta legs; **no `fsync_phase_ns`**. Read at the code: the orderings that carry semantics are the S10 intent barrier FIRST (its errno short-circuits the op), the WHOLE data-barrier step before the meta legs that name the blocks (DUR-1), the meta barrier LAST; independent are the per-namespace device Fsyncs, the staged `sync_key`, and the S11 `FlushExtents` force | `fsync_phase_ns` (always-on, exact-sum: `intent_barrier` / `data_flush` / `data_barrier` / `meta_publish` / `meta_barrier` / `extent_barrier` / `total`) + the `fsync_*` ledger; `SQUEEZEFS_FSYNC_TOUCHED_NAMESPACES` (per-ino touched set = a stripe word of device-ordinal bits + a stamp generation, stamped at `block_ref_ops` — the C8 oracle's closure — and at the two in-place DMA shapes; write-through namespaces skipped); `SQUEEZEFS_FSYNC_PARALLEL_LEGS` (the legs `join_all`ed, every leg run to completion — never cancel a sibling: a dropped `flush()` fails the coalescer's queued waiters). In-process A-B-B-A (2 volumes × 3 ms seamed barrier, 192 two-block files, 8 streams): **+47–58 % fsyncs/s, p50 16.4–17.0 → 9.6–10.0 ms, `data_barrier` −48 %**; the physical barrier count is unchanged (the coalescer already amortized 4 : 1) — the levers move each fsync's WAIT; the ladder is 99.8 % barrier wait on that venue | `w_durable` + a small-file fsync storm on squeeze-test (B-9) — the parent's; on `w_durable` the touched set is every namespace (a stream spreads across all five), so that row measures the joined legs only |

### 3.4 Tier 3 — per-op economies (low each; batched per plane)

| Item | Term | Lever |
|---|---|---|
| ~~Read #6~~ | kernel READ handler **8 allocs/op + ~20 global atomics + ~9 clock reads** — **LANDED 2026-09-08 as R-5** (`.benchmarks/2026-09-08-r5-read-handler-economy.md`): measured by site 8 allocs warm / 11 cold zc, 26 shared-line RMWs, 13 clock reads → **0 / 1 (the injected fetch's box) / 0 stripe-shared / 9**; alloc budgets 1 (warm) + 2 (cold zc, new) pinned; 8-thread phase record −72 %; **field row MET** (`.benchmarks/2026-09-08-campaign-rows-squeeze-test.md` §2: `rr_4k` kern +6.8 % IOPS, daemon CPU/op −9.6 %, `fuse3-tpc` −18 %) | inline custody epochs, `block_key_in` by-ref on the serve arm, stack-key W2 gate, borrowed fetch-ladder key + in-place recheck, the vestigial `backing` Arc deleted, `ZcReadServe` borrows the connection, `ShardedLatencyHistogram` + `ShardedAtomic` for the per-op read families/counters, span-form clock chain (the zc leg now records `classify_probe`) |
| ~~Read #7~~ | ranged read per-op string re-parse — **LANDED (R-5)**: `clean_block_key_ref` (a sub-slice by construction) + `BlockKeyRef` + `with_allocator_for_key` under every hot resolver; the cold zc leg's four re-parses cost 7 allocs + 6 refcount RMWs per op → 0; `fill_incarnation` 143 → 61 ns, `zc_device_resolve` 129 → 76 ns (dev-box microbench) | the parse is heap-free, so "carry the parts" is moot |
| ~~Read #8~~ | kvmap partial resolve: `parse_kvmap_head` per call, `String` clones, **O(4096) allocs per window fill** — **LANDED (R-5)**: one arena per window span (`map_entry_block_key_at_into`), the canonical-decimal guard scans instead of rendering | typed head / borrowed keys |
| ~~Read #9~~ | R2 residency: 5 probes per decision — **LANDED (R-5)**: `touch_no_promote` (clock-bit refresh, no `Bytes` clone) on both RAM tiers; the short-circuit order already made a hot hit one probe | one probe |
...
| ~~Write #7~~ | per-WRITE **4 Box + 4 Arc slot closures, per-block `String`s, ~30 atomics** — **LANDED 2026-09-08 as W-6** (`.benchmarks/2026-09-08-w6-write-handler-economy.md`): the warm W1 patch write 14.6 → 3.06 allocs/op (op-economy suite, budget 4/op; the three left are the device layer's), the 17 shared-line METRICS RMWs per write striped per thread (`write_handler_counters_32t`: 129 → 16.6 ns per 20-RMW set at 32 threads), the zc held-slot mint one `Arc` clone instead of 4 Box + 4 Arc; the census named the write-phase scc map (3 allocs/op from shrink-to-zero) as the largest single site | in-process e2e row `write_handler_e2e` (before/after in the note); the field `rw_4k` CPU/op row is owed to the parent |
| ~~Write #10~~ | gap seeding re-reads the whole old block — **LANDED 2026-09-08 as W-6**: the settle sources each gap by ONE ranged read of exactly its bytes on passthrough undecorated bindings (`SQUEEZEFS_GAP_SEED_RANGED`, default on; `overlay_gap_seed_read_bytes` is the device-byte face — block window → Σ gaps; decorated/transformed keep the whole read by construction) | `tests/overlay_gap_seed_ranged_tests.rs`: one gap 16 KiB → 4 KiB read, two gaps → 12 KiB, control = 16 KiB |
...
| ~~DLM #7~~ | owner `spawn_meta_join` hop — **LANDED 2026-09-08 as D-5** (`.benchmarks/2026-09-08-d5-owner-hop-and-depth.md`): the split instrument `meta_ship_owner_dispatch_ns` {queue_hop, run, wake_hop, total} first (exact-sum per dispatch, both planes), then the lever `SQUEEZEFS_META_SHIP_INLINE_SERVE` (**ships OFF** after the fleet row — see the last column) — `1` polls the served frame on the accepting connection's own thread, both hops 0 by construction, every in-run wake a direct unpark. In-process release, 4 clients × 256 one-verb frames under a 4 × 200 µs `sqz-meta` hog (A-B-B-A, two rolls): hop `dispatch` 768–781 µs = queue_hop 347–350 + run 414–428 + wake_hop 4.5–5.4 → inline 40–65 µs (−92…−95 %), verbs/s 2.54–2.57 k → 4.77–4.91 k, p99 1–2 ms → 128–512 µs; quiet: 47–55 → 41–50 µs, verbs/s par. The publish plane under the same hog: dispatch 1.00–1.13 ms → 441–554 µs, publishes/s +33–35 % | **fleet row RUN 2026-09-08** (`.benchmarks/2026-09-08-d5-fleet-squeeze-test.md`, two same-binary A-B-B-A brackets, both orders, 8 co-writers × 24 streams): the two hops go to 0 exactly but `run` grows 0.6–0.9 ms (536–770 → 1,211–1,473 µs) — the in-work wakes the lanes absorbed as re-queues are OS unparks on the connection thread and the durability lane's fan-out pays them (`sqz-jrnl` +17–25 %); co-writer publish latency +15–49 %, ingest −1.5…−7 %, verbs/s par. The lever ships OFF; the in-process hog venue was the wrong model of the fleet (lanes at ρ ≈ 0.2). What remains of the owner's dispatch is `run` itself — the served commit's conveyor wait and apply, the verb-plane grouping rung's term |
...
| 11 | `perf/read-zc-serve` — **LANDED 2026-09-05, default ON (unaligned r_cold/r_warm A-B-B-A: daemon CPU/GiB −24 % / −25 %, `fuse3-ur` −37..−46 %, throughput ≥ par both orders; `.benchmarks/2026-09-05-r4-read-zc-serve.md` §7)** | R-4 | R #4/#5: `READ_FIXED` into folios (sqz kernel) | CPU/byte; `read_copy_*` closure; zc-capability gate |
| 12 | `perf/zc-odirect-extraction` | W-3 | W #4 | `w_rewrite` O_DIRECT CPU/GiB |
| 13 | `perf/free-grace-accept` — **fleet acceptance NOT MET 2026-09-05** (design landed 08-25; the s11 row reproduces finding 15 — lane ENOSPC at 23 s — and exposes a co-writer WEDGE on ENOSPC'd writes; `.benchmarks/2026-09-05-d4-free-grace-sustain.md` §6) → **the hold-time campaign 2026-09-06, `perf/free-grace-hold-time` (`.benchmarks/2026-09-06-free-grace-hold-time.md`): with the supply leak closed the ring HOLDS the supply (`bound_age` 8,994 ms at every 1 Hz cadence); the hold is now DECOMPOSED per stage on the stats inode (`free_grace_hold_phase_ns`, `free_grace_member_ack_lag_ms`, `free_grace_hold_ms`) — 6.0 s of it is the two derived reader windows (`S + skew`, `S + D_purge`), ≈ 2.5 s cadence terms; levers (b) carriage renewal + (d) refresh-on-ack landed on in-process evidence (−922 ms, −10.7 %, closure exact, zero fences), the checkpoint-cadence lever measured INERT alone (the reader qualifies on a time bound), the capacity law `churn × hold + live ≤ cap/W` published live (`alloc_lane_share_needed_blocks` / `alloc_lane_headroom_pct`); the 6 s core is four coherence-proof adjudication items (note §7); fleet row OWED (parent)** | D-4 | DLM #6: accept the sustain campaign — **LANDED, FIELD OWED** (`.benchmarks/2026-09-05-d4-free-grace-sustain.md`): the campaign's five levers were already in the tree (PRs 1–4, 2026-08-25); D-4 closed the §3 rate equation IN-PROCESS on one deterministic clock — the shipped levers unbind a recycle-bound stream (80.0 MiB/s flat, 0 stalls vs 72.1 / 179; `bound_age` 8.6 s vs 11.8 s; forced = fences = 0), Little's law `rate ≈ spare ÷ bound_age` holds on live gauges, and the 2026-08-30 GREEN cloud row's 23–29 s `bound_age` is the UNCOUPLED routine composite (demand dark by design), not a campaign miss; finding D4-1 (the re-based runway's fleet-rate divisor asks the floor on mid-supply lanes — inventory, not throughput) priced and pinned | owed: the from-zero s11-mpiio row on the finding-15 venue with the sustain columns (the parent runs it; command + verdict columns in the note) |
...
| ~~15~~ | `perf/fsync-economy` — **LANDED 2026-09-08, both levers default ON on in-process evidence (A-B-B-A: +47–58 % fsyncs/s, p50 −40 %, `data_barrier` −48 %; physical barrier count unchanged — the coalescer already amortized 4 : 1; `.benchmarks/2026-09-08-w5-fsync-economy.md` §4)**; **field row MET 2026-09-08 (`.benchmarks/2026-09-08-campaign-rows-squeeze-test.md` §4, E F F E on squeeze-test, kernel 0031, the 5-node nvme-tcp set): fsync storm +23.8 % fsyncs/s, fsync p50 −22.9 %, data sync REQUESTS 10 → 1 per fsync (physical 1.88 → 1.00), meta barrier still one, tripwires 0; `w_durable` PAR by construction (a 24-file stream touches every namespace). The field `fsync_phase_ns` reads `data_flush` = 80 % of a 6.9 ms fsync — the partial-active-block escalation (4 MiB seed + 4 MiB upload per 256 KiB fsync, both arms), filed as write board #11** | W-5 | W #9: `fsync_phase_ns` first (exact-sum, always-on), then touched-namespace flush (write-through skipped) + joined legs (the intent barrier first, the barrier step before the meta legs, the meta barrier last — DUR-1 untouched) | `w_durable` + fsync storm (B-9, tcp + field) |
| ~~16~~ | `perf/read-handler-economy` — **LANDED 2026-09-08 (`.benchmarks/2026-09-08-r5-read-handler-economy.md`); field row MET 2026-09-08 (`.benchmarks/2026-09-08-campaign-rows-squeeze-test.md` §2): `rr_4k` kern 24×8 E F F E on squeeze-test +6.8 % IOPS, daemon CPU/op −9.6 % (`fuse3-tpc` −18 %, `fuse3-ur` −2 %), p99.9 −9.6 %, both orders agree to 0.3 %; the il control row +3.1 % / −4.9 % bounds the free-standing term** | R-5 | R #6/#7/#8/#9 batched + the R-1 residual adjudicated moot for READ (since R-2 a kernel READ never traverses `InboundQueue::pop`). Per-op ledger: warm ladder READ 8 → **0** allocs, cold zc leg 11 → **1** (the injected fetch's box; the live arm carries none), 26 shared-line RMWs → 0 (per-thread stripes, exact fold pinned), clock reads 13 → 9; dev-box microbench: fp build −28 %, cold key resolves −41…−57 %, 8-thread phase record −72 % | `kernel_op_economy_tests` alloc law — budgets warm 1 / cold zc 2, both red on the prior tree — **DONE**; `rr_4k` kern 24×8 A-B-B-A on squeeze-test (IOPS, `daemon_cpu_ns`/op, by-class) — **OWED (parent)** |
| ~~17~~ | `perf/write-handler-economy` — **LANDED 2026-09-08** (`.benchmarks/2026-09-08-w6-write-handler-economy.md`); **field row MET 2026-09-08 (`.benchmarks/2026-09-08-campaign-rows-squeeze-test.md` §3): `rw_4k` kern 24×8 E F F E on squeeze-test +13.2 % IOPS, p50 −12.9 %, daemon CPU/op −12.2 % (`fuse3-ur` −15 % — every write ran on the fused write lane, `fuse3_zc_write_fusions` ≡ `patch_writes`), both orders agree to 0.9 %** | W-6 | W #7/#10 batched: the alloc diet (14.6 → 3.06/op, budget 4), striped per-op words (17 shared-line RMWs → 0), the zc slot's connection vehicle, the ranged gap seed | op-economy suite green at the new budget; in-process `write_handler_e2e` rows in the note; `rw_4k` CPU/op on squeeze-test is the parent's |
| ~~18~~ | `perf/owner-hop-and-depth` — **LANDED 2026-09-08 (`.benchmarks/2026-09-08-d5-owner-hop-and-depth.md`); fleet row RUN 2026-09-08 (`.benchmarks/2026-09-08-d5-fleet-squeeze-test.md`, two same-binary A-B-B-A brackets on the squeeze-test 8-co-writer fleet, both orders): the inline venue deletes the two hops exactly (queue_hop 332–459 + wake_hop 183–237 µs → 0) but `run` grows 0.6–0.9 ms (536–770 → 1,211–1,473 µs; every in-work wake an OS unpark, `sqz-jrnl` +17–25 %), co-writer publish latency +15–49 %, ingest −1.5…−7 %, verbs/s par → `SQUEEZEFS_META_SHIP_INLINE_SERVE` SHIPS OFF (the in-process win needed the artificial lane hog; the field's lanes run at ρ ≈ 0.2). The split instrument, the accept tick, the owner session lane and `TCP_NODELAY` stay; both venue levers are now measurement levers** | D-5 | DLM #7 + #8's single-connection half (#8's framing + session-pool depth landed as D-1b, `perf/d1b-publish-plane-batching`) + the accept-tick. Landed: the split `meta_ship_owner_dispatch_ns` (queue_hop / run / wake_hop / total, exact-sum, both planes); `SQUEEZEFS_META_SHIP_INLINE_SERVE` (default on — the served frame polled on the accepting connection's thread; in-process release under a 4 × 200 µs lane hog: S8 dispatch 768–781 → 40–65 µs, verbs/s +1.9×; publish dispatch 1.00–1.13 ms → 441–554 µs, publishes/s +33–35 %; quiet par); the owner session as a socket-parked lane (read-ahead, concurrent serves per connection) + the client `MuxSession` + `SQUEEZEFS_PUBLISH_SHIP_MULTIPLEX` (depth on one connection — ships OFF on measurement: the pool ran ≈ 1.5× at equal depth in-process, one owner thread vs K); the accept thread waits on the socket (dial 100.17 → 0.20 ms); `TCP_NODELAY` both ends | `owner_dispatch_ns.{queue_hop,wake_hop}` ≡ 0 on the default; `owner_dispatch_inline` ≡ dispatches; `ship_mux_frames` ≡ `ship_frames` and `ship_session_dials` 1 per authority when the multiplex lever is engaged; the fleet A-B-B-A on squeeze-test with `meta_ship_owner_phase_ns.dispatch` mean/p99 + verbs/s |
| 19 | `perf/wire-economy` | D-6 | DLM #9/#10 | after the above |
| 20 | `feat/kernel-bg-per-queue` — **kernel patch AUTHORED + compile-proven 2026-09-06 (7.2-track 0026; `docs/design-kernel-bg-per-queue.md`); `feat/kernel-bg-per-queue-backports` — the 6.19.14 FIELD-track and 7.1-track 0031 backports, compile-proven three ways each (§6 of the note); boot + A/B OWED — the field track is the row's venue** | K-1 (sqz kernel) | R-4 §7 item 1: the COMMIT-lock split — per-queue FUSE background accounting so neither `fuse_uring_queue_bq_req` nor `fuse_uring_req_end` takes the connection-wide `bg_lock` (the ledger's 1.9 µs/op contention class, paid by the worker AND every submitter) | kernel A (the track's series without the patch: 6.19/7.1 0001–0030, 7.2 0001–0025) vs B (with it: 0031 / 0026), same daemon, A A B B across reboots, thermal state recorded: `daemon_cpu_ns_by_class[fuse3-ur]`/op, `read_transport_phase_ns`, the perf ledger's `native_queued_spin_lock_slowpath` share, IOPS, clat p50/p99 — rig `.benchmarks/rigs/2026-09-06-kernel-bg-per-queue-ab.sh`; expected −1.5…−2 µs/op on the worker |
...
| ~~6~~ | kernel READ handler — **landed 2026-09-08 (R-5)** | 8 allocs/op + ~20 global atomics + ~9 clock reads (measured by site: 8 / 26 / 13 warm, 11 allocs cold zc) → 0 / 0 shared / 9 (cold zc 1 — the injected fetch's box) | `.benchmarks/2026-09-08-r5-read-handler-economy.md` |
| ~~7~~ | ranged per-op string re-parse — **landed (R-5)** | 7 allocs + 6 refcount RMWs per cold zc op → 0 | borrowed `clean_block_key_ref` / `BlockKeyRef` / `with_allocator_for_key` |
| ~~8~~ | kvmap partial resolve allocs — **landed (R-5)** | O(4096) `String`s per window fill → one arena; 2 `String`s per head parse → 0 | `KvmapWindowSpan` arena, `map_entry_block_key_at_into` |
| ~~9~~ | R2 residency — **landed (R-5)** | 5 probes, two cloning a `Bytes` → existence probes | `touch_no_promote` |
...
| ~~7~~ | per-WRITE allocs — **LANDED 2026-09-08 (W-6); field row MET** (`.benchmarks/2026-09-08-campaign-rows-squeeze-test.md` §3: `rw_4k` kern +13.2 % IOPS, daemon CPU/op −12.2 %, `fuse3-ur` −15 %) | 4 Box + 4 Arc slot closures, per-block `String`s, ~30 atomics | 14.6 → 3.06 allocs/op; 17 shared-line RMWs striped; one `Arc` per zc slot |
...
| ~~9~~ | fsync flushes ALL data namespaces + 3 serialized meta legs — **LANDED 2026-09-08 (W-5); field row MET** (`.benchmarks/2026-09-08-campaign-rows-squeeze-test.md` §4: fsync storm +23.8 % fsyncs/s, p50 −22.9 %, data sync requests 10 → 1 per fsync; `w_durable` par by construction) | was: no `fsync_phase_ns`; now `fsync_phase_ns` exact-sum + the `fsync_*` ledger (in-process: 99.8 % of an fsync is barrier wait; in the FIELD 80 % is `data_flush` — item 11) | touched-namespace barriers (stripe word stamped at the C8 closure + the in-place DMA shapes; write-through skipped) + joined legs; DUR-1 order kept |
| ~~10~~ | gap seeding re-reads whole old block — **LANDED 2026-09-08 (W-6)** | block window per seed → Σ gaps (`overlay_gap_seed_read_bytes`) | ranged seed (`SQUEEZEFS_GAP_SEED_RANGED`) |
| 11 | **fsync of a partial active block escalates the whole block** — on the 256 KiB-write + fsync storm every fsync reads a 4 MiB seed and uploads 4 MiB (`flush_seed_read_bytes` 18.4× user bytes, `durable_upload_bytes_escalation` 4.19 MiB/fsync, `patch_ineligible_adjacent` 1.00/fsync — the stream-adjacent write rides the active buffer, not the W1 in-place arm); identical on both arms of the 2026-09-08 bracket, named by W-5's `fsync_phase_ns` (`data_flush` = 5.5 of 6.9 ms) | 16× write amplification per fsync; 80 % of the field fsync | candidate: an fsync-time PARTIAL upload driven by the coverage union (the W1 sub-block DMA's shape) — a counted campaign: the seed read is what makes the later whole-block write-through correct, so the coverage-driven form must be proven against the crash contract before it can skip the seed |
...
| ~~6~~ | free-grace ack cadence — **LANDED, FIELD OWED (D-4, 2026-09-05; the hold-time campaign 2026-09-06 cut the coupled hold −10.7 % in-process and decomposed the rest — `.benchmarks/2026-09-06-free-grace-hold-time.md`)** | recycle 65 MiB/s vs 2.2 GiB/s was the PRE-campaign row; the levers landed 2026-08-25 and the in-process closed loop shows them unbinding a recycle-bound stream at zero fences (`.benchmarks/2026-09-05-d4-free-grace-sustain.md`) | the from-zero s11-mpiio row with the sustain columns is the parent's |
| ~~7~~ | owner `spawn_meta_join` hop — **LANDED 2026-09-08 (D-5); fleet row RUN, lever ships OFF**: split first (`meta_ship_owner_dispatch_ns`), then executed on the accepting thread (`SQUEEZEFS_META_SHIP_INLINE_SERVE`); in-process under a lane hog the S8 dispatch fell 768–781 → 40–65 µs, both hops 0 by construction — but on the fleet (`.benchmarks/2026-09-08-d5-fleet-squeeze-test.md`) the hops it deletes (0.5–0.7 ms) are smaller than the `run` it adds (+0.6–0.9 ms: in-work wakes become OS unparks), publish latency +15–49 % | ≤ 32 µs vs ≤ 8 µs read execute quiet; on the fleet the hop is 0.5–0.7 ms of a 1.1–1.5 ms dispatch (C-2's 2.0–2.3 ms read the pre-D-1c/C-2 fleet) | the hop stays the default; the remaining term is `run` itself — the served work's own conveyor wait (`tx_queue_wait` 170–300 µs) and commit, i.e. the verb-plane grouping rung + D-6 |
| ~~8~~ | stop-and-wait frame depth 1 — framing + depth **D-1b**; the single-connection half **D-5** (`MuxSession` + the owner's socket-parked session lane; `SQUEEZEFS_PUBLISH_SHIP_MULTIPLEX`) | — | pipelined on one socket |

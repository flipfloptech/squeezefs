...
| S5 **item 3** | **The freed-offset grace period — LANDED 2026-08-06** (`src/free_grace.rs`; `tests/reader_free_grace_tests.rs`; operator surface `docs/operations.md` §Freed-offset grace period). §6.8's *"highest-value single item in the coherence analysis"*: a terminally-freed offset is held out of the free list until every live registered reader has acknowledged passing it, enforced AT the allocation funnel (`finish_free` defers; the funnel harvests). §6.3's cross-file staleness is therefore ELIMINATED with the plane armed — bounded, unchanged, with it off (the default) | the silent-passthrough window closing is a **two-host** claim: a writer reallocating under a reader while the reader reads the victim file, with `free_grace_forced_releases == 0` and byte-comparison of the reader's serves — **DEFERRED per D11** alongside every other measured row. The in-process contract is pinned instead (14 contracts: the gate, the bound, the fence, the ENOSPC ruling, unarmed byte-identity, reclaim + S7-quarantine composition, both derivations, and the reader ladder's placement) | Channel = **S6** (`ack_free_epoch` → `min_acked_free_epoch` / `members_behind_free_epoch` / `evict`) — a reader performs no metadata write, which is what blocked item 3 until S6 landed. Epoch identity = the owner's own monotonic instant, echoed as a CAUSAL token (the per-volume ledger sequence cannot be the scalar: min/max composition across N independent counters starves in an unbalanced set — the 2026-07-30 field shape). Deliberately **not** a second S7 quarantine (cohort + external drain proof vs FIFO + self-advancing bound); the enforcement SEAM is shared and the two compose custody-first. Laggards are fenced on a derived bound (2 × ack cycle; an override below one cycle refuses), space pressure is ENOSPC-not-corruption with a shorter pressure deadline, and unarmed is one relaxed load. No incompat bit: this is a runtime protocol, not a format change |
| S6 | Membership off the journal (lease-based liveness) — **LANDED 2026-08-05** (`src/membership.rs` + `src/membership_wire.rs` + `src/membership_sim.rs`; `tests/dlm_membership_tests.rs`; operator surface in `docs/operations.md` §Membership plane) | volume-0 journal tx/s → ~0 at 15 k **simulated** clients — **the MEASURED half is DEFERRED per D11**; the in-process contract is pinned instead (500 renewals move `meta_kv_journal_entries` by 0, with a contrast arm proving the counter is live), and the harness that produces the row is landed | Mechanism: durable registration written ONCE (the `membership_owner` rendezvous record; §6.2 item 7's `claim_set` on membership CHANGE, incompat **bit 14** built-not-stamped per D9) + liveness by lease renewal over `cluster_wire` with the lease table in RAM — *durability of liveness is a category error*, the rejected slab/gossip alternatives are argued in the module docs. Read side: ONE `getxattr` + a paged RAM census (readers visible for the first time, with zero metadata writes). Two clocks with the client's strictly stricter (`T_self = T_owner − 2·skew_max − D_purge`, unsafe configurations REFUSE). Owner failure = re-assertion + grace window. The D1 harness (`membership_sim`) lands here and runs at a few hundred clients; the **15 k row is deferred evidence** and reports volume-0 journal tx/s, renewal latency distribution, revoke fan-out and failover grace completion — `SimReport::render` prints exactly that. §6.8 item 3 consumes this plane's API verbatim (`ack_free_epoch` → `min_acked_free_epoch` / `members_behind_free_epoch` / `evict`) and landed on it — see the S5 item-3 row above |
...
S4 gate → S5..S11 ; S6(channel) → S5 grace period(LANDED) → S9 ; S7 → S9 ; S9 → S10 → S11

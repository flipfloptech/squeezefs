...
| **Heap-full sweep** | `checkpoint_cycle`, after the flush pass, before the bitmap write / barrier (so this cycle's barrier covers the merges) | runs while `heap_full` is latched OR a `merge_backlog` stands; `KvTree::merge_underfull` walks this tree's RESIDENT nodes **level by level from the leaves up**, each level in key order (no device reads for the census; a candidate's sibling is demand-paged), exact `f` per node, the pair law per adjacent pair, greedy left-to-right packing (a successor keeps merging rightwards while the pair law admits), the climb after each merge, and the root-collapse chain at the end. The interior levels are walked in their own right, not only through the climb: the separator tombstones a leaf merge mints sit ABOVE the durable tail until the next barriered cycle covers them, so a parent that lost most of its children projects underfull only on a LATER sweep — one that runs no leaf merge and therefore never climbs | O(resident nodes) fold walks per cycle, ZERO in steady state (the posture gates it); merges until the compaction floor refuses (`NoSpace` ⇒ backlog stands, sweep resumes next cycle) | forced retirement |
| **D4 defrag arm** | `JobType::DefragMeta` (`jobs::run_defrag_meta`) after the compaction nudges | `dead_bset_census` gains `merge_candidates` (leaves with `f ≤ ¼·C`, over the paged-in population) and the report/gauge face `mergeable_leaves` / `frag_d4_mergeable_leaves`; `KvMetaBackend::defrag_merge_leaves` runs `merge_underfull` per tree on the SMO mutex | the whole-tree occupancy sweep — the operator's cold-leaf recovery, duty-cycle throttled like the nudges | FIFO valve (`PendingFreeFull` / reserve exhaustion ⇒ one forced cycle + retry, the nudge's ladder); `NoSpace` at the floor ends the pass for this volume (the cycles return the extents; the next run continues) |
...
**(h) Gauges** (stats inode, appended at the end of the `meta_kv_*` group): `meta_kv_node_merges` (leaf/interior merges executed — the engagement gauge; moves on any delete-heavy workload), `meta_kv_root_collapses` (height decrements), `meta_kv_merge_candidates` (per volume, LIVE: underfull leaves the last sweep/census found), `meta_kv_merge_sweeps` (per volume: heap-full/backlog sweeps run), `frag_d4_mergeable_leaves` (the D4 face: mergeable leaves across the set's meta volumes at the last defrag measure). `MaintenanceOutcome` gains `merges` / `root_collapses`.

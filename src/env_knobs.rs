...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_IPC_DD_EAGER_FLUSH", int(0, 1 << 12), "0 (sweep-only)", "Direct-drive issue cadence (third adjudication, write-wall Addendum 7): explicit K > 0 enters inline once a lane's unflushed tail reaches K — K=16 is a COUNTED +4-8% at t32qd32 on the fabric venue (849/847k vs 776/782k and 839k vs 808k, two A-B-B-A windows) now that the system is RTT-limited (Addendum 6); absent/0 = sweep-only. The inflight/2 derivation was falsified in-bracket, so per the no-fixed-constants law the default stays sweep-only until the ProbeCore cadence governor (board) derives it from delivery."),

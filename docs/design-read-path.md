...
4. **Red blocking parked admission** (`parked_gate_waits` / `parked_gate_timeouts`). At Red the halved parked cap is a real bound: a writer whose spill was refused **awaits the never-lossy drain** (posts a byte target, kicks the shared drain worker, sleeps on its progress notify — awaited backpressure in async context per the §4.4-pt-5 ring-admission precedent; no spin, no lock held *acquired* while waiting, deadline-bounded at 10 s with a loud counter so a wedged drain can never wedge the write path). The drain worker acquires leases (2) and other blocks' flush locks (3) in a clean task context and never needs the waiter's own block (not yet in the map) — the P1-9 order is preserved by construction. Below Red the cap keeps its historical soft spill-or-keep semantics.
...
| `mem_budget_unreclaimable_bytes`, `mem_budget_hard_backstops`, `mem_budget_backstop_active`, `mem_budget_tier_publish_paused`, `read_tier_publishes_paused`, `parked_gate_waits`, `parked_gate_timeouts` | §5.7 Red-semantics escalations (finding #2): the unreclaimable arm and its three responses. `hard_backstops` or `parked_gate_timeouts` growing on a quiet workload = convergence regression; `parked_gate_waits` > 0 is designed behavior under genuine Red |

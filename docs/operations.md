...
PR 7b (2026-09-16, `.benchmarks/2026-09-16-sym-pr7b-dir-stripes.md`) lands **directory STRIPING** on PR 4's slot leases, PR 5's tokens and PR 6's cross-owner intents (design §5.6.5, KD-SYM-20; owner ruling R-SYM-5 — the Lustre DNE-2 shape). Dark behind bit 17 AND `SQUEEZEFS_SYMMETRIC_META=1`; `=0`, a bit-17-absent volume and `SQUEEZEFS_SYM_DIR_STRIPES=1` are the shipped posture exactly (no flip, no map, every Striping gauge 0 — pinned).

- **What a striped directory is.** A hot shared directory `D` spreads its dentries over `K` **stripes** — `K` stripe inos `D_0 … D_{K-1}`, ordinary `S_IFDIR` records with no name of their own, each minted in a DIFFERENT node's slot on `D`'s volume — with `stripe(name) = hash54(name) % K` (the volume's seeded dentry hash, so every stripe's entries and the K-way `readdir` merge share ONE order) and stripe `i`'s dentries keyed under `D_i` in `D_i`'s slot tree. Stripe ownership IS slot ownership: a create into `D` is one shipped `InsertDentry` to the name's stripe holder (`dir_stripe_ships`), so `N` creators spread over `K` holders instead of one, and a create recalls only its stripe's readers. **The stripe map is `K + 2` reserved-name dentries in `D`'s own tree** — `\0sqz.stripe:NN → D_i`, `\0sqz.migrating → D`, `\0sqz.striped → D` (the COMMIT marker; the map is live iff it exists) — names whose leading NUL byte no user operation can ever pass (the VFS refuses it), filtered from every listing. The map therefore rides `D`'s token verbatim and C9 sees a stripe as REFERENCED by its map entry; no slot-tree key kind is added (bit 17's kind set stays PR 1 + PR 7's — what this rung adds to the bit's meaning is the reserved NUL name space inside the dentry kind).
- **When a directory flips.** (1) **Automatically** at the holder: when the foreign creates it served into `D` over one `T_idle` window (`SQUEEZEFS_SYM_T_IDLE_MS`) exceed `N_floor` (`slot_offer_n_floor`) AND come from MORE THAN ONE creator (the creator is read off the shipped child's slot) — one foreign creator, however many creates, is a HANDOVER candidate, never a striping one. (2) **Explicitly**: `setfattr -n user.squeezefs.stripes -v 64 <dir>` (a COMMAND the holder executes, never a stored value; `getfattr` answers the `K` in force; a directory another appender holds answers `EAGAIN` naming the holder). (3) **At `mkdir`** under `-o stripe_dirs` (the DNE-2 default; every new directory on an armed volume). `K` = `SQUEEZEFS_SYM_DIR_STRIPES` (derived 64), never re-striped.
- **The flip (§5.6 intent protocol).** The holder asks the `K` heaviest foreign creators it observed to each SUPPLY a stripe ino minted in their own slot (`SupplyStripeIno`, `dir_stripe_supply_rpcs` — a creator that declines or is gone is replaced by the next; the remainder is the holder's), then writes the map as ONE PR 6 intent of `K + 2` marker inserts with the commit marker LAST: a reader sees either no map or the whole map, and a kill at any step rolls forward to the whole map at the slot's next lessee (pinned at every window). From that commit `D`'s EXISTING names re-home **lazily**: a lookup that misses the stripe falls back to `D`'s own tree while `\0sqz.migrating` stands, the holder moves names to their stripes in the background (`dir_stripe_migrated_names` — insert into the stripe first, remove from `D` second, so a name in BOTH homes is served from the stripe: the LWW rule), and a migration the holder died inside resumes at the directory's next access by its slot's lessee.
- **Every operation on a striped directory.** `lookup` / `create` / `unlink` / `link` / `rename` route `(D, name)` to the stripe `hash54(name) % K` names — the ordinary op with the stripe as its key parent (cross-owner through PR 6 when the stripe's holder is another appender; a rename within `D` across two stripes is the two-parent rename). `readdir` is the K-way merge of the stripes in the shipped `(hash54, coll)` cookie order — the cookie of the last entry is the exact continuation, so a name present for the whole scan is listed exactly once under concurrent creates/unlinks (`dir_stripe_readdir_merges`). `stat D` folds the stripes: `nlink = D.nlink + Σ (D_i.nlink − 2)`, `mtime`/`ctime` = the newest — each stripe's record IS the exact delta its own inserts wrote in the same transaction as the dentry, so nothing ships per insert and no delta can be lost; the fold is persisted onto `D`'s record once per cadence when a stripe's time passed it (`dir_stripe_time_batches`). **`rmdir D`**: under `D`'s exclusive guard and the mutation permit the holder MARKS every stripe dying (`nlink 2 → 0`, one intent), probes each `IsEmpty` UNDER the mark — an insert into a dying stripe is refused `ENOENT` at its holder (`dir_stripe_dying_refusals`), so a create that raced the probe is either seen or refused, never orphaned — then removes `D`'s name, the markers and destroys the stripes (`DestroyStripe`; a stripe the process did not reach is an `nlink 0` corpse the next mount's sweep reclaims). A non-empty stripe under the mark REVIVES the set and answers `ENOTEMPTY`.
- **What it costs.** A create into a striped directory is one shipped insert (+ the intent's own entry and barrier) unless the name's stripe is this node's (`1/K` of them); `ls` is `K` stripe pages per `readdir` page (64 RAM-resident range reads per page on the holder, `K` token fetches cold on a reader); `stat D` reads `K` stripe records (RAM); `rmdir D` is `K` marks + `K` probes + `K` destroys (~3 K verbs, rare). The local wave row (7 declared holders, K = 14, 2,000 creates): every foreign create one ship, per-stripe population 113–170 against a mean of 143 at 7 holders (20–47 against 32.3 at 31 holders, K = 62), no handover — the note carries the figures.
- **Crash windows (as built).** A stripe supplied but never recorded (a kill before the intent) is C9's class — an unreferenced `S_IFDIR` record the census cleans. A kill inside the flip's intent rolls forward whole. A kill between a name's stripe insert and its removal from `D` leaves it in both homes — the stripe wins, the resumed migration drops the duplicate. A kill between an `rmdir`'s mark and its revert leaves the stripes DYING: creates into `D` refuse `ENOENT` until the next `rmdir D` attempt, which revives them (`ENOTEMPTY`) or completes the removal.
- **fsck C17 — stripe consistency** (report-only, the C8 posture; `fsck_stripe_findings` **must stay 0**): a stripe named by more than one directory's map, a map entry naming a missing stripe (also C10's dangling-dentry face), a commit marker over an incomplete map, a dentry in stripe `i` whose `hash % K ≠ i`, a name left in `D`'s own tree after `migrating` cleared. The marker census rides the ONE dentry walk C9/C10 already run; only the striped population is re-read.
- **Gauges** (`.stats`): the Striping family — `dir_striped_dirs`, `dir_stripe_flips`, `dir_stripe_supply_rpcs`, `dir_stripe_ships`, `dir_stripe_readdir_merges`, `dir_stripe_migrated_names`, `dir_stripe_time_batches`, `dir_stripe_dying_refusals`, `dir_stripe_rmdirs`, `fsck_stripe_findings` — see [Observability](#observability); every one 0 on an unarmed mount, pinned.
- **Owed by the later rungs**: the per-STRIPE token on a wire reader (PR 5's token client fetches a stripe like any object; the reader's `readdir` merge over token pages is PR 12's join ladder), N daemon processes as stripe holders (PR 12 — the local shape's holders are the declared regions), re-striping (a named follow-on), the `sym-shared-dir` box rows (PR 13).

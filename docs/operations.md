...
| `SQUEEZEFS_META_SHIP_INLINE_SERVE` | bool | `on` | D-5: the owner executes a served dispatch (S8 frame; S9 publish call / group / free / harvest) ON the accepting connection's thread — the two cross-thread hops onto and off the shared `sqz-meta` lanes are gone (0 by construction) and every wake inside the work unparks that thread directly. `0` = the shipped `spawn_meta_join` hop (A/B control, never an operational escape). |
...
| `SQUEEZEFS_META_SHIP_INLINE_SERVE` | `on` | OWNER side, both planes (D-5 — DLM #7, the owner's dispatch hop): a served dispatch — the S8 verb frame, the S9 publish call / group / free / harvest — is polled on the accepting connection's own thread (`sqz-clw-conn`, dedicated and parked for exactly that reply) instead of being hopped onto the two shared `sqz-meta` lanes and joined. The two cross-thread hops are gone (0 by construction) and every wake inside the work — the conveyor's fan-out, a 4a guard — unparks that thread directly instead of queueing behind the lanes' serve/publish storm (C-2's fleet attribution: 2.0–2.3 ms per verb vs ≤ 32 µs quiet). An unwinding verb still answers `PANIC` and the session serves on. `0` = the shipped hop (the same-binary A/B control). Live: `meta_ship.owner_dispatch_inline` (≡ every dispatch on the default) / `owner_dispatch_hops` (carries them under `0`); attribution `meta_ship_owner_dispatch_ns`. |
...
owner's dispatches — `inline` ≡ every dispatch on the default, `hops` carries
them under `SQUEEZEFS_META_SHIP_INLINE_SERVE=0`), and the `dlm_token_cache_*`

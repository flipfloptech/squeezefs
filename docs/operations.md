...
squeezefs volume enable-symmetric sqmeta://<dev0>,<dev1>,... --dry-run  # print the plan + every refusal; write nothing
squeezefs volume enable-symmetric sqmeta://<dev0>,<dev1>,... --resume   # continue a crashed run from its crash point
```

**Procedure.**

1. **Preflight.** Unmount EVERY mount of the set (writers, `-o ro` readers, co-writers — the verb refuses a heartbeat-fresh client, and a reader left mounted across the flip would keep reading the old layout until its next open). Run `--dry-run` first: it prints, per volume, the record count and bytes that would move, what would be **DROPPED** (below), and every refusal — without writing a byte (pinned: sector 0, the ledger, the ring and the bitmap are byte-identical after a dry run).
2. **The refusals** (all loud, all naming the remedy, all checked before anything is written): the set is **already symmetric** (every volume carries bit 17 and no marker — nothing to do); a volume's newest ledger record carries a **bit-8 NON-SOLO partition suffix** (its last writer was a multi-appender era whose slot arithmetic is not a solo mount's — mount the set solo once, unmount cleanly, re-run); an **open cross-volume intent** on any volume (a half-applied `rename`/`link` — mount once so it rolls forward at open, then re-run); an **in-flight `job:` record** (a paused/queued/running maintenance job would resume onto a layout it did not plan for — let it finish or `squeezefs job cancel`); a **live client**; a **marker without `--resume`** (a crashed run must be acknowledged); `--resume` with nothing to resume.
3. **The run.** The verb writes a `sym_upgrade:` marker on ino 1 of EVERY volume first, then converts the volumes one at a time in canonical member order. From the moment a volume carries its marker, **every writable mount of the set refuses** at the D0 gate — before it takes a claim, so a refused mount writes nothing — naming this verb and `--resume`; read-only mounts and probes keep serving the trees the volume's layout bit names (the old ones until the flip, the forest after). Per volume the durable steps are: quiesce under the D0 ladder (a guarded open, a checkpoint, a clean shutdown — so a live foreign writer is refused by the ladder, never converted under) → read every live record through a probe → write the forest into FRESH extents (one mixed-kind tree per routing slot, the control tree naming every guest root `Unleased` with its cursor, the appender directory, the fixed ring reset with appender 0's page) → ONE ledger write naming the old roots AND the forest roots → ONE sector write stamping bit 17 + the directory (the layout flips onto a ledger that already describes it) → the old trees' extents returned and folded out of the ledger → the marker removed through the forest's own first mount. Progress prints per volume: records, bytes, slot trees, records/s, MB/s, extents written / freed / reclaimed.
4. **A crash.** Whatever the instant, the set refuses writers (the marker is present on every unconverted volume and, copied into the forest, on a converted-but-unfinished one) and readers keep serving. `--resume` re-runs the verb with the SAME URI: each volume is resumed from its DURABLE state — a flat volume under its marker is rebuilt (a crashed build's fresh extents are reclaimed first, so nothing leaks — pinned: the resumed volume has exactly the free extents of a straight conversion); a stamped volume whose ledger still names the old trees frees them; a stamped volume with only the marker left drops it. The five windows (after the markers, mid-build, after the hybrid ledger, after the stamp, after the free) are each pinned in `tests/sym_convert_tests.rs`.
5. **Afterwards.** Mount the set: every volume is a forest (`.stats` `meta_kv_forest_*`, `appender_*` families populate; the old `meta_format_version` stays 3 — the forest is a relayout of the same records, and its **post-fold digest equals the source's** by construction). Run `squeezefs fsck` once; it is clean on a converted set (pinned incl. C8 drift 0 and C13 empty). The `.stats` `appenders_known` reads 1 until PR 4's leases put other appenders on the volume.
...
squeezefs volume enable-symmetric <sqmeta-uri> [--dry-run|--resume]
...
- `enable-symmetric` converts every volume of the set from the three shared per-kind trees to the slot-tree forest (incompat bit 17) in one crash-resumable invocation — bracketed per volume by the `sym_upgrade:` marker, which refuses every writable mount until that volume is finished (`--resume` continues a crashed run; read-only mounts keep serving). The program it belongs to is DARK until its PR-14 flip: nothing converts by itself. Procedure, refusals, what is dropped and the two crash windows: [Converting an existing set to the symmetric forest](#converting-an-existing-set-to-the-symmetric-forest-squeezefs-volume-enable-symmetric).

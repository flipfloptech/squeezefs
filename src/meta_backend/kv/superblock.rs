...
///
/// **Second consumer (S9 blocker #3, no new bit — RETIRED at PR 14):** the
/// data-plane allocation partition (`docs/design-mw-data-alloc-partition.md`)
/// gated on this bit too, because its durable per-lane reservation records
/// were precisely "the format's recovery paths expressed for more than one
/// data-plane writer". The symmetric default flip replaced the partition
/// with PR 8's ranged block grants from a per-volume allocation lease
/// (`crate::block_grant`, `crate::data_alloc_bitmap`); the join ladder's
/// rung 2 reads this bit as one of the nine the forest presumes.
///

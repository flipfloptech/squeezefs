...
3. **Transport zero-copy**: in the vendored `third_party/fuse3`, deliver FUSE_WRITE payloads as `Bytes::from_owner` leases over the registered uring payload buffer (killing a 1 MiB alloc + 2 MiB of memcpy per request), with a **deferred COMMIT_AND_FETCH re-arm protocol** that guarantees a leased buffer is never re-posted to the kernel while the lease lives, and an explicit **lease-severance boundary** (§5.4) that bounds every lease's lifetime to one handler invocation — no lease can reach a long-lived cache.
...
stateDiagram-v2
    ...
    Accumulating_Fresh --> Accumulating_Fresh: contiguous/overlapping write<br/>(merge copy, extend covered)
    Accumulating_Fresh --> Accumulating_Seeded: gap write → zero the gap<br/>(degrade to fully-initialized)
    Accumulating_Seeded --> Accumulating_Seeded: merge copy
    Accumulating_Fresh --> ContentComplete: write_end == block_end<br/>(zero uncovered complement first —<br/>elided when covered == full)
    Accumulating_Seeded --> ContentComplete: write_end == block_end<br/>(today's is_block_complete trigger)
...
- **Write-through trigger — normative, one definition**: fire exactly when `write_end == b_end_offset` — **byte-identical to today's `is_block_complete` (`:1393`) for every entry kind and every fill order**, so the completion point never moves. Content-validity is established *at* the trigger, not required before it: Seeded entries are always content-valid; a Fresh entry whose trigger arrives with `covered ≠ [0, block_size)` first zeroes the uncovered complement, then fires (sound because "Fresh" means no prior data exists for this block — the complement's correct content *is* zeros; today's unconditional seed-time zero-fill produced the same bytes). `covered` is therefore **pure memset-elision bookkeeping, never trigger input**: sequential fills reach the trigger with `covered == full` and memset nothing; tail-first fills (`[2M,4M)` first) zero `[0,2M)` at trigger time and stage/upload immediately, exactly as today; a middle-last fill (`[0,2M)` arriving after a tail-first block already published) re-seeds a *Seeded* entry from the published block via RMW, as today. An earlier draft let `covered == full` also fire the trigger — dropped: with complement-zeroing at block-end writes, that disjunct is unreachable for Fresh entries (any earlier block-end write already fired) and would have silently moved today's staging point for the cases it did reach. The PR 4 test matrix pins tail-first, gap-write, and middle-last orderings explicitly (see PR Plan).
...
### 5.4 PR 5: transport zero-copy (vendored `third_party/fuse3` — edit in place)
...
| `is_aligned` direct leg (`fuse_client.rs:2851-2855`): `data.clone()` → `router.write_file`'s striped section, whose per-block tasks hold `data.slice_ref` slices across allocate + DMA (`routing.rs:1992-1997`, `:2010-2070`); dead for the default shape but **live for small-block configs** (`tests/data_path_correctness_tests.rs` runs `block_size = 64 KiB`) until PR 6 deletes the branch | task-scoped (bounded — pre-slice-reuse the LRU gets the pooled copy at `:2054-2058`/`:2076`), but pins leases across device writes: the exposure Alternative C is rejected for | **Must never see a lease** — severed inside the branch (`:2851`); transitional scaffolding that PR 6 deletes together with the branch |
...
**Loom model (required — new lock-free protocol):** per the house extracted-core convention (`loom-models` `#[path]`-includes shipped sources — `incarnation_core` precedent), the refs/parked/re-check word protocol is extracted into a dependency-free `third_party/fuse3/src/raw/connection/lease_core.rs` with `#[cfg(loom)]`-switched atomics, `#[path]`-included by `fuse_over_uring.rs` and by `loom-models/src/lib.rs` — the model checks the exact shipped code, not a hand copy. Model `ent_lease`: thread A = lease drop (refs 1→0, parked check), thread B = worker (refs load, park, re-check). Invariants: the commit executes **exactly once**, never while `refs > 0` — **including the shutdown header-only path** (modeled as a third interleaving: shutdown drain vs late lease drop). `tests/run_loom.sh` joins PR 5's gate.
...
| `third_party/fuse3` `fuse_over_uring.rs` + `lease_core.rs` (new, extracted protocol core) | `PayloadArena`, `EntLeaseState`, `EntPayloadLease`, parked-commit drain in `queue_worker`, shutdown header-only replies; `InboundUringReq.payload` semantics: may be a lease (FUSE_WRITE) or a copy (all else) — type stays `Bytes` |
| `third_party/fuse3` `tokio.rs` / `session.rs` | opcode-gated body-copy skip; `handle_write` size check validates against `payload.len()` on the uring path |
...
| `transport_payload_leases`, `transport_parked_commits` | PR 5 adoption + parked-ent pressure (parked ≫ 0 ⇒ handlers hold payloads too long or Q_DEPTH too small) |
...
- **Files**: `third_party/fuse3/src/raw/connection/fuse_over_uring.rs` (PayloadArena, EntLeaseState/EntPayloadLease, opcode-gated lease at delivery, parked-commit drain, shutdown drain with header-only replies), `third_party/fuse3/src/raw/connection/lease_core.rs` (new — extracted refs/parked protocol core), `third_party/fuse3/src/raw/connection/tokio.rs` (FUSE_WRITE body-copy skip `:462-499`), `third_party/fuse3/src/raw/session.rs` (`handle_write` size check vs `payload.len()` `:2461`), `src/fuse_client.rs` (**`sever_payload` at the two router-route boundaries** in `write()` + transport stats plumb-through), `loom-models/src/lib.rs` (`ent_lease` model `#[path]`-including `lease_core.rs`), tests: `tests/multi_queue_tests.rs` extensions (write storms with held payloads, FORGET storms interleaved, clean umount under parked ents), **single-queue starvation test: small-file (`echo`-class inline/staged) write storm on one queue with `Q_DEPTH=4` — no stall, `transport_parked_commits` ≈ 0, `transport_lease_max_age_ms` bounded** (the §5.4 sinks table, tested row by row — **including a small-block-config (`block_size = 64 KiB`) aligned write exercising the transitional `is_aligned` sever** for the PR 5→PR 6 window), transport unit tests in the vendored crate.
...
- **Gate**: large-seq additive improvement (target cumulative ≥ 3×, ≥ ~1.3 GB/s); small-write ops/s at-or-better (severance replaces two copies with one — strictly less work than today); `transport_parked_commits` ≈ 0 steady-state; `transport_leases_outstanding` returns to 0 at quiesce; umount/EBUSY storm suites green; `tests/run_loom.sh` green; root suites re-run.

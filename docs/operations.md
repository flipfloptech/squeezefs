...
- `SQUEEZEFS_FUSE_ZC_READ_FUSION` (**default on** — e2e perf audit R-3, read board #3's zc-leg form; `.benchmarks/2026-09-03-r3-fill-issue-economy.md`): handler/worker fusion for armed FUSE READs at or under the fusion ceiling — the READ handler runs on the reaping queue worker's fused lane, so the zc direct leg's fetch message, its CQE resume and its prefilled COMMIT are same-thread channel ops instead of three cross-thread wakes (the 4k-random attribution measured them at 35–47 µs each around a 40 µs DMA: `zc_bridge_phase_ns` `msg_hop` / `device_cq` − device / `wake_hop`). Reads above the ceiling (`SQUEEZEFS_FUSE_ZC_FUSION_MAX`, the same bound the write lane uses — a WARM serve copies the request's size on the worker) keep the handler lanes. Local tcp-devsub kern rand-4k 24×8 A-B-B-A: +17 % IOPS, −29 % daemon CPU/op, `msg_hop` 32 → 0.6 µs, `transport_wake_writes` 1.57 → 0.62/op. `0` = the classic handler-lane dispatch (the A/B control). Engagement: `fuse3_zc_read_fusions` ≈ every kernel READ on an armed session (the 1 MiB stream rows read 0 by the ceiling); `fuse3_zc_read_fusion_demotions` ≈ 0 steady-state.

...
- ~~**Sibling merge of underfull nodes is deliberately out of v1** (deletes leave slack until natural compaction; space cost bounded by the fill-factor accounting in §3); the SMO framework accommodates it later without protocol change.~~ **Built 2026-09-11 as §4.6a** — the underfull-sibling SMO rides this protocol verbatim (three steps, parent-then-children locks, reserve-in-window / write-after-release, two-phase replay, coverage-gated pending-free); the sentence stands struck because the 2026-09-11 full-volume fix (§4.7 amended) made its cost visible: a volume that is 90 % deleted could not mint one new leaf.
...
### 4.6a Leaf merge — the underfull-sibling SMO (2026-09-11)

**Why.** The 2026-09-11 metadata-full fix (§4.7 amended; `.benchmarks/2026-09-11-post-123-board-items-2-4.md` §1) made a full heap answer `ENOSPC` instead of fail-stopping, and stated the limitation this section closes: the v1 tree never merged, so a delete returned record space INSIDE its leaf and never an extent — after a fill, creates resumed into the room the deletes had freed and hit `ENOSPC` again the moment a new leaf was needed, on a volume whose leaves were nearly all empty. The law now reads: **a volume that is 90 % deleted keeps creating.** Everything below is the §4.6 SMO framework applied to two adjacent siblings — no new lock population, no new journal record kind, no format change, no new incompat bit — and it is normative for `KvTree::smo_merge` / `smo_root_collapse` / `try_merge_node` / `merge_underfull` (`src/meta_backend/kv/tree.rs`); contracts `tests/kv_leaf_merge_tests.rs`.

**(a) The candidate law — the split's constant read backwards, no new one.** Let `C = NodeLayout::fold_capacity()` (the encoded record bytes one image holds — the compaction-vs-split line) and `f(N) = NodeSnapshot::fold_bytes_upper_with(&[], ·, durable_tail).0` (the K1 fold's byte upper bound over `N`'s serialized log + open delta + frozen delta — the §4.7 heap admission's own estimator, here with the cache's durable tail so a tombstone the compaction fold will elide (`seq < tail`, the §4.2 rule) is credited as gone, since a mostly-deleted leaf IS small once its tombstones are covered; the admission keeps passing `0` — its conservative posture is unchanged). Two nodes `L`, `R` at one level, ADJACENT under ONE parent (`R.min = key_successor(L.max)` — K2's gap-free partition), are **mergeable iff**

```text
f(L) + f(R) ≤ ¾·C                    (the pair fits a fresh split part — the fill target)
∧  min(f(L), f(R)) ≤ ¼·C            (one of them is UNDERFULL — the trigger bound)
```

`¾·C` is `NodeLayout::split_part_capacity()` — the K2 split's fill target, so a merge successor is exactly as full as a fresh split part and carries a part's append headroom. `¼·C = ¾·C − ½·C` (`NodeLayout::merge_candidate_capacity()`): the fill target less the split's **balance point** — `split_node` cuts a fold `> C` at its byte midpoint, so every fresh half holds **more than ½·C**, and an underfull leaf is one that merges with ANY sibling up to half full. Both bounds are derived from `C` alone and tie-tested (`tests/kv_leaf_merge_tests.rs`, the `derivation_sweep_tests` drift-is-red convention). **Hysteresis is structural, not tuned:** a fresh half (`> ½·C`) is never a candidate and never completes a pair with one (`> ½·C + ≤ ¼·C > ¾·C`), so a split is never undone by the next merge; a merge successor (`≤ ¾·C`) needs `> ¼·C` of growth before its fold can exceed `C` and split again. The underfull bound is the trigger's cheap face; the pair rule is what admits the merge.

**(b) The protocol** is §4.6's three-step replacement over TWO frozen sources:

1. **Build, no locks held.** Freeze both nodes under their own write locks (an empty open delta enters `FREEZING` through `begin_forced_freeze` — the `compact_node_forced` shape — so the swap sees freeze-borne sources); load both disk images; run the FIND-VS-A fold-source guards on BOTH (`node_seq` and log-view `tail_offset` must match the live objects — never fold a stale incarnation). Fold both logs + both frozen deltas with the ONE K1 algebra (`bset::compact` over the concatenated newest-first views — the key spaces are disjoint, so cross-node view order is immaterial and per-key LWW is unchanged); re-check the pair law on the exact fold (a candidate can go stale between census and build — that is a clean decline, never an error); claim ONE extent (internal class, admitted at the **compaction floor** — (d) below); write the successor image with `level = L.level`, `min_key = L.min`, `max_key = R.max` (gap-free by K2 — the successor covers exactly the union); load it back; **barrier** (production) — the successor is durable before any pointer record to it can exist in the ring (the split's law, §4.10).
2. **Lock window: parent first, then both children in ascending NodeId (extent address) order.** The commit path locks leaves in ascending NodeId and never waits on an interior lock (§4.4 pt 1, §4.9 4b); the SMO takes the parent (interior, SMO-only) and then the two leaves in the SAME ascending-address order the commit path uses — NOT key order, which is a coincidence of allocation — so a multi-leaf transaction holding one sibling and waiting on the other can never form a cycle with the SMO. Inside the window: **reserve** the entry from the checkpoint-task ring reserve (§4.4 pt 5 — admitted BEFORE any lock; the in-lock `reserve_registered` cannot block); `take_overlay()` from both (the bounded second merge — records that landed during the build move to the successor with their exact per-record `entry_floor` stamps, the §1b discipline); `supersede()` both (snapshots left intact for in-flight readers); publish the successor; apply the parent's records in ONE `apply_locked` (one snapshot swap — a latch-free reader sees the old routing or the new, never half); retire both old objects LAST (the K7 route-flip-before-retire ordering); release. The interior-pointer record shape is **`Put(R.max → (succ_addr, succ_seq))` + `Delete(L.max)`** — the existing §4.2 separator vocabulary, no new kind: the `Put` REPLACES the right sibling's separator (same key `R.max`, higher seq — per-key LWW), the `Delete` tombstones the left's, and `next_live(key)` for any key in `[L.min, L.max]` skips the tombstone and lands on `R.max → succ`. The entry is `[Put, Delete, alloc(succ), free(L), free(R)]` in that order, seqs `res.start + i`: the flips are records 0–1, so the parent's floor pins at `res.start` (§1b's entry-start boundary), and the last free record is the entry's highest seq — the §4.7 coverage gate for BOTH retirements (`tail > free(R).seq ⇒` every record of the entry is covered). **Replay phase 1 orders it correctly relative to another SMO of the same parent** (the SMO task is serialized, so "concurrent" means only "in the same replay window across a crash"): phase 1 applies (level DESC, then seq) — a split of the PARENT journals level-`L+2` flips that route to the parent's successor and apply first, so our level-`L+1` records land on the parent's FINAL lineage; another leaf SMO under the same parent journals level-`L+1` records at other keys, or at `R.max` with a higher seq if it later split the successor — per-key LWW by seq resolves both exactly as it does for splits today. The `Delete` tombstone follows the §4.2 compaction rule (elided from a later interior compaction only once `seq < durable tail`).
3. **After release.** Write the entry bytes (`commit_entry`; a failed write is the §4.4 pt 4 crash-equivalent hole, loud never fatal — verbatim `smo_replace`); park BOTH old extents in pending-free gated on the entry's last seq — forced retirement under the flush pass / heap-full sweep (the §4.7 cycle-break's posture), the FIFO headroom valve (`pending_room ≥ 2`, checked at admission) under the D4 arm; re-enqueue the parent if its delta crossed the writeback threshold; count `meta_kv_node_merges`.

**(c) Interior recursion and root collapse — the ONLY way the height decreases.** A merge removes one child from the parent. If the parent is NOT the root and its own fold is underfull (`f ≤ ¼·C`), it is a merge candidate at ITS level: `smo_merge` is level-agnostic (the successor is pinned when `level > 0`; the grandparent is the parent-of-record), so interior merges are the same code path, and the recursion climbs one level per merge, bounded by the tree height. If the parent IS the root and holds exactly ONE live separator, the tree collapses: **root collapse** (`smo_root_collapse`) makes that child the root — `RootPtr = (child.addr, child.node_seq)`; the child already spans the whole key space (`["", KEY_SPACE_MAX]` — the only child of a gap-free partition; refused loud otherwise), it is pinned, the old root is superseded + retired, its extent freed. It is a **ROOT-SWAP SMO** in §4.6's sense: it journals **no pointer record** (its durable form is the next ledger record's `tree_roots`), its entry is `[free(old_root)]`, and it notes the **dying floor at `res.start`** so every record applied under the new arrangement stays inside the replay window until a ledger record names the new root; it replays through the OLD structure under the §4.7 coverage gate (the old root's extent is parked until the tail passes its free record — the old image routes to the child exactly as before). The collapse chains: a root at level ≥ 1 with one child that itself has one child collapses twice. **The traversal's `level < target_level ⇒ restart` arm (`KvTree::descend`) is exactly right and stays**: a walk that began at the old height and reaches a node BELOW its target level (a root swap lowered the height mid-walk) restarts at the new root; content walks (`target_level = 0`) never observe it, interior-targeted walks are SMO-task- or mount-replay-only.

**(d) The heap-space arithmetic, and the recovery cadence stated honestly.** A merge CLAIMS one extent and FREES two — net **−1** allocated; a root collapse claims none and frees one. The claim is admitted at the **compaction floor** (`claimable − 1 ≥ compaction_floor_extents(reserve)` with `claimable = free − heap_promised`), never at the whole reserve — the §4.7 amended law's compaction class: on a FULL heap (free ≈ reserve) about `reserve/2` merges run per sweep before the floor refuses, and the flush pass's own half of the reserve is never touched. The freed extents return through pending-free only when the durable tail passes the merge entry's last seq: the parent's flip floor pins the next cycle's tail at `res.start` until the parent is flushed, so a merge's two extents are claimable **one to two barriered cycles** after the merge (the `smo_replace` progress theorem — every parked retirement drains within two barriered cycles), and a root collapse's one extent the same. Hence a heap-full recovery is a **wave** — sweep (`claimable − reserve/2` merges, ≈ `reserve/2` on a heap at the reserve) → cycle → cycle → the extents return → sweep again — and the checkpoint keeps sweeping while a `merge_backlog` stands — a lap the floor cut, a lap the budget cut, **or a lap that merged** (its successors and the parents it shrank can pair again; the posture alone let the recovery stall the moment its first returned extents cleared the growth floor) — not only while `heap_full` is latched; a completed lap with no merge and no refusal is the fixed point and the sweep stops. Because every merge returns one extent that funds the next wave, the waves GROW geometrically: the motivating contract's trace on a 24 MiB / 64 KiB-node volume (reserve 8, floor 4) reads 8 → 16 → 32 → 64 → 128 merges per wave, 313 in all over 14 sweeps, free extents 11 → 325 — so a volume that is 90 % deleted recovers ≈ 90 % of its leaf extents in O(log candidates) waves of two barriered cycles each, and the ENOSPC posture clears the moment the growth floor is clear (`heap_growth_floor_clear`, unchanged). The ENOSPC valve (`enospc_refusals`) still answers a growth commit that arrives between waves.

**(d′) Cross-parent shrinkage — PROVEN and pinned (finalized 2026-09-11; `tests/kv_leaf_merge_tests.rs` (10)).** A merge never crosses a parent boundary (two adjacent leaves under DIFFERENT parents are never folded — the successor's range would straddle two immutable parent ranges). Underfull leaves on both sides of a boundary shrink **through interior merges**: their parents, left with few live separators, are themselves underfull at their level (a separator record is ≈ 40 B — a 64 KiB interior is a candidate below ≈ 380 live separators, a 256 KiB one below ≈ 1,600 — so a parent whose leaves merged down to a handful always qualifies), the same `smo_merge` folds them (`meta_kv_interior_merges` is the level-≥1 face of `node_merges`), the former cross-boundary neighbours become siblings, and the next lap merges them. The **merge fixed point** is the checkable law (`merge_fixed_point` in the contract walks every level): at no level are two adjacent SIBLINGS jointly mergeable by the pair law, and two adjacent nodes under DIFFERENT parents are jointly mergeable only when their parents are NOT — the one shape the sibling-only SMO cannot reach: a boundary between two interiors whose separator folds exceed the pair capacity together (`f(P₁)+f(P₂) > ¾·C` — at 40 B per separator, more than ≈ 3,900 live children across the two parents at 256 KiB; ≈ 960 at 64 KiB). Such a boundary strands at most ONE leaf pair (the left parent's rightmost child and the right parent's leftmost), so `stranded ≤ heavy_boundaries ≤ interiors − 1` — bounded by `1/fanout` of the leaf population, never a leak, and it clears as soon as either parent shrinks. The contract's fixture (21,856 records, 3,188 leaves under 4 level-1 interiors, one record kept in every parent's leftmost and rightmost leaf so every parent boundary has an underfull leaf on both sides) converges to 2 leaves at height 1 with 3 interior merges (= parents − 1), 1 root collapse, 0 stranded pairs and 0 heavy boundaries. **Convergence bound, derived:** with unlimited extents a pass is one whole lap; per level, one pass merges it, one pass (after coverage elides the separator tombstones the merges minted above it) merges the parent level, one pass regroups the level under its merged parents — so `passes ≤ 3·h₀ + 1` (the terminal no-op pass); the fixture took 4 of the bound's 7. On a FULL heap the floor cuts each lap into waves, so `sweeps ≤ 3·h₀ + 1 + 2·waves + 2` with `waves ≤ ⌈log₂(candidates/room₀ + 1)⌉ + 1` (`room₀ = claimable − floor` at the start; each wave returns its merges' extents to fund the next) — pinned in contract (11).

**(e) Three triggers, all derived.**

| Trigger | Where | Candidate test | Cost | Posture |
|---|---|---|---|---|
| **Flush pass** | `checkpoint_flush_node`, after a leaf's visit (append or SMO) | O(1) sound-but-incomplete prefilter: the leaf's SERIALIZED log bytes (`tail_offset − NODE_PAGE`) plus its open delta `≤ ¼·C` ⇒ `f ≤ ¼·C` for certain (folding only shrinks; frames only add). A tombstone-heavy full log is missed here by design and caught when its log fills and compacts (the compacted successor's log IS its fold) or by the sweeps | one relaxed read; the O(node) exact fold only on actual candidates | forced retirement (the flush pass's) |
| **Heap-full sweep** | `checkpoint_cycle`, at the END of the cycle — after the durable tail advanced, so the lap's count phase reads the SAME tail a census taken after the cycle reads (a tail advance only elides more tombstones and can only ADD candidates; a count taken before it under-reads). The wave cadence is unchanged by the placement: a merge's flips sit above this cycle's `H` either way, so its extents return at the NEXT cycle's barrier | runs while `heap_full` is latched OR a `merge_backlog` stands; `KvMetaBackend::run_merge_sweep` → `KvTree::merge_underfull(ctx, forced, deadline)` walks each tree's RESIDENT nodes **level by level from the leaves up**, each level in key order (no device reads for the census; a candidate's sibling is demand-paged), one projection per node (`is_merge_candidate`), the pair law per adjacent pair, greedy left-to-right packing, the climb after each merge, the root-collapse chain, then a **count phase** over the leaves that publishes the EXACT underfull population (below). The interior levels are walked in their own right, not only through the climb: the separator tombstones a leaf merge mints sit ABOVE the durable tail until the next barriered cycle covers them, so a parent that lost most of its children projects underfull only on a LATER lap — one that runs no leaf merge and never climbs | **BOUNDED per cycle** to `merge_sweep_budget_ms(flush_interval)` ≡ `checkpoint_tick_period_ms` — one cadence period, finding 49's drain law ("every threshold drain is bounded by one period; at least one item per pass; leftovers re-arm"), never a constant of its own (tie-tested); past the deadline the walk saves its **cursor** (phase + next `min_key`, per tree) and the next cycle RESUMES it, so a full-cache lap (≈ 2,048 resident 256 KiB nodes × the `kv_merge_sweep` bench's ns/leaf) completes across cycles instead of restarting and never stalls the cadence a full-cache volume's ring reclamation rides; ZERO in steady state (the posture gates it); merging stops at the compaction floor (`NoSpace` ⇒ `space_refused`, counting continues); a ring-reserve / FIFO refusal parks the cursor AT the refused node with the merges so far reported (`MergeSweep::refusal`) | forced retirement |
| **D4 defrag arm** | `JobType::DefragMeta` (`jobs::run_defrag_meta`) after the compaction nudges | `dead_bset_census` gains `merge_candidates` (leaves with `f ≤ ¼·C`, over the paged-in population — the SAME predicate `KvTree::is_merge_candidate` the sweep uses) and the report/gauge face `mergeable_leaves` / `frag_d4_mergeable_leaves`; the job drives `KvMetaBackend::defrag_merge_sweep(deadline)` — the SAME bounded, cursor-resumed walk — to one whole volume lap in chunks of `merge_sweep_budget_ms`, `duty_park`ing between chunks (its census IS the walk; no separate O(leaves) pass) | the whole-tree occupancy sweep — the operator's cold-leaf recovery, throttled by the KD-3 duty cycle like every nudge (pinned: a 2 % `defrag --meta` makes progress, parks on pause, completes rethrottled) | FIFO valve (`PendingFreeFull` / reserve exhaustion ⇒ one forced cycle + resume from the parked cursor, the nudge's ladder); `NoSpace` at the floor ends the lap's merging for this volume (the cycles return the extents; the next run continues) and stands the backlog |

**(f) Crash windows**, enumerated the way §4.6 does for splits (the mount's two-phase replay + the §4.7 coverage gate are the machinery; nothing new):

| Window | On-disk state at the kill | Mount outcome |
|---|---|---|
| Before the successor image is durable / before the entry is reserved | old extents live and routed; the successor extent claimed in RAM only (its bitmap bit is not durable — the SMO task IS the checkpoint task, so no checkpoint ran between the claim and the entry) | nothing changed: the old leaves serve; no leak |
| Entry reserved, bytes unwritten (the §4.4 pt 4 hole; also a torn entry under power loss) | successor image durable but unreferenced by any durable record; old extents intact | replay drops the entry: the old structure stands, both old extents stay allocated AND routed — **identical to a split in this window** (design-smo-replay-currency §2-A crash windows: "mount never sees claim or free ⇒ predecessor stays routed and allocated"). If the process lived on and a checkpoint ran before the kill, the parent's appended bset carries the flip and the ledger names roots reaching the successor — the successor is live, not leaked, and the old extents' release stayed gated on the durable tail of a DURABLE ledger record (a torn-newest fallback to the predecessor record is safe because release never precedes that record's own barrier). The successor's bit, if a bitmap page happened to persist it, is the same bounded crash-window leak a split's is — reclaimable by a future fsck class, never corruption |
| Entry written, before the checkpoint names the new structure | successor durable; the entry (flips + alloc + two frees) in the window; the parent's flip maybe un-flushed | phase 1 applies `Put(R.max→succ)` then `Delete(L.max)` on the level-`L+1` node covering them (level DESC, then seq); phase 2 folds content by key through the final routing; the alloc replays; BOTH frees park until the first post-mount durable checkpoint (mount-side re-parking is forced — §4.7). Old images readable but unrouted |
| Root collapse: hole | RAM root swapped; the old root's free never journaled | the next ledger names the child as root (if the process lived); the old root's extent is either released (bit cleared at a later checkpoint) or a bounded one-extent leak — identical to a root GROWTH's hole today |
| Root collapse: entry written, before the ledger names the child | ledger names the OLD root; `[free(old_root)]` in the window | replay through the old structure by design (the C′ carve-out): the old root's image is parked (coverage gate) and still routes to the child; content folds by key; a later ledger naming the child drops the level-`≥1` flips of the window as unroutable (`root_level < level`) — sound, content replays by key |

Replay-twice digest equality holds unchanged: replay is read-only into the cache and (level DESC, seq) is a deterministic total order over the same entries; the merge adds no record class replay has not seen.

**(g) What fsck and the census see.** fsck has **no meta-tree structural class** today (its classes are the block/inode planes: C1–C12); the meta tree's structural detectors are the mount's `child_node_seq` check, `KvTree::open`'s root checks, and `verify_node_extent`. A merge leaves nothing a walk could distinguish from a split's result — gap-free sibling ranges, every separator equal to its child's `max_key` — and the in-tree structural contract is `tests/kv_leaf_merge_tests.rs`'s K2 walk (every child abuts its neighbour, every separator names its child's `max_key`, the last child's `max_key` is the parent's); a future fsck meta-structure class would check exactly that. The D4 census (`dead_bset_census`) counts resident leaves and indexed records — both FALL after merges — and gains the `merge_candidates` face.

**(h) Gauges** (stats inode, appended at the end of the `meta_kv_*` group): `meta_kv_node_merges` (leaf/interior merges executed — the engagement gauge; moves on any delete-heavy workload), `meta_kv_interior_merges` (the level-≥1 subset — cross-parent shrinkage's face), `meta_kv_root_collapses` (height decrements), `meta_kv_merge_candidates` (per volume, **EXACT**: the underfull-leaf population the last completed lap's count phase — or the last census — found, under the durable tail in force then; see (h′)), `meta_kv_merge_sweeps` (per volume: bounded sweep CALLS), `meta_kv_merge_laps` (per volume: whole-volume laps completed — the publish instants), `meta_kv_merge_sweep_ns` / `meta_kv_merge_sweep_projections` (per volume, always-on: the sweep's wall ns and nodes projected — `ns ÷ projections` is the live ns per leaf the `kv_merge_sweep` bench prices offline), `frag_d4_mergeable_leaves` (the D4 face: mergeable leaves across the set's meta volumes at the last defrag measure), `defrag_meta_merges`. `MaintenanceOutcome` gains `merges` / `interior_merges` / `root_collapses`.

**(h′) The exact-candidates law (finalized 2026-09-11; `tests/kv_leaf_merge_tests.rs` (11), (12)).** Every lap ends with a count phase over the resident leaves that publishes the underfull population of the POST-merge tree; a compaction-floor refusal stops merging, never counting; the census and the sweep share ONE predicate (`KvTree::is_merge_candidate[_at]`). Because the predicate consults the durable tail (a covered tombstone projects as elided), a count is comparable to another only under the SAME tail — `KvMetaBackend::merge_candidates_audit` reads, under the SMO mutex, the gauge with the tail it was counted under, a census under that tail and a census under the tail now: `gauge ≡ census_at(gauge_tail)` whenever no user commit touched the tree since the publish, and `census_now ≥ census_at(gauge_tail)` always (a later tail only elides more). The sweep runs at the END of the checkpoint cycle for exactly this reason. On a quiescent volume `frag_d4_mergeable_leaves ≡ meta_kv_merge_candidates` (pinned in `tests/defrag_tests.rs`). The pre-finalization sweep counted only the leaves it saw before the first refusal — its own trace read `1 underfull leaves` against a 335-leaf tree; the finalized trace reads `1 merges … candidates=335`.
...
- **ENOSPC semantics**: allocation failure with the **compaction reserve** intact ⇒ the metadata op fails with `ENOSPC` (mapped through the existing error path; today's analog is "Inode table full", `alloc.rs:67-69`). The reserve (max(8 extents, 2 % of heap)) is allocatable only by compaction/checkpoint internals, so the tree can always fold appends and free space even at user-visible ENOSPC — no write-to-free-space deadlock. `statfs` maps free extents × node fill factor into `f_ffree`/`f_favail` estimates (today's hardcoded health math at `mod.rs:1210` goes away); the stats inode reports exact counters (§9).
  **Amended 2026-09-11 (the inline-raise sweep's P1, `.benchmarks/2026-09-09-inline-raise-sweep-local.md` §3): the user side of this law is a COMMIT-TIME heap admission, because a user commit never claims an extent.** Its records land in leaf overlays and the checkpoint's flush pass claims for the SMOs they force — through the internal class, like every SMO — so as written the reserve was spent by acked user growth, the pass reached `free=0` mid-flush and skip-and-deferred, the deferred nodes' floors pinned the tail below every parked retirement gate, and the wedged-tail audit fail-stopped a FULL volume. The law now reads: **every acked record must be flushable.** The commit pass (`KvMetaBackend::admit_heap_locked`, under the union leaf write locks, before the reservation) projects each touched leaf's flush — `NodeDirty::projected_log_end`: the in-flight frozen frame, the open delta and the batch's pending bytes against the node's log — and a leaf that overflows has its SMO's extents **promised** on the node (`NodeDirty::promise`; the per-volume `NodeCache::heap_promised` ledger) against `claimable − promised`: a fold at-or-under `NodeLayout::fold_capacity` (estimated by `NodeSnapshot::fold_bytes_upper_with`, with the pending records merged in so a `Delete`/`Put` shadows the key's resident records) is a net-zero **compaction** (1 extent) admitted down to the **compaction floor = reserve / 2** (`alloc_ext::compaction_floor_extents` — half the reserve admits deletes and overwrites on a full volume, half stays the flush pass's own for interior cascades and under-projection); above it a **split** (`NodeLayout::smo_extents_for_parts`: the SMO's own greedy ¾-fill part count over the same key order plus two — the packer's rounding and the cascade extent that growth below `NodeLayout::split_growth_window` bytes can add before the flush — plus a new root for a root leaf) is admitted only above the whole reserve. A member that cannot be promised refuses `NoSpace` alone, pre-reserve — `ENOSPC` to userspace — after one retry behind a checkpoint cycle when that cycle can return budget (outstanding promises, parked retirements). Promises are consumed by the SMO that replaces the node (whose leftovers — records applied during the build window — re-promise on the successor whose log they overflow), released when an in-place append leaves the remainder fitting, and Drop-owned; a promised leaf re-walks its fold only once per growth window of admitted bytes (the cheap re-check rides the walk's fold plus the bytes since), so the hot path pays the O(node) estimate once per node fill. The flush pass's claims are therefore budget the admission set aside, never a race against user growth; `meta_kv_heap_promised` is the ledger, 0 at quiesce. ~~What this does NOT do: the v1 tree never shrinks (no merges), so deletes return record space inside leaves, never extents — a filled volume resumes creates into the room its deletes freed and answers `ENOSPC` again once a new leaf is needed. Leaf merge is a separate item.~~ **Closed by §4.6a (leaf merge, 2026-09-11):** deletes now return EXTENTS too — underfull adjacent siblings merge (net −1 extent per merge, admitted at this same compaction floor), the heap-full sweep runs the recovery in waves inside the checkpoint cycle, and a filled volume that is mostly deleted keeps creating.
- **The wedged-tail bound's two classes** (the same amendment; design-smo-replay-currency PR 4 clause b): a barriered cycle that leaves retirements parked with neither a release nor a tail advance is judged by whether its flush pass **deferred a node for `NoSpace`**. If it did, the tail is pinned by a node the pass could not flush for want of an extent — a **space standstill**: capacity, not corruption. It is counted (`meta_kv_heap_full_cycles`), latches the per-volume posture word `meta_kv_heap_full` (one WARN per transition), neither advances nor resets the wedge rung, and is **never terminal**; it clears when budget returns (a delete's compaction, a released claim) and a pass defers nothing. Since the heap admission promises every SMO's extents this class is the residual (a foreign claimant, an under-projection, an interior cascade past the reserve). The FAILED terminal is reserved for the **wedge** class: nothing deferred for space, retirements parked, no release, no tail advance for `PENDING_FREE_FORCE_CYCLES` cycles — a tail nothing can discharge. `meta_kv_enospc_refusals` counts the admission's refusals per volume.
...
| `meta_kv_node_merges` / `meta_kv_root_collapses` / `meta_kv_merge_candidates` / `meta_kv_merge_sweeps` (+ `frag_d4_mergeable_leaves`) | counters / per-volume gauges | §4.6a leaf merge: merges executed (the engagement gauge — moves on any delete-heavy workload; 0 on a fill-only one) / height decrements / underfull leaves the last sweep or census found (LIVE) / heap-full + backlog sweeps run. `merge_candidates` high with `node_merges` flat and `heap_full` set ⇒ the compaction floor is refusing the sweep — read `meta_kv_free_extents` / `heap_promised` beside it |
...
**§5.3 second consumer (2026-09-09, the inline raise):** the same `xattr_value_cap(ino) − 4 KiB` derivation that bounds the inline BLOCK MAP now also bounds the inline DATA payload — `routing::inline_max_bytes_ceiling` is the inline-layout ceiling's FORMAT BOUND (60 KiB at the default node, 12 KiB at the 64 KiB floor), the maximum `SQUEEZEFS_INLINE_MAX_BYTES` may name (range 4096..=61440); the DEFAULT stays one page (`routing::INLINE_MAX_FLOOR`) — the phase-B sweep (`.benchmarks/2026-09-09-inline-raise-sweep-local.md`) priced a raised default out, since an inline payload rides this plane twice per write (journal entry + node append). A file up to it keeps its payload as the layout record's `data_key` (one KV value, ≤ the cap by construction; the layout-delta wire's `u16` data-key length is satisfied a fortiori), and `promote_staged_file` dispatches a staged file at or under it INTO that record instead of a whole block (`.benchmarks/2026-09-09-fsync-promote-staged-ab.md` §3–4). Nothing on disk changes: the record is the same `"layout"` xattr value the format already admits at this size.

...
| `symmetric_meta`, `slot_leases_held`, `slot_rotor`, `slot_acquires`, `slot_grants`, `slot_offers`, `slot_offers_{idle,dominated,expired}`, `slot_handovers`, `slot_handover_phase_ns`, `slot_ships`, `slot_lru_releases`, `slot_forced_shrinks`, `slot_lease_conflicts`, `slot_resolve_rpcs`, `slot_resolve_redirects`, `slot_tree_inos_{p50,p99,max}`, `slot_tree_bytes_{p99,max}`, `affinity_a_max_bytes`, `affinity_mints`, `rotor_mints`, `affinity_ceiling_spills`, `affinity_ceiling_overflows`, `slot_offer_n_floor`, `slot_region_releases`, `slot_recall_notices`, `meta_kv_leaf_lease_refusals` | **Slot leases** (design-symmetric-metadata §5.1 / §11, incompat bit 17 + `SQUEEZEFS_SYMMETRIC_META=1` — PR 4, dark; per volume, and every one 0 — `dlm_mode` `solo` — on an unarmed or bit-17-absent mount, pinned). `symmetric_meta` = 1 when the plane is armed; `slot_leases_held` = the slots this mount leases (native + the rotor + every slot acquired since); `slot_rotor` = `M` in force (`clamp(W / (2 × writers_known), 1, 64)`, 64 solo; `SQUEEZEFS_SYM_MINT_SLOTS`); `slot_acquires` / `slot_grants` = `AcquireSlot(s)` verbs and fresh grants (a grant moves the slot's `g` once; a replay answers `already` and moves nothing); `slot_offers` = offers made by this holder, split `_idle` (an idle tree — `ops_h` = 0 — offered to the requester that shipped ≥ `N_floor` ops into it) / `_dominated` (a live tree offered to the ONE requester whose ships ≥ 2 × the holder's ops over the common `T_idle` window) / `_expired` (lapsed after one renewal beat unaccepted) — closure `offers ≡ handovers + expired + standing`; `slot_handovers` = accepted offers completed flush-then-transfer, `slot_handover_phase_ns` = exact-sum `flush / page / tree0 / grant / total` (`flush` = the covering cycle AND the tail-passing cycle — the departing ring's window is clear of the slot before any other home says it moved); `slot_ships` = served ships counted for dominance (**the crowd is counted here and moves nothing** — twelve thousand single-shot creators read as 12,000 ships and 0 offers); `slot_lru_releases` = idle non-rotor slots released past the page budget (108), `slot_forced_shrinks` = idle rotor slots released when the census grew and `M` fell, `slot_region_releases` = declared regions gone `Free` because their last slot went; `slot_lease_conflicts` **must stay 0** (two `Live` pages attesting one slot, or a `Live` entry contradicting tree 0's lessee at a higher `g` — C14's live face, the mount REFUSES); `slot_resolve_rpcs` = `ResolveSlot` round trips (0 on the warm path — the holder cache answers off tree 0), `slot_resolve_redirects` = `NotHolder { current }` corrections; `slot_tree_inos_*` / `slot_tree_bytes_*` = the leased trees' minted-ino and extent-byte distribution (the divisibility face — a solo mount's load over 64 trees); `affinity_a_max_bytes` = the ceiling in force (`max(used_leaf_bytes / 64, node_size)`, or the static knob); `affinity_mints` / `rotor_mints` partition every mint (`≡ mints`), `affinity_ceiling_spills` = children spilled from a parent's over-cap tree to the rotor slot with the most headroom, `affinity_ceiling_overflows` = the extra rotor slots granted when EVERY rotor tree was strictly over the cap (≤ `M` more; a solo mount stays at 64); `slot_offer_n_floor` = `max(2, ceil(handover cost / ship cost))` in force; `slot_recall_notices` = recalls of a slot from a WIRE holder recorded for its renewal grant's `slot_release_notices`; `meta_kv_leaf_lease_refusals` **must stay 0** (a leaf mutation of a slot this mount does not lease reached `apply_locked` past the commit door — the §5.4.1 partition law's belt). |

...
- **Pack per `(writer, slot, data volume)` (§5.4.3 law 1).** On an armed set the packer keys its open packs by the tenant's SLOT: two inos of two slots never share a pack block, so every reference of a pack block lives in ONE slot tree and its refcount there is exact — one range probe, nothing proportional to a handed-over tree. Open packs per data volume ≤ the leased slots being written (up to 64 rotor packs + affinity packs on a solo mount, ≤ 108 with inherited slots — PR 2's page budget) where PK2 has ONE; the seal residue at dismount is therefore ≤ that many partial blocks per data volume (`pack_blocks_sealed_dismount` reports it beside the PK acceptance figure; `pack_open_scopes` is the live population; `defrag --pack` consumes the partials). Unarmed, every tenant's scope is 0 and PK2's one open pack per data volume stands.
- **`refcount(b)` is probed on demand (§5.4.3 law 2)** — `block_ref_probes`: one `06 ‖ vol ‖ blk` range probe in one slot tree (the referencing ino's, which by the pack law holds every reference of an uncloned block), or the shared index. What PR 7 does NOT change: the mount still seeds the RAM refcount map and the free list from ONE by-block scan (the durable ledger's `recover_durable_block_refs`) — **the free list is design §5.5.1's allocation bitmap, PR 8's**; until it lands the derivation stays, and "a forest mount's open performs zero by-block census reads" is PR 8's pin, not this rung's. The RAM map stays the free-list / cursor / pin engine; the two DECISIONS the design names read the durable truth beside it: the W1 sole-owner patch confirms its RAM verdict with ONE probe of the patching ino's slot tree on an armed set (`DataRouter::sole_owner_durably` — a second reference a predecessor lessee committed into a handed-over tree, which this mount's RAM map never saw, refuses the patch into the CoW fallback; unarmed, no read), and a SHARED block's terminal free is decided at the index home.
- **The shared-block index and the clone protocol (§5.4.4).** A block referenced from two slots arises only through clone/reflink; on an armed set `clone_file` runs the ordered protocol: `MarkShared` at the source's holder (the SHARED bit — bit 1 of the reference value — set on the source's record in ONE tx under its 4a guard; `Gone` when no record exists ⇒ the clone aborts `ENOENT`-class with every pin undone), `ShareBlock` at the **index home** (one tree-0 `shared_ref:` record per `(block, ino)` for BOTH inos, so C16 can check both sides; idempotent), then the cloner's own publish WITH the bit (a clone of a clone inherits it). **The home is resolved behind ONE function** (`shared_refs::index_home_volume`): in PR 7 volume 0's tree 0 — the manager's control plane, written through its one control-entry writer in ring 0's user class; PR 8 re-points it to the data volume's allocation-lease holder (the design's kind `0x09` in that holder's slot tree), and PR 12 makes a non-manager reach it over the wire (`ManagerCall::{MarkShared, ShareBlock, ReleaseShared}` exist and are served; this mount's own tree 0 answers today). **A terminal free of a SHARED block is never decided locally**: the block's RAM SHARED mark (seeded from the index at the mount's arm, set by `MarkShared`) sends the release to the home (`ReleaseShared`), which drops the releaser's entry, GC's an entry whose ino holds no reference (the "cloner died after `ShareBlock`" window) and answers `Held` (nonterminal — one RAM reference released, never to 0) or `Freed` (the ordinary terminal free runs, the mark clears). **The W1 in-place patch reads the mark after its `SeqCst` fence and stands down on it whatever the count reads** (`patch_ineligible_shared` — the mark protects the block between step 1 and step 3, when the count still reads 1; loom `shared_ref_core`). Gauges: `shared_blocks` (marked population per data volume), `mark_shared_calls`, `share_block_calls`, `release_shared_calls`.
...
- **Owed**: the free list as the allocation bitmap and the zero-scan open (PR 8), the index's move to the allocation holder (PR 8), the wire path of the three verbs for a non-manager (PR 12), the ONE-walk census (PR 1's), the `ls -l` row with/without gather and the pack ratio on the box (PR 13).

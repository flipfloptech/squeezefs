...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_RANGE_CUSTODY", Kind::Bool, "off", "DLM S11 (rung 15, KD-MW-7): distributed byte-range custody on the S9 custody lease. `=1` on an armed co-writer routes sub-file write custody through required/desired range grants (block-aligned desired, admit-time coalescing, the §9.2 geometry cap + dlm_grant_table_bytes R5 ceiling) instead of whole-file leases, cached in the S8 token cache's range extension. Read ONLY when the mw ownership plane is armed (the SQUEEZEFS_DELEGATION precedent); set-but-unarmed is announced-inert (every range_custody gauge structurally 0), never a refusal. STATIC DEFAULT OFF UNTIL RUNGS 16/17 LAND (adjudicated 2026-08-17, .benchmarks/2026-08-17-s11-range-wire.md): the custody plane is complete, but two co-writers publishing ONE ino's layout concurrently still race the delta-chain/full-save base (the design-mw-layout-versions §6 residual, C8-visible — drift 702 measured live), so an armed default would let a lossy composition stand where a loud whole-file conflict stands today; the never-lossy law outranks §11's provisional default, and every rung ships dark (the rollout rule). The s11-range leg arms it explicitly and is the composition rungs' acceptance surface."),

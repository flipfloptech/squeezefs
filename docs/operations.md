...
- **Recall-driven free-grace** (§5.7.3): see [Freed-offset grace period](#freed-offset-grace-period-spec-68-item-3) — a terminal free publishes directly after the recalls (`free_grace_recall_gated_frees`); the ring is the TIMEOUT path (`free_grace_timeout_deferrals`).

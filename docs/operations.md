...
| `SQUEEZEFS_PUBLISH_SHIP_DEPTH` | int 1..64 | `derived clamp(ceil(cpus/8), 2, 8)` | Publish frames one co-writer keeps in flight per authority (all on one pipelined session by default; one session each under `SQUEEZEFS_PUBLISH_SHIP_MULTIPLEX=0`). `1` = stop-and-wait (A/B control, never an operational escape). |
| `SQUEEZEFS_PUBLISH_SHIP_MULTIPLEX` | bool | `on` | D-5: the in-flight publish frames toward one authority ride ONE pipelined session (replies demultiplexed by id; the authority's session lane serves them concurrently), so depth no longer costs a connection per frame against the authority's connection cap. `0` = the D-1b session pool (A/B control, never an operational escape). |
...
| `SQUEEZEFS_PUBLISH_SHIP_MULTIPLEX` | `on` | S9 publish plane (D-5 — DLM #8's single-connection half): the co-writer's in-flight publish frames toward one authority ride ONE pipelined cluster-wire session — every call carries its id, a per-session reader thread (`sqz-clw-mux`) demultiplexes the replies, and the authority's session lane serves the in-flight frames concurrently — so `SQUEEZEFS_PUBLISH_SHIP_DEPTH` no longer costs that many connections against the authority's connection cap (one owner thread per connection). `0` = the D-1b session pool (depth request/reply sessions per authority; the same-binary A/B control). Live: `meta_ship_publish.ship_mux_frames` (≡ `ship_frames` on the default, 0 under `0`); `ship_session_dials` reads 1 per endpoint at steady state instead of `depth`. |

...
| **3** | **Wake-hop inflation on the conveyor** (DLM #3, the W #5 residue) — **now THE binding term on the fleet, isolated by D-2's instruments** | spawn-per-leadership pass task (`backend.rs` `try_lead` → `spawn_meta`); the journal write's completion round trip = queue hop → `uring_fs` worker → CQE → oneshot wake onto one of TWO `sqz-meta` lane threads (shared with the checkpoint task and every `spawn_meta_join` owner call, 7 k/s on the fleet): **mean 1.0–1.3 ms, mode 50–200 µs, 5 % ≥ 4 ms** per window (`meta_txpass_phase_ns.journal_ring_write` post-D-2), `window_lane_wait` bimodal (≤ 16 µs or ≥ 1 ms behind a tail head) | the whole D-2 headroom (ρ 0.13) waits on it / HIGH mechanism, MEDIUM number (per-window, measured) | D-1b's fleet rig A-B-B-A (the D-2 row is the baseline) with a resident pass + lane on `sqz_notify`, fan-out on the committer's lane, and the `uring_fs` completion delivered without the lane hop; `journal_ring_write` mean → its mode, `window_lane_wait` tail gone, commit latency (queue + `window_total`) down |
...
| 3 | wake-hop inflation — **the binding term post-D-2**: journal write completion round trip mean 1.0–1.3 ms / mode 50–200 µs per window on the fleet | spawn-per-leadership pass task; ~0.68 ms residue | resident pass task on Notify; fan-out on the committer's lane; the `uring_fs` completion without the lane hop |

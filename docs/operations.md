...
| `SQUEEZEFS_INLINE_MAX_BYTES` | int 4096..=61440 | derived: value cap − 4 KiB (`61440` at the 256 KiB node) | The inline-layout ceiling, bytes: a file up to it keeps its whole payload in its layout record on the metadata volume — visible to every client at the commit, no block, no promotion; past it the file is staged (with a staging dir) or striped. Derived per volume as the KV xattr value cap `min(64 KiB, node_size/4)` minus the layout wire's 4 KiB framing headroom (the largest payload one KV value holds); the explicit value wins verbatim and is bounded by a smaller-node volume's format bound (logged once). The trade the override prices (the phase-B threshold sweep): metadata-plane bytes per small write vs. a whole striped block per small file at promotion — the 64× space law of [`.benchmarks/2026-09-09-fsync-promote-staged-ab.md`](../.benchmarks/2026-09-09-fsync-promote-staged-ab.md) §3. `4096` = the shipped ceiling, the A/B control. Published live as `inline_max_bytes`; engagement `layout_inline_writes` vs `layout_staged_writes`, `layout_promoted_inline` vs `layout_promoted_block`. See [Staged-layout files and multi-client visibility](#breaking-changes--migration-notes). |

...
/// Stage a 16-byte `mtime`+`ctime` field patch for inode `index` (inside
...
///
/// Takes the sector **read** lock only when *not* inside a transaction (inside
/// a tx the staging overlay + the DLM lock give a consistent view, and taking
/// the sector lock in the closure would risk commit-time reentrancy on the
/// non-reentrant tokio lock, design §3.4).
...
/// Persist inode `index`.
///
/// In-transaction: stages a **256-byte sub-sector patch** at the slot's byte
/// offset. The whole-sector RMW is performed under the sector write lock at
...
///
/// Inside a tx it stages the slot patch (commit does the RMW); outside a tx it
/// does a sector-safe full RMW under the sector **write** lock, so direct
/// writers (e.g. `set_layout_and_size`, the routed cross-volume paths) never
/// write a sector outside its lock (invariant R1).

...
> **ℹ️ Incompat bit 17 `KV_SYMMETRIC_FOREST` — the slot-tree forest (2026-09-13, design-symmetric-metadata PR 1).** A binary older than 2026-09-13 refuses a stamped volume loudly at the superblock gate, naming the unknown bit. Nothing stamps the bit yet — not `format`, not any verb — only the test seam `SQUEEZEFS_TEST_STAMP_SYMMETRIC=1`; every volume in the field is unaffected and mounts on old and new binaries exactly as before (see [the forest](#symmetric-metadata-program--the-slot-tree-forest-pr-1-dark)). The stamping paths (`format --symmetric`, `volume enable-symmetric`) are PR 11's; the default flip, at which the bit becomes presence-required, is PR 14's.

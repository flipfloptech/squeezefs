...
- `SQUEEZEFS_FUSE_IO_URING_SPIN_US` (default **derived**; range 0–100000; e2e perf audit R-4, `.benchmarks/2026-09-03-r4-reap-thread-economy.md`): the spin-before-park CAP, µs, for the FUSE-over-io_uring queue workers (`f3-ur*`). Where a worker would block in `submit_and_wait` it first spins a bounded window watching `IORING_SQ_TASKRUN` (the ring's "deferred task work landed" flag — rings are built with `IORING_SETUP_TASKRUN_FLAG`), the CQ tail, the wake coalescer's armed flag and the shutdown word, so a device CQE or a lane message landing inside the window is reaped without a park and the scheduler's wake latency (35–60 µs at the field's load; the worker parked 0.76×/op with 84 % of parks ≤ 32 µs). ADAPTIVE by default: the window is 2 × the worker's own park-gap EWMA (a reaction clock — every event it waits for echoes something it just did), capped at the 200 µs rail, engaged only while that worker's queues hold ops in flight (a delivered-not-committed slot or a bridge pend — an idle queue never spins, so an idle mount burns nothing), and refused past the box's queueing knee (whole-box busy > 80 %, the Erlang-C wait onset for c ≥ 16 servers; `/proc/stat` sampled once per 100 ms across the pool). `0` = never spin (the A/B control); an explicit value = the operator's verbatim cap (the box gauge does not refuse it; the in-flight gate still applies). Engagement (stats inode): `transport_spin_absorbed` (parks the spin deleted — an event caught inside the window), `transport_spin_expired` (windows that ran out and parked), `transport_spin_ns` (the CPU paid — the cost column of any row crediting the lever), `transport_spin_refused_busy` (regime hits the box gauge refused), `transport_spin_window_us` (the last derived window). Closure: `absorbed + expired` ≡ the spins run.

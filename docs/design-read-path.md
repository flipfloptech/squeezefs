...
> **Amended (2026-07-29, read-saturation campaign — `.benchmarks/2026-07-29-read-saturation.md`):** three shipped changes to this section's mechanism, each pinned red-first:
> 1. **Ring reads feed the lanes.** The classifier/pipeline driver (`pipeline_touch`) was handler-only; IPC ring ops never observed, so il streams never classified — the §5.6 streaming veto never engaged and every il 4k miss paid one ranged fabric round trip via direct-drive forever (the field's burst-then-collapse: 900k–1.2M → 200–300k sustained). `DataRouter::ring_read_lane_touch` now feeds every ring read once at the sink (warm serves after completion — silent consumption must advance the consume edge; misses before the ladder — the 4th contiguous op's classification vetoes direct-drive for the 5th); ring handoffs carry `ReadClassHint::lane_pre_fed` so the handler never observes them twice (16 double-observations would declassify the stream that routed them). Pinned: `tests/read_saturation_tests.rs`.
> 2. **The issue bounds split.** `effective_window = min(window, share/active)` charged **in-flight** fetches against the **resident** share — at the default shape (128 MiB hot budget, 4 MiB blocks, 16 streams) that is a structurally depth-1 pipeline (measured: prefetch covered 6 % of the kernel seq-row fetches). Now landed-unconsumed fills are resident-bounded by the per-lane share (zero-share still never speculates — the no-floor rationale verbatim) while in-flight depth rides the AIMD window only (in-flight DMA bytes are R5-charged transients via `prefetch_inflight_bytes`, not hot-tier residents). One pure decision: `prefetch_issue_admits`, table-pinned in `tests/read_prefetch_window_tests.rs`.
> 3. **The window cap derives; growth adds the overrun edge.** The fixed cap 16 retired (no-constants law): default cap = `share% × hot_budget / block_size`, railed [4, 4096] (`derived_prefetch_window_cap`); explicit `SQUEEZEFS_READ_PREFETCH_WINDOW` wins verbatim (≤ 4096; 0 = kill switch). Growth (`prefetch_window_grows`) fires on foreground-wait OR a **clean plan overrun** — on the il path warm serves never wait on the single flight, so the reader passing the whole issued plan is a healthy ring stream's only shallowness signal; overrun growth is refused under an evicted-unconsumed streak (never grow into demonstrated starvation), all growth Green-gated. The AIMD collapse, progress-clocked quiescence, spawn shedding, and R5 gates are untouched.
...
| Red | > 95 % | shed to weights: prefetch plans cleared; hot tier clamps toward floor; parked-buffer spill threshold halves (early `flush_memory_buffers_*` — the existing never-lossy staging path, just earlier) **and becomes a real bound — writers past it await the drain (blocking admission, below)**; staging admission keeps its existing refusal behavior |
...
**Red semantics — convergence escalations (finding #2, `.benchmarks/2026-07-12-saturation-suite-oom-finding2.md`).** The bare saturation suite (16 threads × 2 GiB O_DIRECT against an 8 GiB cage, 5 GiB budget) proved advisory-at-admission alone does not converge: Red fired and shed, but the gauge sum rode 2.1 GiB over budget for 100+ s and the kernel killed 7.94 GiB of *anon*. Attribution (per-component 0.5 s traces): (i) `parked_write_buffers` ballooned to 1,937 buffers / 7.6 GiB against the 256-soft-cap — staging refused 100 % of spills (4 MiB entries vs ~10 MiB shards) and the advisory cap let inserts proceed while the 1 Hz drain lost the race; (ii) `read_tier_mmap` grew 0 → 5.06 GiB in ~10 s (weight 0, no shed lever, no admission response — its cap alone ≥ the whole budget); (iii) the seq-write pass ran RSS 3.9 GiB over the gauge sum (jemalloc-retained churn + in-flight upload anon — statm-visible, purge-shed). Three escalations close the gap, all never-lossy:

1. **Unreclaimable pressure arm.** The sampler reads cgroup-v2 `memory.stat` per tick and windows the *kill-relevant* set — `anon + file_dirty + file_writeback + shmem + unevictable + slab_unreclaimable` (clean file cache deliberately excluded: the kernel drops it rather than kill) — in the same 5-slot decaying window as RSS. `pressure = max(Σ non-reclaimable gauges, windowed statm RSS, windowed unreclaimable)`. This is the gauge-undercount defense: dirty page cache is invisible to statm, and anon floods in un-inventoried components (the PR 7 KV-core residual) are invisible to gauges; the kernel's own accounting is not. The corollary defense against gauge *over*-count: the mmap-tier components (`staging_mmap`, `read_tier_mmap`) are registered `kernel_reclaimable` — attribution-only, excluded from the pressure basis — because their logical bytes sit over page cache the kernel reclaims on demand (the fixed-tip f1 trace showed 5 GiB of already-reclaimed tier logical bytes pinning phantom Red through the rand passes and throttling rand-write through the parked gate for nothing); their kill-relevant residue is exactly what the unreclaimable arm measures.
2. **Disk-tier publish pause** (`mem_budget_tier_publish_paused`). While the *unreclaimable arm* sits in the Red band (enter ≥ 95 %, release < 91 % — the R-7 hysteresis), every read-tier publish is skipped at the `cache_read_block` funnel (one gate covers fill-path, dehydration and p2p producers by construction; counted as `read_tier_publishes_paused`). Keyed on the unreclaimable arm, NOT the level: gauge- or statm-driven Red over *clean* (kernel-reclaimable) tier bytes keeps publishing, so warm-up on healthy oversubscribed boxes (the PR 3 knee scenario) is untouched.
3. **Hard backstop** (`mem_budget_hard_backstops`, `mem_budget_backstop_active`). Unreclaimable ≥ 100 % of budget for 3 consecutive ticks — Red demonstrably not converging — collapses the shed target from 85 % to the **floors** until the window decays below the Red-exit edge (entry-edge counted, loud log).
4. **Red blocking parked admission** (`parked_gate_waits` / `parked_gate_self_flushes` / `parked_gate_timeouts`). At Red the halved parked cap is a real bound, enforced in two stages: (i) a brief **drain assist** (`SQUEEZEFS_PARKED_GATE_ASSIST_MS`, default 250 ms) — the writer posts a byte target, kicks the shared drain worker and sleeps on its progress notify (awaited backpressure in async context per the §4.4-pt-5 ring-admission precedent; no spin, no lock acquired while waiting); (ii) still over cap ⇒ **self-flush**: the writer makes *its own* block durable through `upload_active_block_bytes` (the fsync staging-refusal escalation — upload + merge under the already-held block guard, the established 3 → 3.5 extended order) and never parks it, so RAM frees immediately and the cost is bounded by the device, not by other writers. The first cut waited on the drain alone with a 10 s deadline; the leg-B acceptance traces showed why that is wrong — drain *convoys* (writers of one hot inode queueing behind whole-inode flush passes) produced 36 ten-second p99 stalls across 4 suites. Self-flush FAILURE (backend refusal) parks past the cap **loud** (`parked_gate_timeouts`) — RAM is the never-lossy last-resort custody of dirty bytes. The drain worker acquires leases (2) and other blocks' flush locks (3) in a clean task context and never needs the waiter's own block (not yet in the map) — the P1-9 order is preserved by construction. Below Red the cap keeps its historical soft spill-or-keep semantics.
...
| `crates/fuse3` `raw/abi.rs` / `raw/session.rs` / `raw/filesystem.rs` + `path/{path_filesystem.rs,inode_path_bridge.rs}` | `fuse_read_in.flags` exposed; `Filesystem::read(..., flags: u32)` (vendored crate edited in place; both vendored impl surfaces + `SqueezefsFilesystem` + a mechanical sweep of the ~19 integration-test files calling `fs.read(...)` directly) |
...
| `ipc_direct_ineligible_policy` | DIALED P1.5 (2026-07-27, `.benchmarks/2026-07-27-ipc-direct-drive-default.md`): the DEFAULT-mount direct-drive prelude's *deliberate-routing* ledger row — a governed-shape O_DIRECT ring miss sent to the handler **by policy**, not prelude rot: a non-`second-touch` admission mode, a §5.6 ranged-dispatch exclusion (threshold / stream-classified file), or a **GRANT-shaped escalation candidate** (the admission fetch + publish machinery stays on the handler; `allow_escalation` there remains the only token-reservation site — the prelude's `escalation_would_admit` peek is non-reserving). Coherence tripwires: on a clamped churn workload through the shim, `read_admission_governor_denials ≈ ipc_direct_drive_serves` (every denied miss direct-drives, one denial each); on skewed workloads `ranged_read_ghost_escalations` must keep growing (ghost touches are recorded by the direct-drive prelude, so hot subsets still earn admission through the ring) |
| `read_admission_evicted_unhit`, `read_admission_wasted_bytes`, `read_admission_governor_denials`, `read_admission_governor_clamped` | Scan-resistant admission governor (2026-07-26 — the beyond-budget random-read collapse: escalation admissions saturated the device with whole-block fetches that evicted before reuse; per-key cooldown under-bounds ~7× via slot collisions). `evicted_unhit` = ghost-admitted hot victims evicted without EVER serving (the `prefetch_evicted_unconsumed` sibling for admissions — sustained growth means the cache is not earning); `wasted_bytes` = cumulative payback shortfall of admitted victims (`served_bytes` credit vs entry length) — **the bounded-waste verdict is `wasted_bytes` ÷ device read bytes ≤ ~`SQUEEZEFS_READ_ADMISSION_FILL_PCT` % (default 5) under churn**; `governor_denials` = escalations refused by the clamp (each stayed a device-true ranged window read; **must be ≈ 0 on working sets the hot tier retains** — fitting sets never produce waste, so a growing denial count on a fitting set = the waste signal is misfiring, check hot-tier shard collisions); `clamped` = the latest escalation attempt's clamp verdict (gauge). Windows are two 2-s epochs; denied keys record no escalation cooldown so hot subsets converge under skew (pinned by `tests/read_admission_governor_tests.rs`) |
...
Existing signals that must not regress: `get_obj` (the churn detector — per-unique-block ratios in every PR gate; **semantics note post-R3:** it stays the raw device-read-op counter and counts ranged ops too, so `get_obj/unique ≈ 1.0` framings apply to whole-block workloads — streams, the churn suite — where ranged never fires, while random rows gate on `ranged_read_bytes` amplification, §5.6; **2026-07-26 attribution note:** it counts *data* reads only — the health worker's liveness probe rides `NvmeBlockDev::probe_read_block`, uncounted, because a control-plane probe completing behind queued data I/O inflated arbitrary counter windows by +1, the flake class pinned in `tests/backend_health_probe_tests.rs`), `cache_hits`/`cache_misses`, `stale_binding_rebinds` (must stay ≈ 0 growth on quiet workloads; every new serve path increments it on movement like the existing ones), `staged_identity_retries`, `uring_queue_full`, `nvme_staging_current_bytes` / `nvme_read_cache_current_bytes` (row-2 tier-write elimination shows here), `bg_spawn_rejected` (prefetch admission), write-path fields (`write_through_*`). Perf provenance: **every perf PR commits a `.benchmarks/` note** (attribution-doc format) with elbencho rows, `/proc/<pid>/io` device-byte deltas, `.stats` counter deltas, and — for admission/prefetch PRs — a fresh `perf` memcpy share.

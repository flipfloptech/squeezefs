...
- **Owed by the later rungs**: the manager's `JoinAppender` (PR 3 — the directory chain grows only there; a declared id past the first extent's pairs refuses naming it), extent grants (PR 3 — until then every allocator delta is the manager's), slot leases and the `apply_locked` gate (PR 4 — the page's `g` and `slot_tree_extents` are written 0), recovery of a FOREIGN dead appender's ring (PR 10 — `appender clear` with it), `format --symmetric` / `enable-symmetric` (PR 11).

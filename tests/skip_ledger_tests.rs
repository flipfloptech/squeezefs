...
fn the_mount_gated_suites_all_use_the_shared_gate() {
    // The live-mount files (the surface a phantom-green run hides). If a
    // file leaves this list, it must be because it no longer needs a
    ...
    const MOUNT_GATED: &[&str] = &[
        ...
        "overlay_growth_merge_tests.rs",
        "pack_compaction_tests.rs",

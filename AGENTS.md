...
- `src/block_reclaim.rs` — the background block-reclaim queue (2026-07-27 overwrite-throughput fix): terminal frees enqueue their device reclaim (`BLKDISCARD`/`PUNCH_HOLE`) + `finish_free` here instead of issuing them on the write path; exactly-once by pop ownership, offsets stay non-reallocatable until reclaimed, ENOSPC pressure valve wired into `BlockAllocator::allocate_block`, unmount teardown drains, and the queue observes the D0 writer-guard fence (the per-volume `failed` latch, once per batch): a fenced holder ceases ALL device reclaims permanently — entries drop without `finish_free` (successor recovery owns the accounting) into `block_free_reclaim_fence_halts`. Since the 2026-07-31 write-wall campaign (`.benchmarks/2026-07-31-write-wall.md` — the field's rewrite wall: queue pinned at cap, ≥ 80 % of reclaims running inline on the write path) the worker drains across **demand-derived parallel blocking-pool lanes** (group by device → sort → consecutive-slice chunking so adjacency runs survive into each lane's coalescer; one lane per `batch_blocks` of a device's queued demand, capped per device — width = backends × demand, never a fixed funnel; pop-ownership/`processing`-reservation/finish_free-after-reclaim/fence-per-lane invariants unchanged). Knobs `SQUEEZEFS_RECLAIM_BATCH_{BLOCKS,MS}` / `SQUEEZEFS_RECLAIM_QUEUE_MAX_BLOCKS` / `SQUEEZEFS_RECLAIM_LANES_PER_DEV` (default 8); counters `block_free_reclaim_{queued,queue_bytes,batches,sync_drains,fence_halts,inline_spills,commands}` — `inline_spills` (at-cap enqueue-path processing) is a **must-stay-0** engagement tripwire, and blocks ÷ `commands` is the live coalesce factor.

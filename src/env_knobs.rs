...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_DEVICE_OVERLAY", Kind::Bool, "off", "Approach B device-backed visible overlay (design-device-overlay, PR B2): eligible fresh/hole aligned segments store slot->device at reserved unpublished offsets, publication at coverage completion/fsync. Default OFF per the B2 falsifier verdict (2026-08-09 bracket: engagement exact, 0.77x of the extraction control — ACK-after-CQE x per-inode serialization bounds in-flight custody to lanes x 1 MiB; .benchmarks/2026-08-09-device-overlay-b1-b2.md). The lever stays the counted A/B instrument with correctness green; ACK-early (kernel 0029) is the named re-bracket precondition."),
    ...
    k("SQUEEZEFS_FUSE_PLACED_MERGE", Kind::Bool, "off", "Approach A — the FUSE placed-merge assembly (write-bandwidth program, 2026-08-09): eligible armed streaming WRITE chunks bridge WRITE_FIXED(slot -> per-(ino,block) memfd assembly) and the first merging handler ADOPTS the assembly's mmap view as the ActiveBlockBuf backing. Default OFF per the bounded-falsification verdict (D17 measured posture; .benchmarks/2026-08-09-fuse-placed-merge.md): whole-cohort capture ceilings at ~25% of stream bytes on every reachable streaming shape — per-inode write serialization (kernel i_rwsem-exclusive extending direct writes, per-inode writeback flushing) sits UPSTREAM of delivery, so sibling chunks of one block never coexist and the extract/nt copy terms do not fall. `1` = the counted A/B instrument (correctness green; the machinery seeds Approach B). Engagement: fuse3_zc_write_placements/_bytes/_place_fallbacks + placed_fuse_claims/placed_adoption_refusals."),
    ...
    k("SQUEEZEFS_ZC_ACK_EARLY", Kind::Bool, "on", "ACK-early for device-overlay stores (design-device-overlay §8's accelerator): eligible overlay stores reply while the device DMA runs — the B2 falsification's named depth-cap release. Page-cache-sound writes retain the zc slot (kernel 0029, fuse3_zc_retain_commits). O_DIRECT needs SQUEEZEFS_ZC_ACK_EARLY_ODIRECT and snapshots (extract) before the reply so a post-ACK buffer reuse cannot alias dests. Engagement additionally requires SQUEEZEFS_DEVICE_OVERLAY; the ON default is inert on every shipped posture. Coverage/publication/fsync/read-wait stay CQE-anchored; acked custody retries forever (overlay_ack_early_retries), fence is the one drop (overlay_fence_drops). `0` = the A/B lever. Engagement: overlay_ack_early_stores/_bytes."),
    k("SQUEEZEFS_ZC_ACK_EARLY_ODIRECT", Kind::Bool, "off", "Lets O_DIRECT/GUP-class overlay stores ACK early. The payload is SNAPSHOTTED (extract) before the reply — bytes are sampled at ACK, so a post-ACK buffer reuse cannot change what lands (the 2026-08-09 live-smoke aliasing; NFS UNSTABLE's actual contract). Off by default (the extract is a copy the page-cache retain arm does not pay); `1` is the labeled O_DIRECT benchmark posture (R4). Engagement: overlay_ack_early_stores + fuse3_zc_write_extract_bytes (retain_commits stay on the page-cache arm)."),
    ...
    k("SQUEEZEFS_TEST_OVERLAY_BYTES", Kind::Bool, "0", "Test seam: let WritePayload::Bytes shapes ride the device-overlay store via the pooled vehicle (in-process suites have no zc slot). Never set in production — Bytes shapes ride accumulation."),

...
async fn test_sharded_url_parsing_and_routing() {
    ...
    // ino % 3
    let _conn_0 = dlm.get_connection_for_inode(3).await.expect("Failed to get connection");
    let _conn_1 = dlm.get_connection_for_inode(4).await.expect("Failed to get connection");
    let _conn_2 = dlm.get_connection_for_inode(5).await.expect("Failed to get connection");
...
async fn test_sharded_volume_formatting_and_initialization() {
    ...
    for db in 1..=3 {
        let client = redis::Client::open(format!("redis://127.0.0.1:6379/{}", db)).expect("Client open");
    ...
    let status = get_volume_status(&sharded_url).await.expect("Failed to get status");
    ...
    let mut con_1 = client_1.get_multiplexed_tokio_connection().await.expect("db 2 connection");
    let root_exists: bool = con_1.exists("squeezefs:attr:1").await.unwrap_or(false);
    assert!(root_exists, "Root inode 1 attributes not formatted on Shard 1 (db 2)");
    ...
    let mut con_2 = client_2.get_multiplexed_tokio_connection().await.expect("db 3 connection");

...
#      SQZ_BIN, SQZ_PVSET_SYMMETRIC=1 (symmetric PR 13, gate 6: `format
#      --symmetric` + every mount under SQUEEZEFS_SYMMETRIC_META=1 — the
#      forest's per-slot extent floor, slot-tree bytes, ring and page
#      cost per volume width; the default formats FLAT, byte-identical
#      to every pre-PR-13 row).
#
...
SYMMETRIC="${SQZ_PVSET_SYMMETRIC:-0}"
...
do_mount() { # n meta_uri mnt mem_budget node_cache_mb flush_ms ttl_ms
    ...
    local -a menv=()
    # The armed plane on a `--symmetric` set (gate 6); a flat set arms
    # nothing.
    [ "$SYMMETRIC" = "1" ] && menv+=("SQUEEZEFS_SYMMETRIC_META=1" "SQUEEZEFS_SYM_ALLOW_NON_PR=1")
...
create() {
    ...
    log "format: $n metadata volume(s), substrate=$substrate, tag=$tag${SYMMETRIC:+ symmetric=$SYMMETRIC}"
...
remount() {
    ...
    . "$CONF"
    # The plane the set was formatted for (a `--symmetric` set mounts
    # armed on every remount, whatever the caller's env).
    SYMMETRIC="${PVSET_SYMMETRIC:-0}"
    local t0 t1 t2
    t0="$(date +%s.%N)"

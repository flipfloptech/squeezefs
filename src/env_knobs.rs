...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_REWRITE_SUPPLY_CLOSE_PER_VOLUME", Kind::Bool, "on", "The supply-coupled close's PER-VOLUME plan (finding 15's file-per-proc residue, `.benchmarks/2026-09-07-cowriter-fpp-supply-residue.md`). A co-writer's lane exhausts and refills PER VOLUME (512 blocks per lane per volume on the s11 fleet), and a closed epoch's parked A keys return to the lane list of the volume each lives on — so the refill tick that found ONE volume short must close the epochs whose parked keys live on THAT volume. ON (the default): the asking allocator's volume selects the candidates — an epoch's weight is the keys it parks on that volume, largest first until that yield covers the deficit; an epoch parking only on other volumes is no candidate (never `bounded`), and a tick with parked keys only elsewhere declines instead of publishing an epoch that would restock a sibling. `0` = the first landing's mount-wide plan verbatim (largest epoch by total parked keys — on the fleet's ~64/36 per-volume supply skew, about that share of every close's yield restocked the volume that was not short). Inert wherever the close itself is (single-writer / authority mounts). Engagement: rewrite_shadow_supply_close_volume_blocks (the yield that landed on the asking volume, ⊆ rewrite_shadow_supply_close_blocks); ledger rewrite_shadow_supply_close_declined_offvolume."),

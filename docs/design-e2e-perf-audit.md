...
| ~~DLM #7~~ | owner `spawn_meta_join` hop — **LANDED 2026-09-08 as D-5** (`.benchmarks/2026-09-08-d5-owner-hop-and-depth.md`): the split instrument `meta_ship_owner_dispatch_ns` {queue_hop, run, wake_hop, total} first (exact-sum per dispatch, both planes), then the lever `SQUEEZEFS_META_SHIP_INLINE_SERVE` (default on) — the served frame is polled on the accepting connection's own thread, both hops 0 by construction, every in-run wake a direct unpark. In-process release, 4 clients × 256 one-verb frames under a 4 × 200 µs `sqz-meta` hog (A-B-B-A, two rolls): hop `dispatch` 768–781 µs = queue_hop 347–350 + run 414–428 + wake_hop 4.5–5.4 → inline 40–65 µs (−92…−95 %), verbs/s 2.54–2.57 k → 4.77–4.91 k, p99 1–2 ms → 128–512 µs; quiet: 47–55 → 41–50 µs, verbs/s par. The publish plane under the same hog: dispatch 1.00–1.13 ms → 441–554 µs, publishes/s +33–35 % | the fleet row (squeeze-test) is the parent's; the split's `run` term is now the owner's whole dispatch |
...
| ~~7~~ | owner `spawn_meta_join` hop — **LANDED 2026-09-08 (D-5)**: split first (`meta_ship_owner_dispatch_ns`), then executed on the accepting thread (`SQUEEZEFS_META_SHIP_INLINE_SERVE`); in-process under a lane hog the S8 dispatch fell 768–781 → 40–65 µs, both hops 0 by construction | ≤ 32 µs vs ≤ 8 µs read execute quiet; 2.0–2.3 ms on the fleet (C-2) | executed on the accepting lane; fleet row owed |

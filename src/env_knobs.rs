...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_RANGE_CUSTODY", Kind::Bool, "on", "DLM S11 (rung 15, KD-MW-7): distributed byte-range custody on the S9 custody lease — the A/B lever (design-full-multi-writer §11; the instrument stays alive both sides). On (the default), an armed co-writer's sub-file write custody rides required/desired range grants (block-aligned desired, admit-time coalescing, the §9.2 geometry cap + dlm_grant_table_bytes R5 ceiling) instead of whole-file leases, cached in the S8 token cache's range extension. Read ONLY when the mw ownership plane is armed (the SQUEEZEFS_DELEGATION precedent): `=1` on an unarmed mount is announced-inert (every range_custody gauge structurally 0), never a refusal; `=0` on an armed mount is the A/B control the S11 rows compare against — co-writer writes take whole-file custody exactly as rung 10's."),

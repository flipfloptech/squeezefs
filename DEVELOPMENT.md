...
* *Status*: Fully implemented. Large sequential writes are staged on the local NVMe `.staging` area and immediately acknowledged. Asynchronously multiplexes massive uploads directly to S3 via S3 multipart uploads. When the file is closed (`release`), active multipart uploads are finalized and the block map is updated in Garnet. Coalesces dirty blocks by inode before flushing them, avoiding read-modify-write backend fetches. Verified via `tests/checkpoint_writeback_tests.rs`.
...
* *Status*: Fully implemented. Added FUSE `ioctl` support for the custom `SQUEEZEFS_IOC_GDS_READ` command. Implemented client-process memory reading via `/proc/{pid}/mem` using the calling client's PID to resolve buffers. Refactored `read_direct` in `src/cache/gds.rs` to take `DataRouter` and download blocks on-demand (handling decryption/decompression). Fully verified with comprehensive integration tests in `tests/gds_hardening_tests.rs`.

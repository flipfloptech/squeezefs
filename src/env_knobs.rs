...
pub static KNOBS: &[Knob] = &[
    ...
    k("SQUEEZEFS_IPC_SPIN_US", int(0, 100_000), "0", "Service-thread empty-pass spin window, us, before parking. Default 0 — every nonzero setting taxes sync-lane CPU (2026-07-26 reap-economy), so it is an explicit fleet lever; re-counted 2026-08-14 (client-topology census): +6.7% median at 32-proc rand-4k fan-in on a ~17%-utilized box at 100us (A-B-B-A, engagement exact) — the park-cycle latency term; the adaptive utilization-derived form is the named successor design."),
    ...
    k("SQUEEZEFS_IL_SPARSE_BATCH_MARKS", Kind::Bool, "off", "Sparse-arm batch park marks (design-il-wake-economy OQ2): 1 = the event park's per-session mark derives from its pending population (reap_batch_wake_threshold, age-bounded at the reap quantum; single-pending sessions stay event-exact by derivation). DEFAULT OFF — counted -3..-3.5% at local defaults 2026-08-14 (delivery delay outprices syscall savings on unsaturated reapers); a FIELD measurement lever for CPU-saturated reaper fleets, never an ambient default."),

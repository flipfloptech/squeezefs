...
| `SQUEEZEFS_FREE_GRACE_DRAIN_EPOCH_STAMP` | bool | `on` | Ladder re-derivation item 2 (reader/co-writer-side): the daemon's layout cache stamps every entry with the reader's purge generation and treats a pre-step entry as a miss on every binding-serving read, so the acknowledgement's drain window drops the caches' TTL term (`D_purge` instead of `staleness + D_purge`). `0` = the caches serve to their TTL and the drain keeps `S`. Published `free_grace_drain_lag_ms`, `reader_layout_step_gen`, `reader_layout_step_misses`. |
